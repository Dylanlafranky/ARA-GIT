#!/usr/bin/env python3
"""
Script 243AW — Mode Coupling (Wave-Eating)

Based on 243AJ champion (Solar LOO 42.89).

WAVE PHYSICS ADDITION: Mode coupling — energy transfer between harmonics.
In nonlinear wave systems, the dominant mode drains amplitude from weaker
neighbors (inverse energy cascade). After computing all eps_vals independently,
we let the strongest harmonic "eat" a fraction of energy from the others.

Transfer fraction scaled by 1/φ⁴ (whisper strength) to avoid overpowering
the existing structure.

  Previous: eps_vals computed independently, no interaction
  New: dominant eps steals energy from weaker eps, creating "rich get richer"

  Half 1 (existing midline): center = 1 + acc_frac × (ARA - 1)
  Half 2 (NEW amp_scale):    amp    = 1 + snap_frac × (ARA - 1) × φ

For Solar (ARA=φ):  midline = 1.236,  amp_scale = φ = 1.618
For Clock (ARA=1):  midline = 1.000,  amp_scale = 1.000
For Consumer (1/φ): midline = 0.764,  amp_scale = 0.764

Note: midline × amp_scale = 2.0 for Solar. Perfect framework symmetry.

  Deviation from midline is scaled by amp_scale:
    shape = midline + (raw_shape - midline) × amp_scale
...
class EngineMemoryNode(OrigARANode):
    ...
    def cascade_shape(self, t):
        ...
        # ── MODE COUPLING v2: inverse cascade — long periods eat short ──
        # Energy flows from shorter periods (higher d) to longer periods (lower d).
        # Nearest-neighbor transfer only. Strength = 1/φ⁹ (whisper of a whisper).
        # Direction: always short → long (inverse cascade).
        coupling_strength = INV_PHI_9
        for j in range(len(eps_vals) - 1, 0, -1):
            # j is the shorter period, j-1 is the longer period
            # Transfer a fraction of the shorter period's energy upward
            transfer = eps_vals[j] * coupling_strength
            eps_vals[j] -= transfer
...
print("=" * 78)
print("Script 243AW — Mode Coupling (Wave-Eating)")
print("=" * 78)
print()
...
print("  Fix 5: Amplitude scale (the other half)                       ← ON")
print("  NEW:   Mode coupling v2 (inverse cascade, long eats short)    ← ON")
print(f"         Coupling strength: 1/φ⁹ = {INV_PHI_9:.6f}")
print()
...
print(f"\n{'=' * 78}")
print("SUMMARY — 243AW Mode Coupling (Wave-Eating)")
print("=" * 78)
print()
