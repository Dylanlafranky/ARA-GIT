#!/usr/bin/env python3
"""
Script 243X — Dynamic Ladder (ARA-Driven Log-Inverse)

243W showed the ladder resonance concept works (ENSO 0.39, EQ 0.42)
but the static rung-position gradient over-damped Solar's engine.
The fix: compute the ladder modifier DYNAMICALLY from the node's
instantaneous ARA, not from a fixed rung position.

The node's own ARA dictates how aggressively it gets shaken:
  High ARA (engine) → low ladder_mod → resists shaking
  Low ARA (consumer) → high ladder_mod → shaken freely
  Clock (ARA=1) → moderate

The curve is log-inverse so it's gentle near consumer territory
and steep as you approach engine territory.

Two-level gating remains:
  gear_tilt_gated = gear_tilt_raw × receiver_acc × ladder_mod(inst_ara)

Three curve variants:
  A — φ-exponential: ladder_mod = φ^(-inst_ara)
      ARA=0.15→0.93, ARA=1→0.62, ARA=φ→0.43, ARA=2→0.38

  B — Log-inverse: ladder_mod = 1/(1 + ln(1 + inst_ara))
      ARA=0.15→0.88, ARA=1→0.59, ARA=φ→0.51, ARA=2→0.48

  C — Quadratic-inverse: ladder_mod = 1/(1 + inst_ara²/φ)
      ARA=0.15→0.99, ARA=1→0.62, ARA=φ→0.38, ARA=2→0.29
      (Note: at ARA=φ this gives 1/(1+φ) = 1/φ² — self-similar!)
"""
...
# ── Dynamic ladder curves ──
def ladder_phi_exp(inst_ara):
    """φ^(-ARA): exponential decay. Gentle for consumers, steep for engines."""
    return INV_PHI ** max(0.01, inst_ara)

def ladder_log_inv(inst_ara):
    """1/(1+ln(1+ARA)): log-inverse. Compressed range, very gentle."""
    return 1.0 / (1.0 + math.log(1.0 + max(0.01, inst_ara)))

def ladder_quad_inv(inst_ara):
    """1/(1+ARA²/φ): quadratic-inverse. Nearly transparent for consumers,
    strong damping for engines. At ARA=φ gives 1/(1+φ) = 1/φ²."""
    return 1.0 / (1.0 + max(0.01, inst_ara)**2 / PHI)


LADDER_FUNCS = {
    "phi_exp":  ladder_phi_exp,
    "log_inv":  ladder_log_inv,
    "quad_inv": ladder_quad_inv,
}


class BreathingGearNode(OrigARANode):
    """
    Triangle rider node with DYNAMIC LADDER resonance.

    Two-level gating, both computed from instantaneous ARA:
      1. receiver_acc = 1/(1+inst_ara)  — the palindrome valve
      2. ladder_mod = f(inst_ara)        — ARA-driven resonance curve

    gear_tilt_gated = gear_tilt_raw × receiver_acc × ladder_mod
    """

    def __init__(self, name, period, ara, rung, base_amp, archetype,
                 seed_ara=None, rung_offset=0,
                 child_sign=1, parent_sign=-1,
                 gate_send=False, ladder_func="phi_exp"):
        ...
        self.ladder_func = LADDER_FUNCS[ladder_func]
    ...
    def cascade_shape(self, t):
        ...
        # ── Level 1: BREATHING VALVE (palindrome) ──
        receiver_acc = 1.0 / (1.0 + max(0.01, inst_ara))

        # ── Level 2: DYNAMIC LADDER MODIFIER ──
        # The ARA itself dictates how aggressively this node gets shaken.
        # High ARA → low ladder_mod → engine resists
        # Low ARA → high ladder_mod → consumer absorbs
        ladder_mod = self.ladder_func(inst_ara)
        ...
        self.ladder_mod_history.append(ladder_mod)
...
def run_breathing_gear(times, peaks, period, ara, target_rung,
    ...
    fit_tr, fit_ba, fit_mae = grid_search(
        ...
        gate_send=gate_send, ladder_func=ladder_func)
    ...
    lmod_hist = target_node.ladder_mod_history
    ...
    return {
        ...
        "avg_ladder": np.mean(lmod_hist) if lmod_hist else 0,
        "ladder_range": (min(lmod_hist), max(lmod_hist)) if lmod_hist else (0, 0),
        "avg_eff_gate": np.mean([v*l for v,l in zip(valve_hist, lmod_hist)]) if valve_hist else 0,
...
print("=" * 78)
print("Script 243X — Dynamic Ladder (ARA-Driven Log-Inverse)")
print("=" * 78)
print()
print("  gear_tilt_gated = tilt_raw × receiver_acc × ladder_mod(inst_ara)")
print("  The ARA itself dictates how hard each node gets shaken.")
print()

print("  Curve values at key ARA levels:")
print(f"  {'ARA':>6} │ {'Role':>10} │ {'acc':>6} │ {'φ-exp':>7} │ {'log-inv':>7} │ {'quad':>7} │ "
      f"{'eff(φ)':>7} │ {'eff(log)':>8} │ {'eff(quad)':>9}")
print(f"  {'─'*6}─┼─{'─'*10}─┼─{'─'*6}─┼─{'─'*7}─┼─{'─'*7}─┼─{'─'*7}─┼─{'─'*7}─┼─{'─'*8}─┼─{'─'*9}")
for ara, role in [(0.15, "deep cons"), (INV_PHI, "consumer"),
                   (1.0, "clock"), (PHI, "engine"), (2.0, "exotherm")]:
    acc = 1.0 / (1.0 + ara)
    pe = ladder_phi_exp(ara)
    li = ladder_log_inv(ara)
    qi = ladder_quad_inv(ara)
    print(f"  {ara:>6.3f} │ {role:>10} │ {acc:>6.4f} │ {pe:>7.4f} │ {li:>7.4f} │ {qi:>7.4f} │ "
          f"{acc*pe:>7.4f} │ {acc*li:>8.4f} │ {acc*qi:>9.4f}")
...
VARIANTS = {
    "A": {
        "label": "φ-exponential: φ^(-ARA)",
        "child_sign": -1, "parent_sign": -1, "gate_send": True,
        "ladder_func": "phi_exp",
    },
    "B": {
        "label": "Log-inverse: 1/(1+ln(1+ARA))",
        "child_sign": -1, "parent_sign": -1, "gate_send": True,
        "ladder_func": "log_inv",
    },
    "C": {
        "label": "Quadratic-inverse: 1/(1+ARA²/φ)  [self-similar!]",
        "child_sign": -1, "parent_sign": -1, "gate_send": True,
        "ladder_func": "quad_inv",
    },
}
...
for var_key in ["A", "B", "C"]:
    ...
    print(f"{'─' * 78}")
    print(f"\n  {'System':<16} │ {'235b':>7} │ {'243O-C':>7} │ {'243W-A':>7} │ {'243X':>7} │ "
          f"{'Δ vs 235b':>9} │ {'Δ vs O-C':>9} │ {'lad_mod':>7} │ {'eff_gate':>8}")
    print(f"  {'─'*16}─┼─{'─'*7}─┼─{'─'*7}─┼─{'─'*7}─┼─{'─'*7}─┼─{'─'*9}─┼─{'─'*9}─┼─{'─'*7}─┼─{'─'*8}")
    ...
    for name, times, peaks, period, ara, rung in SYSTEMS:
        ...
        print(f"\r  {name:<16} │ {ref['235b'][name]:>7.2f} │ {ref['243O-C'][name]:>7.2f} │ "
              f"{ref['243W-A'][name]:>7.2f} │ {res['mae']:>7.2f} │ {d235:>+8.2f}{b235} │ "
              f"{dOC:>+8.2f}{bOC} │ {res['avg_ladder']:>7.4f} │ {res['avg_eff_gate']:>8.4f}")
    ...
    for name in var_results:
        ...
        lr = r["ladder_range"]
        print(f"    {name:<16}: acc={r['avg_valve']:.4f}, ladder=[{lr[0]:.4f},{lr[1]:.4f}], "
              f"eff_gate={r['avg_eff_gate']:.4f}, "
...
print(f"\n{'=' * 78}")
print("SUMMARY — 243X Dynamic Ladder")
print("=" * 78)
print()
print("  The ladder modifier is now DYNAMIC — computed from inst_ara")
print("  at runtime, not from a static rung position.")
print("  High ARA → low ladder_mod → engine resists shaking")
print("  Low ARA → high ladder_mod → consumer absorbs shaking")
print()
print("  Two-level gating: receiver_acc × ladder_mod(inst_ara)")
print("  Both levels use the same ARA, creating resonance selectivity.")
print()
print(f"  Baselines: 235b Solar=46.30, 243O-C Solar=48.45")
print(f"  243W-A records: ENSO=0.39, EQ=0.42")
