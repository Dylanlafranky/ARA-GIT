"""
ARA seasonal + SPRING HEARTBEAT predictor
=========================================
Extends ara_seasonal_calibrated_predictor.py (ocean-only: NINO + WWV west/east + seasonal
clock + calibration) by adding the ATMOSPHERE (SOI) and a SPRING HEARTBEAT:

  heartbeat = gate(month) * (zWWV * zSOI)      gate = 1 in boreal spring (Mar-Apr-May) else 0

i.e. the ocean<->atmosphere MIXING term, switched ON only at the spring gate (the system's
clock / contact point), fed into the same walk-forward seasonal propagator. From the gate the
blend propagates both up and down the leads. Strictly causal: gate is calendar-known; SOI and
WWV standardized on train-only; refit past-only at every origin.

Modes:  ocean      = baseline (T, zW, zE) seasonal           [matches capstone]
        atmos      = + zSOI as 4th state (two systems, no gate)
        heartbeat  = atmos + spring-gated zW*zSOI mixing term  (Dylan's heartbeat)
Reports correlation (LEADS, per project rule) and MSE-skill vs climatology, held-out.
"""
...
def feats(X,m,mode):
    ...
    cols=[X, X*c[:,None], X*s[:,None], c, s, np.ones(len(X))]
    if mode=="heartbeat":
        gate=np.array([1.0 if int(mm) in SPRING else 0.0 for mm in m])
        hb=gate*(X[:,1]*X[:,3])           # zW * zSOI, spring only
        cols.append(hb[:,None])
    return np.column_stack(cols)

def walk(T,W,E,SOI,yr,mon,mode):
    nd=3 if mode=="ocean" else 4
    ...
    for i in range(len(T)):
        ...
        clim=T[idx].mean()
        B=np.linalg.lstsq(feats(X[idx][:-1],mon[idx][:-1],mode), X[idx][1:], rcond=None)[0]
        for h in range(1,HMAX+1):
            if i+h>=len(T): break
            x=X[i].copy()
            for kk in range(h):
                mm=((mon[i]-1+kk)%12)+1
                x=feats(x[None,:],np.array([mm]),mode)[0]@B
...
def main():
    ...
    res={m:calib(walk(T,Wv,Ev,Sv,yr,mon,m)) for m in ("ocean","atmos","heartbeat")}
    print("HELD-OUT CORRELATION (leads) | MSE-skill in [brackets]\n")
    print(f"{'lead':>4}  {'ocean(base)':>22} {'+atmos':>22} {'+SPRING HEARTBEAT':>22}")
    for h in (1,3,6,9,12,15,18,21,24,27):
        row=f"{h:>4} "
        for m in ("ocean","atmos","heartbeat"):
            d=res[m].get(h)
            if d: row+=f"  {d['corr']:>+6.3f} [{d['skill']:>+5.2f}]      "
            else: row+=f"  {'--':>14}      "
        print(row)
    print("\ncorr LEADS (project rule). Heartbeat = spring-gated ocean*atmosphere mixing term.")
