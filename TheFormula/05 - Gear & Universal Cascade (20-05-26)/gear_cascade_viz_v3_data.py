"""V3 viz data: actual + persistence + UC base + UC+snapback (Dylan 2026-05-18).

Snapback is Dylan's symmetric meta-snapback: when current value is within
70% of either wall, push prediction back by 0.5 × signal_std × ramp.
"""
import os, json, math
import numpy as np
import pandas as pd
...
class UC:
    def __init__(self, ara, dom_P, n_rungs=5, snapback=False,
    ...
    def predict(self, t, current_value=None):
        ...
        if self.snapback:
            base += self._snapback_force(current_value if current_value is not None else 0.0)
        return base
...
predictions = {}
for h in HORIZONS:
    uc = UC(ara=2.0, dom_P=48, n_rungs=5, snapback=False)
    uc.fit(nino, [soi_a, pdo_a], train_n, h)
    ucs = UC(ara=2.0, dom_P=48, n_rungs=5, snapback=True,
             snap_fraction=0.5, wall_threshold=0.5)
    ucs.fit(nino, [soi_a, pdo_a], train_n, h)
    uc_p, ucs_p, pers_p, act, dates = [], [], [], [], []
    for t in test_idx:
    ...
    up = np.array(uc_p); usp = np.array(ucs_p); pp = np.array(pers_p); aa = np.array(act)
    ...
    print(f'h={h:3d}: UC MAE {um:.3f} corr {uc_corr:+.3f} | UC+snap MAE {um_s:.3f} corr {ucs_corr:+.3f} | pers MAE {pm:.3f} corr {pc:+.3f}')

out = {'horizons': HORIZONS, 'predictions': predictions,
       'system': 'ENSO (NINO 3.4)', 'ara': 2.0, 'period': '48 months',
       'snap_params': {'wall_threshold': 0.5, 'snap_fraction': 0.5}}
with open(os.path.join(_HERE, 'gear_cascade_viz_v3_data.js'), 'w') as f:
    f.write('window.cascadeVizData = '); json.dump(out, f); f.write(';')
print('Saved v3 viz data')
