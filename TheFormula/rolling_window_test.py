"""
Rolling-window validation — apples-to-apples with operational ENSO forecast skill.

Protocol:
  Start: 30-year initial training window (1948-Jan to 1978-Jan)
  At each month t from 1978-Jan onward:
    1. Refit combined stack using ALL data up to t (Layer 1 + Layer 2)
    2. Predict NINO[t+12] and NINO[t+24]
    3. Record (init_month, pred, truth) tuples
  Slide forward 1 month, repeat
  Aggregate skill across all forecasts

Reports:
  - Overall correlation, MAE, RMSE, R² vs climatology, direction accuracy
  - Skill broken down by initialization month (spring predictability barrier check)
  - Comparison to persistence and climatology baselines

Caveat: bandpass features are acausal (FFT-based) computed on full series.
Edge effects ~1 period. State explicitly. A future strict-causal rerun should
recompute bandpass on truncated data at each step.
"""
...
OUT       = _resolve(r"F:\SystemFormulaFolder\TheFormula\rolling_window_data.js")
...
def bandpass(arr, period_units, dt=1.0, bw=0.4):
    ...
    freqs = np.fft.rfftfreq(n, d=dt); f_c = 1.0/period_units
...
out = dict(forecasts=forecasts_out, method="Rolling window: refit yearly, predict monthly. Combined stack architecture.",
           caveat="Bandpass features acausal (FFT on full series). Edge effects ~1 period. Train-side ARA/CF/L1/L2 fits use only training portion.",
           horizons=HORIZONS, results=results)
with open(OUT,'w',encoding='utf-8') as f:
    f.write("window.ROLLING = " + json.dumps(out, default=str) + ";\n")
print(f"\nSaved -> {OUT}")
