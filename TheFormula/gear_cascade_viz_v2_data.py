"""V2 viz data: actual + persistence + UNIVERSAL cascade (the new winner)."""
import os, json, math
import numpy as np
import pandas as pd
...
for h in HORIZONS:
    ...
    up=np.array(uc_p); pp=np.array(pers_p); aa=np.array(act)
    ...
    pm,pc=met(pp,aa)
    predictions[str(h)]={'dates':dates,'actual':[float(x) for x in act],
        'universal_cascade':up_adj.tolist(),'persistence':pers_p,
        'uc_mae':um,'uc_corr':uc_corr,'pers_mae':pm,'pers_corr':pc}
    print(f'h={h:3d}: UC MAE {um:.3f} corr {uc_corr:+.3f} | pers MAE {pm:.3f} corr {pc:+.3f}')

out={'horizons':HORIZONS,'predictions':predictions,'system':'ENSO (NINO 3.4)','ara':2.0,'period':'48 months'}
with open(os.path.join(_HERE,'gear_cascade_viz_v2_data.js'),'w') as f:
    f.write('window.cascadeVizData = '); json.dump(out,f); f.write(';')
print('Saved v2 viz data')
