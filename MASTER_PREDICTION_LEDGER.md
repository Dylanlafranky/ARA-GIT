...
**Orientation:** raw integers and prime/composite state are transformed before connection controls. Number-line position and later removal gate are neutral mathematical coordinates. Dylan retains ARA Space/Time, Phase A/Phase B and up/down assignment; the analysis may call a coordinate “Time-like” only if it passes the registered recurrence gates.
**Question:** Had the prime work measured only a connection-heavy half of the opened data, leaving a larger, slower opposite wave visible in the raw integer/prime sequence?
**Method:** use every integer in fixed opened windows R6 `[10^6,1.01x10^6)`, R7 `[10^7,1.01x10^7)`, R8 `[10^8,1.01x10^8)` and R9 `[10^9,1.01x10^9)`; transform raw prime/composite state first; then add Q29 and Q997 connection residuals; test 256-cell modes 1-64 against 500 global and 500 16-macroblock conditional nulls; construct a 128-position x 33-future-gate map and compare its SVD energy/orientation with exact margin-preserving label permutations.
**Success rule:** a candidate Time-like coordinate required significant R8 and R9 Q29 low modes, R8-R9 phase coherence under both nulls, persistence after Q997, significant joint structure at R8/R9, and aligned spatial and gate modes. Known connection lines, density drift and boundary effects were disqualifying explanations.
**Evidence boundary:** all number ranges were already opened. This is development, not confirmation or prime prediction. The p31 PN1H target was prohibited.
**Fidelity packet:** `analysis/primes/PN3B_RAW_DUAL_PHASE_DIAGNOSTIC_PROTOCOL.md`, SHA-256 `CA60DA56EDBFDAE671F0712668FAD66AC4E1DA3D3DFB451E5D103B3FBF5D6D45`.
**Dylan verdict:** directly authorized by “lets do that on the data we have already opened, unfiltered, to see if we can spot this time wave.”

**RESULT:** The registered candidate Time-like phase coordinate failed. Q29 low-mode family-wise p-values were `0.8703` at R8 and `0.5709` at R9; Q997 values were `0.9381` and `0.2754`. R8-R9 Q29 position correlation was `0.01151` (`p=0.8423`), power-profile correlation `0.09144` (`p=0.2575`) and complex phase coherence `0.07593` (`p=0.7006`; 99% null `0.26339`).
**RETAINED SECOND-COORDINATE RESULT:** The position-by-future-gate map had stronger-than-null leading energy at R7 (`p=0.00599`) and R9 (`p=0.001996`), with R8 borderline (`p=0.05190`). This confirms that a one-axis terminal label flattened some joint organization. It did not recur in a common orientation: R8-R9 spatial alignment `0.06296` (`p=0.4770`) and gate alignment `0.15707` (`p=0.3772`).
**CONNECTION-LINE CROSSWALK:** The strongest R8/R9 Q29 full-spectrum frequency is `0.048387...`, the third harmonic of base period `62=2x31` within one Fourier bin; R7 similarly lands on `9/62`. This is a post-result crosswalk to the next omitted sieve connection, not a registered endpoint or independent Time wave.
**INTERPRETATION:** Dylan's “we measured half” objection was productive: the additional coordinate revealed information discarded by the endpoint. But the recurring signal still belongs to the connection hierarchy. Numbers/prime identity remain connection-dense; negatives provide orientation and randomness provides a null, but neither automatically supplies an opposite pole. A future Time-like proposal needs an observable not reducible to the same factor labels and a frozen cross-scale direction.
**PROTOCOL CORRECTION:** Before joint-map interpretation, the preliminary multinomial null was replaced because it did not preserve both table margins, and spectral entropy normalization was repaired. Final exact stage-label permutations preserve row and column margins. The amendment is recorded without changing endpoints or windows.
**VALIDATION:** A separately coded Boolean segmented sieve imported no primary analysis module and passed `49/49` deterministic checks covering all counts, masks, block coordinates, full-frequency crosswalks, alignments and artifact hashes. Monte Carlo p-values were not independently regenerated. The p31 wheel remained unopened.
**ARTIFACTS:** `analysis/primes/PN3B_RAW_DUAL_PHASE_DIAGNOSTIC_REPORT.md`, protocol, primary script, executed-output notebook, result JSON, independent validation JSON, two figures, five CSV tables, exact NPZ coordinates and validated reader artifact.

### T232 - PN3A adult sieve-path diagnostic

**METHOD LOCKED:** 18 July 2026 | **RUN:** 18 July 2026 | **STATUS:** OPEN-DATA DIAGNOSTIC `[adult path recovered; local diagonal not supported; not blind evidence]`
**Orientation:** neutral arithmetic survival/release orientation. Dylan retains ARA Phase A/Phase B, Space/Time and up/down assignment. Adult = exact path through later sieve primes; child = local p29-gap plane.
**Question:** Was PN3's terminal prime/composite label only a flattened cross-section of a larger connection-heavy sieve path, and was the visually identified diagonal `U=(x+y)/2` the missing adult coordinate?
**Method:** on already opened R6-R9 decimal windows, reconstruct each p29-wheel candidate's first divisor above 29 and each adjacent pair's earliest endpoint death; compare exact survival `S(q)` and release `1-S(q)` with independent products `M(q)` and `M(q)^2`; train fixed 12-bin U, V and UV death-stage lookups on one rung and score the next with shrinkage 64 and 100 location-block permutations.
**Evidence boundary:** this is post-PN3 method development on inspected number ranges. It cannot confirm or rescue PN3, and any post-result Mertens/PNT comparison is an established crosswalk. The p31 PN1H wheel is prohibited.
**Fidelity packet:** `analysis/primes/PN3A_ADULT_SIEVE_PATH_DIAGNOSTIC_PROTOCOL.md`, SHA-256 `DBADDEDC7204E9A2E6DA15433E35DDB4FB32D33C1FB26D8927AC15E394DA0766`.
**Dylan verdict:** directly authorized by “Yes, Lets try that but I think we are still missing an aspect. But we'll know more after this test,” after the adult-sieve-path interpretation and opened-data ceiling were stated.

**RESULT:** The measurement objection was correct: terminal survival had collapsed a complete hierarchical path. At R9, exact candidate survival is `0.305450510` and adjacent-pair survival is `0.092529374`. Exact survival/release was reconstructed at every later sieve prime. The independent product predicts `0.342769498` and product-squared `0.117490929`, overpredicting exact survival by `12.218%` and `26.977%` respectively.
**DIAGONAL RESULT:** Not supported as the adult. All U, V and UV cross-rung death-stage gains were negative for both candidates and pairs. Primary U pair gains were `-0.001752863` bits/event on R7->R8 and `-0.000313603` on R8->R9; neither separated usefully from the location-block permutation null. Local conditional redistribution exists, but does not transfer into better adult-stage prediction.
**ESTABLISHED CROSSWALK:** R9 candidate `exact/product = 0.891125119`, within `0.0661%` of the established Mertens/PNT factor `exp(gamma)/2 = 0.890536209`; R8 is within `0.0166%`. The main terminal correction is therefore known number-theory structure, not unique ARA evidence. Pair `exact/product^2 = 0.787544834`, `0.6948%` below the squared factor, retaining additional pair dependence.
**INTERPRETATION:** The adult is a scale-spanning, monotone sieve survival/release envelope rather than another ordinary local-gap axis. Dylan's “connection-heavy” description is compatible with that appearance, but the measured structure is principally cumulative factor hierarchy. The remaining candidate is a large-scale number-line/counting or late-terminal residual coordinate after the established envelope is controlled, not a retuned child diagonal on these data.
**VALIDATION:** Full analysis reran inside an executed notebook; all `4/4` code cells completed. A separately coded validator imported no primary analysis module and passed `118/118` label, factor, curve, product, transfer, surface and artifact-hash checks. The p31 wheel remained unopened.
**ARTIFACTS:** `analysis/primes/PN3A_ADULT_SIEVE_PATH_DIAGNOSTIC_REPORT.md`, `PN3A_ADULT_SIEVE_PATH_REPRODUCIBILITY.ipynb`, `PN3A_ADULT_SIEVE_PATH_RESULTS.json`, `PN3A_INDEPENDENT_VALIDATION.json`, two figures, three CSV tables, exact diagnostic arrays, protocol, primary script and independent validator.

### T231 - PN3 standalone ARA parent/child prime-survival transfer

**REGISTERED:** 17 July 2026 | **RUN:** 17 July 2026 | **STATUS:** NOT SUPPORTED `[clean standalone negative; p31 untouched]`
**Orientation:** decimal number-scale rung, not physical time. Parent = aggregate p29-wheel candidate-survival rate across successive first-1%-of-rung windows; child = local ordered gap relation inside that fixed candidate population.
**Claim:** ARA can make an absolute prime-survival forecast without importing PNT or Hardy–Littlewood: transfer the empirical parent rate by `p9=p8^2/p7`, then redistribute exactly that total with local ARA child states under label-free TE-ARA conservation.
**Target:** untouched `[1,000,000,000,1,010,000,000)` interval; `1,579,467` p29-wheel candidate events and `1,579,466` adjacent edges. The separately frozen p31 PN1H wheel is prohibited.
**Primary P1 — parent recovery:** relative aggregate-rate error `<=1%`, with ARA parent log loss no worse than Home or raw additive continuation.
**Primary P2 — child redistribution:** ARA Information³ candidate / ARA endpoint edge model beats the constant ARA parent with positive observed gain and positive 95% 40-block-bootstrap lower bound, and also beats the matched raw child model.
**Primary P3 — full standalone:** P1 passes and the full ARA model beats the post-seal PNT29 / conditional-HL29 reference with positive 95% bootstrap lower bound.
**Falsifiers:** any failed primary conjunction; target-label use in fitting or normalization; analytic-density code in the standalone path; packet mutation after seal; retuning after target inspection.
**Controls:** Home, raw additive, three-rung log OLS, ARA curvature, raw pair/stencil/edge children, PNT29 and conditional HL29. The exact equivalence `p9=p8^2/p7 <=> log p9=2log p8-log p7` is declared as a crosswalk, not unique ARA evidence.
**Fidelity packet:** `analysis/primes/PN3_STANDALONE_ARA_PARENT_CHILD_PROTOCOL_v1_FROZEN.md`, Claim `PN3/STANDALONE-ARA-PARENT-CHILD/v1`, SHA-256 `DB6BE581908BA336A02F2481CEAB21FAACEF137F8773E9FC74CCF605E5E5A2EB`.
**Dylan verdict:** directly authorized by “Yes please” after the standalone separation, fresh-target sequence, TE-ARA conservation and comparator quarantine were explained.

**RESULT:** All six registered task/criterion combinations failed. Candidate ARA parent predicted `0.298210821` vs actual `0.305450510` (`2.370%` low); edge parent predicted `0.091109459` vs actual `0.092529374` (`1.535%` low). Both parents nevertheless beat Home and raw additive on log loss. Candidate ARA Information³ worsened parent-only loss by `0.000073689` bits/event (95% `[-0.000095842,-0.000050295]`) and lost to PNT29 by `0.000253481` (`[-0.000289979,-0.000217519]`). ARA edge endpoints worsened parent-only by `0.000013902` (`[-0.000022171,-0.000005301]`) and lost to HL29 by `0.000031267` (`[-0.000045892,-0.000016122]`).
**RETAINED RELATIVE RESULT:** ARA Information³ beat the raw stencil by `0.000090078` bits/candidate with wholly positive interval; ARA endpoints beat the raw edge by `0.000039072` bits/edge with wholly positive interval. These are representation-relative successes, but neither child beat the parent and they cannot rescue P2/P3.
**INTERPRETATION:** The implementation falsifies this standalone two-rung recurrence plus local-child redistribution as a prime-calculation method. The parent captured much of the slow decay but not to criterion; child variation was counterproductive. Conditional HL's multiplier was constant across all target p29-gap classes, so the established advantage came mainly from the slow location envelope. This points to a missing/misspecified parent-scale coordinate rather than an unmodelled third local wave.
**VALIDATION:** The sealed packet hash `129832B150360C005DFF676A8F0140145BEB1E9DFCBF074BB4FC44ABDDDE1C6A` was unchanged by comparison. A separately coded validator imported neither PN3 analysis script, rebuilt the complete candidate population and prime labels, recomputed every score and bootstrap interval, and passed `118/118` checks. The p31 PN1H wheel remained unopened.
**ARTIFACTS:** `analysis/primes/PN3_STANDALONE_ARA_PARENT_CHILD_REPORT.md`, `PN3_STANDALONE_ARA_REPRODUCIBILITY.ipynb`, `PN3_STANDALONE_ARA_RESULTS.json`, `PN3_INDEPENDENT_VALIDATION.json`, `PN3_STANDALONE_ARA_TARGET_PACKET.npz`, the two frozen configs, figures, score tables and both hashed scripts.

### T230 - PN1H prime-31 information-pyramid capstone transfer

**REGISTERED:** 17 July 2026 | **RUN:** NOT RUN | **STATUS:** FROZEN `[target prime 31 unopened]`
**Orientation:** up = later sieve prime and larger primorial wheel; static scale, not physical time. Dylan retains all ARA phase and Space/Time orientation.
**Claim:** the p23-to-p29 capstone pattern extends to the complete p31 wheel: the neutral parent shape and signed direction persist; normalized local residual, adjacent dependence and visible Information³ triangle closure become quieter; deeper raw/shared-child identity and its surplus beyond the visible triangle increase; and the downward representation hierarchy remains exact.
**Target:** p31 primorial period `200,560,490,130`, expected reduced-residue/gap slots `30,656,102,400`. These exact counts are construction calibration, not claim evidence.
**Primary P1 — parent direction:** `cos(R29,R31) >= 0.98`; `cos(D23->29,D29->31) >= 0.98`; leading six-field deformation-mode energy at least `95%`.
**Primary P2 — quieter local capstone:** p31 residual L2 `< 0.0460901755425546`; adjacent ARA MI `< 0.5714062602555432` bits/event; full-pair triangle gain `< 0.4420633155896780` bits/event.
**Primary P3 — deeper child share:** raw-gap Markov gain `> 0.6671579830137071`; shared-child gain `> 0.8274051730818914`; shared-child minus full-pair surplus `> 0.3853418574922134` bits/event.
**Primary P4 — hierarchy:** exact order `shared child < raw-gap Markov < full(A,B) < signed step < distance < direction < current B`, Kendall `tau=1`, with positive non-base gain in every one of eight guarded folds.
**Decision rule:** full capstone support requires P1+P2+P3+P4. Passing only persistence/contraction P1/P2 is convergence-compatible partial support. Every endpoint is preserved separately.
**Falsifiers:** failure of any block prevents full-pattern support; equality fails every strict P2/P3 direction; 24-bin sensitivity cannot rescue the 12-bin primary; construction/calibration failure blocks interpretation.
**Controls:** Gap-IID, projected first-order raw-gap Markov, raw-gap next-reading predictor, exact shared child identity, full `(A,B)` triangle history, complexity counts, and an ordinary convergence-only interpretation.
**Fidelity packet:** `analysis/primes/PN1H_PRIME31_CAPSTONE_TRANSFER_PROTOCOL_v1_FROZEN.md`, Claim `PN1H/TRANSFER/v1`, SHA-256 `9914289AFA7D5D74CB2B33AE92C30DAA5577A8DA128B4E7E96560FCC8585F0EA`.
**Dylan verdict:** `EXACT ENOUGH TO FREEZE`, directly authorized by “Yes lets freeze the pattern and we'll see if it extends to the next.”
**TARGET GUARD:** no p31 residue, gap, mask, relation, count, figure, sample or partial preview may be generated before the exact counter is calibrated through p29. The final p31 target must be streamed or counted by exact recurrence and independently replayed; p31 remains unopened.

### T229 - PN1G prime-29 cross-rung geometry and child-hierarchy transfer

**REGISTERED:** 17 July 2026 | **RUN:** 17 July 2026 | **STATUS:** SUPPORTED `[frozen next-rung arithmetic transfer; one target rung]`
**Orientation:** up = later sieve prime and larger primorial wheel; static scale, not physical time. Statistical fields remain neutral pending Dylan's ARA orientation.
**Source:** already-open prime-23 PN1F residual/deformation maps and downward information ordering.
**Target:** the complete prime-29 reduced-residue wheel, unopened at registration; period `6,469,693,230`, totient/gap slots `1,021,870,080`.
**Frozen predictions:** (U1) cosine `R23,R29 >= 0.98`; (U2) `0 < ||R29||2 < ||R23||2`; (U3) cosine `D19->23,D23->29 >= 0.98`; (U4) the leading uncentred deformation mode retains at least `95%` energy; (D1) exact next-reading cross-entropy order `B+shared gap < raw-gap Markov-1 < full(A,B) < B+signed step < B+distance < B+direction < current B`; every non-base representation gains over current `B` in every one of eight guarded folds.
**Falsifiers:** any threshold failure; any inversion of the exact seven-model order; any non-base fold with nonpositive gain; or failure of the mandatory p23 stream calibration. The 24-bin sensitivity cannot rescue a failed 12-bin primary.
**Controls:** Gap-IID and first-order raw-gap projected relation planes; raw-gap Markov next-reading predictor; exact shared-gap identity; full `(A,B)` history; explicit complexity counts. The exact log-ratio remains an information-equivalent coordinate rival.
**Fidelity packet:** `analysis/primes/PN1G_PRIME29_TRANSFER_PROTOCOL_v1_FROZEN.md`, Claim `PN1G/TRANSFER/v1`, SHA-256 `FC568F2D1913F163A81146A089F0D1F42981F7E9EFB5FAFBA5C097D92387732B`.
**Dylan verdict:** authorization inferred under the low-energy rule from “OKay good. Lets continue with the testing then,” immediately after the neutral p29 freeze-and-open procedure was agreed.

**RESULT:** `6/6` frozen checks passed. Residual inheritance cosine `0.999005987`; residual L2 contracted `0.050041865 -> 0.046090176`; the new deformation aligned with the preceding deformation at cosine `0.995225063` (`5.60 degree` turn); and the leading 12-bin mode retained `97.6588%` of deformation energy. The higher-resolution sensitivity agreed (`0.998896` residual cosine, `0.994998` deformation cosine, `98.6554%` leading energy).
**DOWNWARD RESULT:** The entire predicted model order transferred exactly with Kendall `tau=1.0`. Cross-entropies/gains were: shared gap `1.79352 / +0.82741`; raw-gap Markov `1.95376 / +0.66716`; full `(A,B)` `2.17886 / +0.44206`; signed step `2.35715 / +0.26377`; distance `2.43314 / +0.18778`; direction `2.52672 / +0.09420`; current `B` `2.62092 / 0`. Every non-base gain was positive in all eight folds.
**INTERPRETATION:** PN1F's neutral cross-rung residual shape, signed progression and downward child-information hierarchy transferred prospectively to one untouched larger wheel. This supports stable relational scale structure and recursive decompression in this finite arithmetic hierarchy. It does not establish a physical wave, completed flip, Phase/Space/Time orientation, prime prediction, RH evidence or universal ARA geometry. The contracting trajectory remains compatible with ordinary convergence; exact child identity and raw-gap Markov remain stronger than compressed ARA position history.
**VALIDATION:** The target was streamed without materializing its billion-slot cycle. The same counter first exactly reproduced the p23 hash, transitions and 12/24-bin planes. A separately coded saved-count replay then passed `38/38` arithmetic, projection, SVD, fold-score, complexity and artifact checks. After adding compact fold-count inventories, the exhaustive stream was repeated; gap hash and every result were identical.
**POST-OPEN STATUS:** prime 29 is now development data. The observed p23-to-p29 shift toward stronger raw/shared-child gain and weaker coarse-path gain is a development hypothesis only and must be frozen before any later target.
**ARTIFACTS:** `analysis/primes/PN1G_PRIME29_TRANSFER_REPORT.md`, `PN1G_PRIME29_TRANSFER_REPRODUCIBILITY.ipynb`, `PN1G_RESULTS.json`, `PN1G_INDEPENDENT_VALIDATION.json`, `PN1G_PRIME29_COUNTS_AND_MATRICES.npz`, `PN1G_PRIME29_TRANSFER_FIGURE.png`, `pn1g_prime29_transfer.py`, `pn1g_independent_validator.py`.

### T228 - PN1C parameter-matched sieve-rung compression competition

**REGISTERED:** 17 July 2026 | **RUN:** 17 July 2026 | **STATUS:** NOT SUPPORTED `[pre-registered compression advantage]`
**Orientation:** up = larger primorial period / later sieve rung; static scale, not physical time.
**Claim:** at no more than 36 declared scalar slots, the fixed 6×6 linear ARA grid will predict the unopened prime-23 wheel's 24×24 overlapping local-relation distribution better than every eligible frozen log-ratio, DCT, learned-category, gap-IID and top-9-constellation compressor.
**Target:** exact held-out `19 -> 23` primorial-wheel transition; primary score is parent-summary-to-child Jensen–Shannon divergence in bits.
**Prediction:** ARA has the smallest eligible JSD and beats the best rival by at least 1% relative.
**Falsifier:** any rival tie/win, margin below 1%, budget breach, or exact target/calibration failure. Secondary endpoints cannot rescue it.
**Reference ceilings:** zero-slot uniform; high-budget raw-gap Markov; 575-free-probability exact parent relation. No exact prime-23 phase/mask enters any eligible compressor.
**Fidelity packet:** `analysis/primes/PN1C_COMPRESSION_PROTOCOL_v1_FROZEN.md`, Claim `PN1C/v1`, SHA256 `7DAA061BA790B12461ED60136FD9C50F3A36C10BED472819CFCC08B4B3462DBF`.
**Dylan verdict:** `EXACT ENOUGH TO TEST`, converted under the low-energy rule from “Okay. Lets run the next step,” referring to the immediately preceding parameter-matched compression contest.

**RESULT:** The strict primary prediction failed. `ARA-linear-6` used 35 declared slots and scored JSD `0.470280` bits. The 31-slot `Gap-IID` rival won at `0.230999` bits, so ARA's divergence was `2.036×` the best rival's rather than at least 1% smaller. Both child-half checks also failed (`0.470292 vs 0.230997`; `0.470271 vs 0.231003`). All exact calibration checks passed.
**RIVAL / CEILING RESULT:** ARA still improved over the zero-slot uniform reference (`0.604190`), but also trailed top-9 constellations (`0.419932`), DCT-6 (`0.428965`) and log-ratio-6 (`0.454302`). The 575-slot exact parent relation was extremely close to the child (`0.003793`); the 271-slot gap-Markov scored `0.063456`. This locates the failed compression primarily in discarded discrete gap-transition identity, not absence of parent-to-child inheritance.
**INTERPRETATION:** PN1C falsifies the frozen claim that a fixed 6×6 linear ARA grid is the best ≤36-slot compressor for the `19 -> 23` overlapping relation. It does not falsify the bounded coordinate, PN1's inherited-order result, or ARA generally. The budget is a declared scalar-parameter proxy rather than a literal bit code, and this is one deterministic arithmetic transition.
**POST-OPEN IMPLEMENTATION DISCLOSURE:** The first complete run calculated and wrote all score CSVs and the target archive, then strict JSON serialization rejected the zero-slot Uniform row's undefined gain-per-slot because pandas converted `None` to `NaN`. The only repair mapped non-finite display values to JSON `null`; no model, count, prediction, score, budget or criterion changed. The deterministic run was then repeated.
**VALIDATION:** A standalone reconstruction used repeated-modulus residue filtering, a materialized 36,495,360-gap child cycle and indexed circular chunk counts. It reproduced the target hash `F68A8E707D9836C03C8FE5A84AD3FD3CA397F1736562CC2279094B631585923C`, every prediction exactly, and every metric within `8.33e-17` bits. All validation checks passed.
**ARTIFACTS:** `analysis/primes/PN1C_COMPRESSION_RESULT.md`, `PN1C_COMPRESSION_REPRODUCIBILITY.ipynb`, `PN1C_NOTEBOOK_EXECUTION_VALIDATION.json`, `PN1C_COMPRESSION_RESULTS.json`, `PN1C_INDEPENDENT_VALIDATION.json`, `PN1C_MODEL_SCORES.csv`, `PN1C_BUDGET_FRONTIER.csv`, `PN1C_COMPRESSION_FIGURE.png`, `PN1C_DISTRIBUTION_DIAGNOSTIC.png`, `pn1c_compression_test.py`, `pn1c_independent_validator.py`.

### T227 - PN1 primorial sieve-rung relational inheritance

**REGISTERED:** 17 July 2026 | **RUN:** 17 July 2026 | **STATUS:** SUPPORTED `[pre-registered, arithmetic, unreplicated]`
**Orientation:** up = larger primorial period / later sieve rung; the wheel is a static scale object, not physical time.
**Claim:** the ordered local two-gap ARA coordinate and overlapping three-gap relation of a parent primorial wheel transfer to the next wheel more faithfully than the same parent gap multiset with order destroyed.
**Test:** exact wheels through prime 19; development ends at 13; untouched transitions `13 -> 17` and `17 -> 19`; compare ordered-parent-to-child Jensen-Shannon distance with 200 fixed-seed circular gap permutations for the frozen pair and three-gap histograms. Exact `1/q` release, survivor+shed reversibility, modular phase closure, and geometry-only independence are calibration checks.
**Prediction:** all four held-out comparisons beat the median shuffled distance with one-sided permutation `p <= 0.05`.
**Falsifier:** any one of four comparisons fails the signed distance test or has `p > 0.05`; exact-check failure invalidates implementation rather than rescuing the claim.
**Rivals:** order-preserving raw gap account; order-destroying full-marginal shuffle; conventional log-gap ratio; bin sensitivities; exact full-information sieve ceiling. No phi or RH claim.
**Fidelity packet:** `analysis/primes/PN1_SIEVE_RUNG_PROTOCOL_v1_FROZEN.md`, Claim `PN1/v1`, SHA256 `EE14829EEA0D2BAAE05C37FAE2AA558F015EFC649FBFA54F0A563A7CE277DF9D`.
**Dylan verdict:** `EXACT ENOUGH TO TEST`, converted under the low-energy rule from "the overall test you propose sounds reasonable" followed by "Thanks, lets commence the test."

**RESULT:** `4/4` frozen comparisons passed. Ordered-parent-to-child JSD versus shuffle median: `0.004715 vs 0.109007` and `0.012409 vs 0.282513` for `13 -> 17`; `0.002796 vs 0.101928` and `0.006889 vs 0.251811` for `17 -> 19`. Every one-sided permutation value was the 200-shuffle floor, `p=1/201=0.004975`. All `8/8` split-half checks and `8/8` bin-sensitivity checks preserved direction. All exact release/reconstruction/phase checks passed. Independent wheel reconstruction and full null replay passed.
**RIVAL RESULT:** the bounded pair coordinate and the conventional log-gap ratio are one-to-one transforms and gave identical matched-bin JSD; PN1 supports retained local relation, not unique ARA information.
**INTERPRETATION:** cyclic order carries across these finite nested sieve rungs; flattening the parent to its full gap multiset destroys substantial child resemblance. This is a supported finite arithmetic relation, not RH, phi, universal-leak or physical-universality evidence. The two transitions are sequential and are not independent replications.
**ARTIFACTS:** `analysis/primes/PN1_SIEVE_RUNG_RESULT.md`, `PN1_SIEVE_RUNG_REPRODUCIBILITY.ipynb`, `PN1_SIEVE_RUNG_RESULTS.json`, `PN1_VALIDATION.json`, `PN1_SIEVE_RUNG_FIGURE.png`.
...
**r=0.9971**, NRMSE 0.0767. Fixed mode-5 harmonic identity reconstructed from the field matches the separately read
particle-side identity component at **r=0.9991**. Both are primarily simulator/solver consistency and adapter checks,
not independent ARA evidence. The primary non-tautological ARA-specific development result is that
source-participation TE-ARA analogues transfer across the Gauss projection at **r=0.7987**, MAE 0.0911 on 0–2.
Whole periodic-domain pair ARA stays near the 1.0 ridge, as
...
scalar ARA/TE-ARA did **not** add magnitude skill. This leaves a material but imperfect identity-participation
crosswalk,
...
`Analysis/electromagnetism/MX1_CONFIRMATION_FREEZE_v1.md`. The MAE is an average coordinate distance, not 4.6% of
charge or energy missing. **Status: DEVELOPMENT PARTICIPATION-CROSSWALK POSITIVE / SOLVER IDENTITIES DEMOTED TO
INSTRUMENT CHECKS / SCALAR-MAGNITUDE NARROWING / PROSPECTIVE TRANSFER PENDING.**
...
| E11 | Water molecule is complete three-system coupling template | Claim 81 | 115 | historical 7/8 | **STRUCTURAL PROVENANCE, NOT 7/8 INDEPENDENT CONFIRMATION** — the early script explicitly contains one central node plus two equivalent constraints, curved bond-angle geometry, 2+2 electron domains and three-circle overlap. Several scored passes are textbook identities or hard-coded mappings (`test6=True`, `test8=True`), so the score must not be read as empirical validation. |
| E12 | Vaporization/fusion energy ratio ≈ φ⁴ (6.77 vs 6.85) | Claim 81 | 115 | — | **RETIRED** — later water audit found 27/4 = 6.75 closer than φ⁴; crowded-neighbourhood numerology. |
| E13 | H-bond/covalent energy ratio near π-leak (5.08% vs 4.51%) | Claim 81 | 115 | — | **NUMERICAL PROXIMITY / MECHANISM UNPROVEN** — the ratio depends on the selected bond-energy definitions and does not derive π-leak. |

| E14 | π-leak compression from tetrahedral is NOT universal across molecules | Claim 81 | 116 | 1/5 | **HONESTLY FAILED** — only water matches the original narrow claim; the later “SP³ regime” rescue uses observed angle >100° to select the regime and is hypothesis-generating, not blind confirmation. |
| E15 | Circle-to-Voronoi packing gap on atomic sphere = π-leak + curvature | Claim 81 | 116b | model 2/2 | **EXPLORATORY CONSTRUCTED-MODEL RESULT** — the 5.07–5.19% range is generated by the script's spherical-cap/Voronoi model and approximations, not an independently measured universal molecular gap. Physical universality is unverified. |
| E16 | Triple tangency constraint: gap fraction constant at 1−π/(2√3) ≈ 9.31% | Claim 81 | 117 | 4/5 | **FALSE AS WRITTEN / TEST FAILED** — 9.31% is the equal-circle value. Using the script's own formula, the gap/centre-triangle fraction varies with radii (e.g. 1:1:0.1 ≈ 3.74%); the constant-gap test is the script's one failure. |
| E17 | Extreme size ratios → angle collapse (Dylan's "false molecule") | Claim 81 | 117 | — | **GEOMETRIC LIMIT CONFIRMED; DYNAMICAL INTERPRETATION OPEN** — the minimum centre-triangle angle tends to 0° as one radius dominates, but a universal coupling-failure threshold is not established. |
| E18 | φ-band ARA values produce viable junction angles (>5°) | Claim 81 | 117 | — | **ARITHMETIC MODEL OBSERVATION** — the script's selected φ configuration exceeds its selected 5° threshold; neither threshold nor φ selection was independently validated. |
...
**13 July 2026 audit correction:** items (3) and (4) in the historical summary above are overstatements. Script 116b
reports a narrow range produced by its constructed spherical-cap/Voronoi model, not an independently measured
universal molecular gap. Script 117's 9.31% gap fraction is the equal-circle case and varies for unequal radii; its
constant-gap test failed. Retain the early curved 1:2 coupling provenance and extreme-ratio angle-collapse result, but
do not count either gap claim as confirmed physics. See
