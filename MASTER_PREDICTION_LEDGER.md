...
**9 June 2026 — river landscape: conserved φ-thalweg + morphed-sphere regime map (Retrodiction/RIVER_LANDSCAPE_AND_THALWEG_RESULT.md):** Long exploratory arc from "does anything take energy OUT of ENSO?" to a validated state-space terrain. SIPHONS real (ENSO→WWV +0.011/→IOD +0.005/→PDO +0.001 = the 0.382 shed; SOI=anti-phase mirror −0.011) but FOLLOWERS — null for value (hurts post-1980), null for wobble (gap-imbalance lead +0.18 failed split-half → −0.11), drain-rate decay signature −0.12 redundant with own memory. CATCHMENT recombination = amplitude DISCRIMINATOR not recovery (24mo event-size corr +0.77 vs +0.61; spread-only, fails concentrated ECG QRS). WATERSHED: φ is fast on the slope, valley floor (fastest) at balance ~1.0, Torricelli speed∝√drop +0.25 z=1.8. φ-LANE (ARA 0.382 & 1.618, each 1/φ² off a bank) NAVIGABILITY MIGRATES: middle clean short (3-6mo), φ-lane clean mid (9-15mo +14-19%), concentration-gated (sunspots stay middle), breaks past wall. ⭐ CONSERVED φ-THALWEG VALIDATED (Dylan's calm-by-cancellation refinement): high-energy φ-lane +28-33% calmer than turbulent middle, h9-18, bootstrap P=0.98-1.00, advantage GROWS with energy (+2/+13/+31%) = beats regression-to-mean; ENSO-only, sunspots null. NULLS flagged: forward-turbulence (middle loses both value+direction), clock-centralizes-turbulence (symmetric synth puts error at extremes), collision-by-max-error (regression-to-mean confound). MORPHED SPHERE: null as value predictor (thalweg-weighted blend ≡ equal-weight; fractal cross-sections = mild ensembling only — turbulence is real energy loss not a routing problem), but ⭐ VALIDATED as REGIME MAP — causal terrain position predicts behaviour class: bank→snap (corr +0.38 directionality/+0.46 magnitude/−0.30 reversals), channel→bounded oscillation (D 0.30, reversal 0.55); ENSO clean, sunspots clock-flooded. Synthesis: morphed sphere = regime + confidence map, NOT a value predictor. Ridge→divergent-clock untested (needs 2-D field). 6 figures embedded.

**9 June 2026 — morphed-sphere geometry engine: quotes + geometry-driven tests (Retrodiction/MORPHED_SPHERE_MODEL_AND_QUOTES.md):** Recorded Dylan's verbatim model of energy movement + geometry. CORE MODEL: training morphs a FRACTAL sphere; multiple ARAs from multiple directions (the canonical axes X=Mapping, Y=Rungs[fractal, each rung its own 0-2 sub-ARA], Z=Coupling, φ-line diagonal) morph the SURFACE; φ-thalwegs = conserved HIGH ridges the energy rides, the middle 1.0 = LOW dissipative sink where opposing flows collide+cancel, banks 0/2 = snap edges; clock = symmetric banks colliding at 1.0, bend shifts the collision; turbulence is FORWARD (cancels then propelled on). RUN by calibrating position from training, then rotating+wobbling the sphere: energy rolls to the LOWEST unless its counter-wave lifts it up a ridge; the path = the forecast. TESTED: pure geometry-driven rollout (no regression) gives DIRECTION +0.40→+0.68 (beats persistence, grows with horizon); 2-axis molding ≡ bare clock (+0.00) but 3-axis (engine+wobble+energy) BEATS clock +0.02-0.07 value / +0.14 vs +0.11 @h6 — completing the geometry added real small drive; value still goes to regression (geometry delivers direction+confidence not the exact number). 3D models built in 3D Models/ (canonical-aligned morphed sphere + live rotating/wobbling run) BUT terrain still TOO SMOOTH — "keeps flattening", not finished; parked for next session. Pick-up checklist in the doc. Builds on the river arc (RIVER_LANDSCAPE_AND_THALWEG_RESULT.md, 8 Jun).
