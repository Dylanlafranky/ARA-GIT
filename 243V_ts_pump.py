#!/usr/bin/env python3
"""
Script 243V — Time-Space Energy Pump

243U's restoring valve helped (Solar 52 in variant B) but snap timing
is still wrong — duplicate predictions show the effective period is
stretched in places. The energy accumulation rate is FIXED at
dt/period, ignoring the triangle balance entirely.

New mechanism: the time-space balance in the triangle modulates
the energy pump.

  ts_balance = pos_t - pos_s   (time minus space)

  ts_balance > 0 (time dominant):
    → FEED energy → compress toward next snap → shorter effective period
  ts_balance < 0 (space dominant):
    → VENT energy → extend the gap → longer effective period
  ts_balance ≈ 0 (balanced):
    → normal fill rate

At each rung level this plays out differently because each rung sits
at a different natural time-space balance. The ladder breathes.

The energy modulation:
  energy_mod = 1 + ts_balance × φ × pump_strength
  fill_rate = (dt / period) × energy_mod

Combined with 243U-B's restoring valve (k=φ³) which showed the best
Solar trajectory.

Three pump strength variants:
  A — pump = 1/φ   (gentle)
  B — pump = 1     (moderate)
  C — pump = φ     (strong)
"""
...
class BreathingGearNode(OrigARANode):
    """
    Triangle rider node with:
    1. Restoring force valve (from 243U-B, k=φ³)
    2. Time-space energy pump (NEW)

    The pump modulates energy fill rate by the triangle's
    time-space balance. Time-dominant → feed, space-dominant → vent.
    """

    def __init__(self, name, period, ara, rung, base_amp, archetype,
                 seed_ara=None, rung_offset=0,
                 child_sign=1, parent_sign=-1,
                 gate_send=False, restore_k=PHI_3,
                 pump_strength=1.0):
        ...
        self.valve_mult_history = []
        self.ts_balance_history = []
        self.energy_mod_history = []

    def get_ts_balance(self):
        """Time - Space balance from current triangle position."""
        return self.pos_t - self.pos_s

    def get_energy_mod(self):
        """Energy pump modulation factor based on time-space balance.
        time > space → mod > 1 → faster fill (compress period)
        space > time → mod < 1 → slower fill (extend period)
        """
        ts = self.get_ts_balance()
        mod = 1.0 + ts * PHI * self.pump_strength
        # Clamp: don't go negative (can't have negative fill rate)
        # but allow it to go quite low (near-zero fill = very extended period)
        return max(0.1, mod)
    ...
    def cascade_shape(self, t):
        ...
        # ── TEMPORAL GEAR TILT (compute raw first) ──
        gear_tilt_raw = 0.0
        ...
        # ── RESTORING FORCE VALVE (from 243U-B) ──
        deviation = inst_ara - self.ara
        alignment_raw = gear_tilt_raw * deviation
        alignment = math.tanh(alignment_raw * self.restore_k)
        valve_mult = 1.0 - alignment * INV_PHI
        valve_mult = max(0.0, valve_mult)

        gear_tilt_gated = gear_tilt_raw * valve_mult

        # Track diagnostics
        ts_bal = self.get_ts_balance()
        e_mod = self.get_energy_mod()
        ...
        self.valve_mult_history.append(valve_mult)
        self.ts_balance_history.append(ts_bal)
        self.energy_mod_history.append(e_mod)
...
def grid_search(period, ara, rung, times, peaks, n_tr=80, n_ba=40,
                ...
                restore_k=PHI_3, pump_strength=1.0):
    ...
    for tr in t_refs:
        for ba in base_amps:
            ...
            node.set_t_ref(tr)
            errors = []
            for k_idx in range(len(times)):
                node.prev_amp = peaks[k_idx-1] if k_idx > 0 else None
                if k_idx > 0: node.amp_history = list(peaks[:k_idx])
                pred = node.cascade_amplitude(times[k_idx])
                errors.append(abs(pred - peaks[k_idx]))
            mae = np.mean(errors)
            if mae < best_mae:
...
def run_breathing_gear(times, peaks, period, ara, target_rung,
    ...
    fit_tr, fit_ba, fit_mae = grid_search(
        ...
        gate_send=gate_send, restore_k=restore_k,
        pump_strength=pump_strength)
    ...
    # ════════════════════════════════════════════════════════════
    # SIMULATE — with TIME-SPACE ENERGY PUMP
    # ════════════════════════════════════════════════════════════
    dt = period * 0.005
    ...
    while sim_time < sim_end:
        for name, node in all_nodes.items():
            # ── PUMPED FILL RATE ──
            # Standard: fill_rate = dt / period
            # Pumped: modulated by time-space balance
            energy_mod = node.get_energy_mod()
            fill_rate = dt / node.period * energy_mod
            node.accumulated_energy += fill_rate * node.singularity_threshold

        for name, node in all_nodes.items():
            if node.archetype in ("consumer", "clock"):
                # ── PUMPED DRAIN ──
                # When space > time, also increase drain (vent energy)
                ts = node.get_ts_balance()
                if ts < 0:
                    # Space dominant → extra drain proportional to imbalance
                    extra_drain = abs(ts) * node.pump_strength * INV_PHI
                    drain_mod = 1.0 + extra_drain
                else:
                    drain_mod = 1.0
                node.apply_drain(dt * drain_mod)
    ...
    # Compute snap spacing statistics
    if len(snap_t) > 1:
        snap_gaps = np.diff(snap_t)
        avg_gap = np.mean(snap_gaps)
        min_gap = np.min(snap_gaps)
        max_gap = np.max(snap_gaps)
    else:
    ...
    # Diagnostics
    ts_hist = target_node.ts_balance_history
    em_hist = target_node.energy_mod_history
    align_hist = target_node.alignment_history
    vm_hist = target_node.valve_mult_history
    ...
    return {
        ...
        "avg_ts_balance": np.mean(ts_hist) if ts_hist else 0,
        "ts_range": (min(ts_hist), max(ts_hist)) if ts_hist else (0, 0),
        "avg_energy_mod": np.mean(em_hist) if em_hist else 0,
        "energy_mod_range": (min(em_hist), max(em_hist)) if em_hist else (0, 0),
        "avg_valve_mult": np.mean(vm_hist) if vm_hist else 0,
        "avg_alignment": np.mean(align_hist) if align_hist else 0,
        "avg_gap": avg_gap,
        "min_gap": min_gap,
        "max_gap": max_gap,
        "snap_times": snap_t,
...
# ════════════════════════════════════════════════════════════════
# SHOW INITIAL TRIANGLE POSITIONS
# ════════════════════════════════════════════════════════════════

print("=" * 78)
print("Script 243V — Time-Space Energy Pump")
print("=" * 78)
print()
print("  Energy fill rate = (dt/period) × (1 + ts_balance × φ × pump)")
print("  ts_balance = pos_t - pos_s")
print("  time > space → faster fill → compressed period")
print("  space > time → slower fill + extra drain → extended period")
print()

# Show initial triangle positions for each system's engine
print("  Initial triangle positions (target engines):")
print(f"  {'System':<16} │ {'ARA':>5} │ {'pos_s':>6} │ {'pos_t':>6} │ {'pos_r':>6} │ {'ts_bal':>7} │ {'e_mod(1)':>8}")
print(f"  {'─'*16}─┼─{'─'*5}─┼─{'─'*6}─┼─{'─'*6}─┼─{'─'*6}─┼─{'─'*7}─┼─{'─'*8}")
for name, _, _, period, ara, rung in SYSTEMS:
    s, t, r = find_triangle_position(ara)
    ts = t - s
    e_mod = 1.0 + ts * PHI * 1.0  # pump=1
    print(f"  {name:<16} │ {ara:>5.3f} │ {s:>6.4f} │ {t:>6.4f} │ {r:>6.4f} │ {ts:>+7.4f} │ {e_mod:>8.4f}")
...
VARIANTS = {
    "A": {
        "label": "Gentle pump: strength=1/φ",
        "child_sign": -1, "parent_sign": -1, "gate_send": True,
        "restore_k": PHI_3, "pump_strength": INV_PHI,
    },
    "B": {
        "label": "Moderate pump: strength=1",
        "child_sign": -1, "parent_sign": -1, "gate_send": True,
        "restore_k": PHI_3, "pump_strength": 1.0,
    },
    "C": {
        "label": "Strong pump: strength=φ",
        "child_sign": -1, "parent_sign": -1, "gate_send": True,
        "restore_k": PHI_3, "pump_strength": PHI,
    },
}
...
ref = {
    ...
    '243U-B': {"Solar (SSN)": 52.14, "ENSO (ONI)": 0.53, "Sanriku EQ": 0.77},
}
...
for var_key in ["A", "B", "C"]:
    ...
    print(f"    gate_send={var['gate_send']}, restore_k={var['restore_k']:.3f}, "
          f"pump={var['pump_strength']:.4f}")
    print(f"{'─' * 78}")
    print(f"\n  {'System':<16} │ {'235b':>7} │ {'243O-C':>7} │ {'243U-B':>7} │ {'243V':>7} │ "
          f"{'Δ vs 235b':>9} │ {'e_mod':>6} │ {'snaps':>5} │ {'avg_gap':>8}")
    print(f"  {'─'*16}─┼─{'─'*7}─┼─{'─'*7}─┼─{'─'*7}─┼─{'─'*7}─┼─{'─'*9}─┼─{'─'*6}─┼─{'─'*5}─┼─{'─'*8}")
    ...
    for name, times, peaks, period, ara, rung in SYSTEMS:
        ...
        res = run_breathing_gear(
            ...
            restore_k=var["restore_k"],
            pump_strength=var["pump_strength"])
        ...
        print(f"\r  {name:<16} │ {ref['235b'][name]:>7.2f} │ {ref['243O-C'][name]:>7.2f} │ "
              f"{ref['243U-B'][name]:>7.2f} │ {res['mae']:>7.2f} │ {d235:>+8.2f}{b235} │ "
              f"{res['avg_energy_mod']:>6.3f} │ {res['snaps']:>5} │ {res['avg_gap']:>8.2f}")

    # Pump diagnostics
    print(f"\n  Time-space pump diagnostics:")
    for name in var_results:
        r = var_results[name]
        if r["failed"]: continue
        tsr = r["ts_range"]
        emr = r["energy_mod_range"]
        print(f"    {name:<16}: avg_ts={r['avg_ts_balance']:>+.4f}, "
              f"ts_range=[{tsr[0]:>+.4f}, {tsr[1]:>+.4f}], "
              f"avg_e_mod={r['avg_energy_mod']:.4f}, "
              f"e_mod_range=[{emr[0]:.4f}, {emr[1]:.4f}]")
        print(f"    {'':16}  snaps={r['snaps']}, "
              f"avg_gap={r['avg_gap']:.2f}, "
              f"gap_range=[{r['min_gap']:.2f}, {r['max_gap']:.2f}], "
              f"valve_mult={r['avg_valve_mult']:.4f}")

    # Solar per-cycle + snap timing
    if "Solar (SSN)" in var_results and not var_results["Solar (SSN)"]["failed"]:
        ra = var_results["Solar (SSN)"]
        print(f"\n  Solar per-cycle (Variant {var_key}):")
        print(f"    {'C':>3} {'Year':>7} {'Act':>6} │ {'Pred':>6} {'Err':>5} │ {'Nearest snap':>13}")
        snap_t_arr = ra["snap_times"]
        for i in range(len(solar_t)):
            idx = np.argmin(np.abs(snap_t_arr - solar_t[i]))
            nearest_snap_t = snap_t_arr[idx]
            time_off = nearest_snap_t - solar_t[i]
            err_flag = " ◀" if ra['errors'][i] > 60 else ""
            print(f"    C{SOLAR_CYCLES[i][0]:>2} {solar_t[i]:>7.1f} {solar_a[i]:>6.1f} │ "
                  f"{ra['preds'][i]:>6.1f} {ra['errors'][i]:>5.1f} │ {nearest_snap_t:>7.1f} ({time_off:>+5.1f}){err_flag}")
...
print(f"\n{'=' * 78}")
print("SUMMARY — 243V Time-Space Energy Pump")
print("=" * 78)
print()
print("  The energy fill rate is now modulated by the triangle's")
print("  time-space balance:")
print("    time > space → faster fill → snaps come sooner")
print("    space > time → slower fill + extra drain → snaps delayed")
print()
print("  Combined with 243U-B restoring valve (k=φ³).")
print("  Key question: does dynamic snap timing fix the period stretch?")
print()
print(f"  Baselines: 235b Solar=46.30, 243O-C=48.45, 243U-B=52.14")
print(f"  Best EQ: 243T-A=0.44, Best ENSO: 243O-C=0.41")
