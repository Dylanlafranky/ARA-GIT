#!/usr/bin/env python3
"""
Script 223n — Two Axes Combined (Horizontal + Vertical Grief)

DYLAN'S INSIGHT:
  3 coordinates. Track 2, derive the 3rd.
  Best horizontal + best vertical = third coordinate emerges.

  HORIZONTAL (mirror pair, lovers):
    Champion mirror collision: -cos_prev × cos_curr
    Same-level cascade interaction. Grief is symmetric.

  VERTICAL (parent/child, above/below):
    Asymmetric grief:
    - Parent dies (energy flows UP from below): natural, expected.
      The inner system "dies" to feed the Sun. Normal Schwabe.
    - Child dies (energy flows DOWN, cycle crashes): devastating.
      Grand minima. Exponentially worse grief.

  Currently the vertical coupling is SYMMETRIC:
    Inner pulse: ba × 1/φ³ × decay × cos(schwabe)
    Hale: -h_cc × prev_dev × exp(-φ)

  Making it ASYMMETRIC:
    When below system contributes (cos > 0): normal
    When below system drains (cos < 0): amplified by φ
    When previous cycle was weak (dev < 0): grief amplified

3 models:
  [1] Champion mirror + asymmetric inner pulse
      (negative Schwabe half amplified by φ = child dying)
  [2] Champion mirror + asymmetric Hale correction
      (weak previous cycle = child died, grief × φ)
  [3] Phase-diff collision + both vertical asymmetries
      (best horizontal LOO + both vertical grief effects)
"""
...
# =================================================================
# MODEL 1: Champion mirror + asymmetric inner pulse
# =================================================================
def mk_asym_pulse(h_cc=INV_PHI_3):
    """
    Horizontal: champion mirror collision (unchanged).
    Vertical: asymmetric inner pulse.

    Inner pulse cos(schwabe):
      cos > 0: below system contributing (parent alive) → normal
      cos < 0: below system draining (child dying) → amplified by φ

    The negative half of the Schwabe cycle hits harder.
    Grand minima = child dying = devastating grief.
    """
    def _p(years, amps, ba, tr):
        ...
        for i, t in enumerate(years):
            ...
            # Gleissberg (unchanged)
            amp += ba * INV_PHI_9 * np.cos(gp)

            # ASYMMETRIC inner pulse (vertical grief)
            sp = 2*np.pi*(t-tr)/SCHWABE
            pulse = np.cos(sp)
            if pulse < 0:
                pulse *= PHI  # child dying = amplified grief
            amp += ba * INV_PHI_3 * sing_pulse_decay(sp, PHI) * pulse

            # Hale correction (unchanged)
            if i > 0 and h_cc != 0:
...
# =================================================================
# MODEL 2: Champion mirror + asymmetric Hale correction
# =================================================================
def mk_asym_hale(h_cc=INV_PHI_3):
    """
    Horizontal: champion mirror collision (unchanged).
    Vertical: asymmetric Hale correction.

    Hale correction: -h_cc × prev_dev × exp(-φ)
      prev_dev > 0 (previous cycle strong): parent thriving,
        natural correction downward. Normal grief (1×).
      prev_dev < 0 (previous cycle weak): child dying,
        devastating grief. Correction amplified by φ.

    Weak cycles propagate grief forward more powerfully
    than strong cycles propagate their success.
    """
    def _p(years, amps, ba, tr):
        preds = []
        for i, t in enumerate(years):
            ...
            # Inner pulse (unchanged)
            sp = 2*np.pi*(t-tr)/SCHWABE
            amp += ba * INV_PHI_3 * sing_pulse_decay(sp, PHI) * np.cos(sp)

            # ASYMMETRIC Hale correction (vertical grief)
            if i > 0 and h_cc != 0:
                prev_dev = (amps[i-1] - ba) / ba
                grief_mult = PHI if prev_dev < 0 else 1.0  # child dying
...
# =================================================================
# MODEL 3: Phase-diff + both vertical asymmetries
# =================================================================
def mk_phasediff_both(h_cc=INV_PHI_3):
    """
    Best horizontal (phase-diff, LOO=33.20) + both vertical griefs.

    Horizontal: cos(Δphase) collision (ARA of the wave)
    Vertical 1: asymmetric inner pulse (child dying amplified)
    Vertical 2: asymmetric Hale (weak cycle grief amplified)

    Two axes fully mapped → third coordinate emerges.
    """
    def _p(years, amps, ba, tr):
        preds = []
        for i, t in enumerate(years):
            ...
            for j, per in enumerate(PERIODS):
                ...
                # Phase-difference collision (best horizontal LOO)
                if j > 0:
...
if __name__ == '__main__':
    t0 = time.time()
    print("="*70)
    print("SCRIPT 223n — TWO AXES COMBINED")
    print("="*70)
    print(f"""
  HORIZONTAL: mirror collision / phase-diff (best LOO models)
  VERTICAL:   asymmetric grief (parent/child)
    - Parent dies (energy up): natural → 1× correction
    - Child dies (energy down): devastating → φ× correction

  Map 2 coordinates → derive the 3rd.
""")

    results = []

    # [1] Champion mirror + asymmetric pulse
    print("[1/3] Champion mirror + asymmetric inner pulse...")
    fn1 = mk_asym_pulse(h_cc=INV_PHI_3)
    r = evaluate("Mirror+asym pulse", fn1)
    results.append(r)
    print(f"  LOO={r['loo']:.2f}, r={r['rr']:+.3f}, s={r['sw']}/7  [{time.time()-t0:.0f}s]")

    # [2] Champion mirror + asymmetric Hale
    print("\n[2/3] Champion mirror + asymmetric Hale...")
    fn2 = mk_asym_hale(h_cc=INV_PHI_3)
    r = evaluate("Mirror+asym Hale", fn2)
    results.append(r)
    print(f"  LOO={r['loo']:.2f}, r={r['rr']:+.3f}, s={r['sw']}/7  [{time.time()-t0:.0f}s]")

    # [3] Phase-diff + both
    print("\n[3/3] Phase-diff + both vertical griefs...")
    fn3 = mk_phasediff_both(h_cc=INV_PHI_3)
    r = evaluate("PhaseDiff+both", fn3)
    results.append(r)
    print(f"  LOO={r['loo']:.2f}, r={r['rr']:+.3f}, s={r['sw']}/7  [{time.time()-t0:.0f}s]")
