#!/usr/bin/env python3
"""
Script 243AL — Bouncing Ball: Carry-Forward Amplitude

Based on 243AJ (static midline amp_scale, Solar LOO 42.89).

Like a ball bouncing once thrown — the energy doesn't vanish at the peak,
it transfers into the next bounce. The difference between the φ target
and what the midline can sustain (φ - midline = 1/φ²) carries forward
through inst_ara (= prev_amp / base_amp).

  Center: STATIC midline (stable pivot point)
  Amplitude: DYNAMIC — uses ara_midline(inst_ara) instead of static midline

When previous cycle ran hot (inst_ara > ARA): dynamic_amp > midline → bigger bounce
When previous cycle ran cold (inst_ara < ARA): dynamic_amp < midline → smaller bounce
When normal (inst_ara ≈ ARA): dynamic_amp ≈ midline (same as 243AJ champion)
...
class EngineMemoryNode(OrigARANode):
    ...
    def cascade_shape(self, t):
        ...
        # ── FIX 5: Bouncing ball — carry-forward amplitude ──
        # The excess energy (φ - midline) can't be sustained statically,
        # so it carries into the NEXT cycle via inst_ara.
        # When inst_ara ≈ ARA → dynamic_amp ≈ midline (same as 243AJ)
        # When inst_ara > ARA → previous cycle was strong → boost amplitude
        # When inst_ara < ARA → previous cycle was weak → dampen amplitude
        dynamic_amp = ara_midline(inst_ara)
        deviation = shape - self.midline
        shape = self.midline + deviation * dynamic_amp
...
print("=" * 78)
print("Script 243AL — Carry-Forward Amplitude (Bouncing Ball)")
print("=" * 78)
print()
