#!/usr/bin/env python3
"""
Script 243AF-A — Static ARA Tension Selection

From 226 v4: tension type (standard vs log) should be chosen by the
system's STATIC ARA (self.ara), not the dynamic inst_ara.

Solar (ARA=φ) should ALWAYS get standard tension, even when inst_ara
dips below 1.0 during weak cycles. Currently the code uses inst_ara,
which incorrectly compresses engine dynamics during weak phases.

  Change: `inst_ara >= 1.0` → `self.ara >= 1.0` in tension selection
...
class EngineMemoryNode(OrigARANode):
    ...
    def cascade_shape(self, t):
        ...
        # ── No memory buffer — use prev_amp only (original 243AB-C) ──
        ...
        for j in range(len(phases)):
            ...
            tens = -sin_vals[j]
            if self.ara >= 1.0:  # STATIC ARA — engines always get standard tension
...
print("=" * 78)
print("Script 243AF-A — Ablation: Static ARA Tension Selection")
print("=" * 78)
print()
print("  Static ARA tension: self.ara >= 1.0 (not inst_ara)           ← ON")
print("  Memory buffer                                                ← OFF")
print("  Midline reintegration                                        ← OFF")
print("  Grief bleed / ARA-dependent momentum                         ← OFF")
print()
...
print(f"\n  {'System':<17} │ {'235b':>7} │ {'243AB-C':>7} │ {'243AF-A':>7} │ {'Δ vs 235b':>9} │ {'Δ vs AB-C':>9}")
...
print(f"\n  {'System':<17} │ {'Prev LOO':>8} │ {'Best Ever':>9} │ {'AF-A LOO':>9} �� "
...
print(f"\n{'=' * 78}")
print("SUMMARY — 243AF-A Static ARA Tension")
print("=" * 78)
print()
