#!/usr/bin/env python3
"""
Script 243AF-B — Consumer Wobble Gate

From 226 v4: consumers (ARA < 1.0) get Schwabe-frequency wobble on their
gate accumulator. Wobble amplitude scales with distance below 1.0:
  ara_distance = max(0.0, 1.0 - self.ara)
  v_wobble = 1/φ * ara_distance * sin(sp) * 1/φ
  h_wobble = 1/φ * ara_distance * cos(sp * 1/φ) * 1/φ

Engines (ARA ≥ 1.0) get zero wobble — they run free.

  All other fixes OFF (no memory buffer, no midline, no grief bleed).
"""
...
class EngineMemoryNode(OrigARANode):
    """
    Node with consumer wobble gate from 226 v4.
    All other fixes OFF (no memory buffer, no midline, no grief bleed).
    ...
    def __init__(self, name, period, ara, rung, base_amp, archetype,
        ...
        # FIX 4 DISABLED: midline = 1.0 (no shift)
        self.midline = 1.0
    ...
    def cascade_shape(self, t):
        ...
        # ── FIX 1 DISABLED: No memory buffer, use prev_amp directly ──
        ...
        # ── CONSUMER WOBBLE GATE (from 226 v4) ──
        # Only consumers (ARA < 1.0) wobble; engines run free
        ara_distance = max(0.0, 1.0 - self.ara)
        v_wobble = INV_PHI * ara_distance * math.sin(sp) * INV_PHI
        h_wobble = INV_PHI * ara_distance * math.cos(sp * INV_PHI) * INV_PHI
        acc = max(0.01, min(0.99, acc + v_wobble + h_wobble))
...
print("=" * 78)
print("Script 243AF-B — Ablation: Consumer Wobble Gate")
print("=" * 78)
print()
print("  Consumer wobble gate (ara_distance scaling) (φ-decaying rolling average)  ← ON")
print("  Fix 2: Grief bleed                                           ← OFF")
print("  Fix 3: ARA-dependent momentum                                ← OFF")
print("  Fix 4: Midline reintegration                                 ← OFF")
print()
...
print(f"\n  {'System':<17} │ {'235b':>7} │ {'243AB-C':>7} │ {'243AF-B':>7} │ {'Δ vs 235b':>9} │ {'Δ vs AB-C':>9}")
...
print(f"\n  {'System':<17} │ {'Prev LOO':>8} │ {'Best Ever':>9} │ {'AF-B LOO':>9} �� "
...
print(f"\n{'=' * 78}")
print("SUMMARY — 243AF-B Memory Buffer Only")
print("=" * 78)
print()
