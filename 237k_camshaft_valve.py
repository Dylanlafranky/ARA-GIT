#!/usr/bin/env python3
"""
Script 237k — Camshaft Valve (φ-Palindrome Midline)

Dylan's insight: near-clock systems (ARA ≈ 1) don't experience wave
collision — their up/down energy streams point to OPPOSITE vertices
of the triangle. Like a camshaft, the energy transfer has perfect φ
timing. The palindrome: forward energy = backward energy, perfectly
offset in geometric space.

The key observation: "the closer the two numbers [ARA and 1/ARA],
the further apart they are on the triangle — almost exact opposites."

This means:
  - Near ARA=1: streams are orthogonal on triangle → no midline shift
  - Far from ARA=1: streams converge toward same vertex → dominant
    direction creates midline shift
  - The transition follows φ-geometry (rungs on the φ-ladder)

phi_dist = |ln(ARA)| / ln(φ)   (distance from clock in φ-rungs)

At ARA=1: phi_dist = 0 (at clock)
At ARA=φ: phi_dist = 1 (one rung out)
At ARA=φ²: phi_dist = 2 (two rungs)
At ARA=0.15: phi_dist ≈ 3.94 (deep consumer)
At ARA=1.35: phi_dist ≈ 0.62 (less than one rung — still in palindrome zone)

Variants:
A) Square ramp: factor = min(1, phi_dist²)
   Gentle near clock, snaps to 1.0 at first full rung.

B) Cubic ramp: factor = min(1, phi_dist³)
   Even more gentle near clock.

C) φ-zone threshold: factor = max(0, (phi_dist - 1/φ) / phi_dist)²
   Zero inside the 1/φ-rung zone, quadratic ramp outside.
   1/φ ≈ 0.618 rungs = the "palindrome zone" width.
   Heart at 0.624 rungs is BARELY outside.

D) Sigmoid φ-ramp: factor = phi_dist^φ / (1 + phi_dist^φ)
   Smooth sigmoid centered at phi_dist = 1.
"""
...
def midline_inverse_valve(ara):
    """237h: engines use ARA, consumers use 1/ARA."""
    if ara >= 1.0:
...
def _phi_dist(ara):
    """Distance from clock in φ-rungs."""
    a = max(0.01, ara)
    return abs(math.log(a)) / math.log(PHI)


def midline_camshaft_A(ara):
    """A) Square ramp: gentle near clock, full at 1 rung."""
    inv_offset = midline_inverse_valve(ara) - 1.0
    pd = _phi_dist(ara)
    factor = min(1.0, pd * pd)
    return 1.0 + inv_offset * factor


def midline_camshaft_B(ara):
    """B) Cubic ramp: very gentle near clock."""
    inv_offset = midline_inverse_valve(ara) - 1.0
    pd = _phi_dist(ara)
    factor = min(1.0, pd * pd * pd)
    return 1.0 + inv_offset * factor


def midline_camshaft_C(ara):
    """C) φ-zone threshold: zero inside 1/φ rung, ramp outside.

    The palindrome zone = [1/φ^(1/φ), φ^(1/φ)] ≈ [0.743, 1.346].
    Inside: perfect energy transfer → midline = 1.0.
    Outside: ramp up quadratically.
    """
    inv_offset = midline_inverse_valve(ara) - 1.0
    pd = _phi_dist(ara)
    zone = 1.0 / PHI  # palindrome zone width in φ-rungs
    if pd <= zone:
        return 1.0
    excess = (pd - zone) / pd
    factor = excess * excess
    return 1.0 + inv_offset * factor


def midline_camshaft_D(ara):
    """D) Sigmoid φ-ramp: smooth transition centered at 1 rung.

    factor = pd^φ / (1 + pd^φ)
    At pd=0: factor=0
    At pd=1: factor = 1/(1+1) = 0.5
    At pd→∞: factor→1
    """
    inv_offset = midline_inverse_valve(ara) - 1.0
    pd = _phi_dist(ara)
...
print("=" * 78)
print("237k — Camshaft Valve (φ-Palindrome Midline)")
print("=" * 78)
print()
print("MIDLINE VALUES:")
print(f"  {'System':<12} {'ARA':>6} {'φ-dist':>7} {'InvV':>8} "
      f"{'Sq-A':>8} {'Cu-B':>8} {'Zone-C':>8} {'Sig-D':>8}")
print(f"  {'─'*12} {'─'*6} {'─'*7} {'─'*8} "
      f"{'─'*8} {'─'*8} {'─'*8} {'─'*8}")
for name, a in test_aras:
    pd = _phi_dist(a)
    print(f"  {name:<12} {a:>6.3f} {pd:>7.3f} "
          f"{midline_inverse_valve(a):>8.4f} "
          f"{midline_camshaft_A(a):>8.4f} "
          f"{midline_camshaft_B(a):>8.4f} "
          f"{midline_camshaft_C(a):>8.4f} "
          f"{midline_camshaft_D(a):>8.4f}")
print()
...
# ================================================================
# RUN — lean: top 3 + baseline
# ================================================================

configs = [
    ("Baseline",   predict_baseline),
    ("InvValve",   make_predict_fn(midline_inverse_valve)),
    ("Camshaft-C", make_predict_fn(midline_camshaft_C)),
    ("Camshaft-D", make_predict_fn(midline_camshaft_D)),
]
...
for label in labels[1:]:
    ...
    print(f"  {label:<14} helps [{', '.join(helps or ['none'])}]  {status}")
