#!/usr/bin/env python3
"""
Script 243d — Atom Rider with Step-Decayed Pipe Transfer

Key insight from 243c failure: The whisper is too small to matter.
The EQ win in 243b came from GEOMETRIC ENGINE ARAs, not the whisper.
243c killed that win by reverting engines to proven values.

So: keep 243b's geometric ARAs (the thing that actually helped),
but apply Dylan's step decay to the PIPE TRANSFER RATES between rungs:

  - Pipe coupling decays (1/φ) per rung step from the target
  - Rung 0→1: full coupling (INV_PHI_4)
  - Rung 1→2: INV_PHI_4 × (1/φ) = INV_PHI_5
  - Rung 2→3: INV_PHI_4 × (1/φ)² = INV_PHI_6
  - Directional: 2φ down / φ up pipe capacity preserved

This should:
  1. Keep EQ's massive improvement (geometric ARAs intact)
  2. Help Solar by reducing distant-rung interference
  3. The step decay means nearby rungs dominate, distant ones fade

Champion to beat: Solar MAE = 28.11 (triangle rider 236j)
243b results: Solar 56.84 (worse), ENSO 0.43, EQ 0.60 (-49.9%)
243c results: ALL WORSE — proved whisper isn't the mechanism
"""
...
# ════════════════════════════════════════════════════════════════
# ATOM RIDER NODE WITH FIELD WHISPER (from 243b, proven for EQ)
# ════════════════════════════════════════════════════════════════

class AtomPipeNode(OrigARANode):
    """
    Triangle rider + connection field whisper (243b style).
    The pipe decay happens at the WIRING level, not here.
    """
    ...
    def cascade_shape(self, t):
        ...
        # ══ CONNECTION FIELD WHISPER (243b style) ══
        # BUT with step-decayed partner contributions
        if self.field_partners:
            field_sum = 0.0
            for partner_period, coupling_weight, direction, hops in self.field_partners:
                ...
                if direction == 'horizontal':
                    field_sum -= partner_cos * coupling_weight * decay
                elif direction == 'vertical_down':
                    field_sum += partner_cos * coupling_weight * decay * INV_PHI
                elif direction == 'vertical_up':
                    field_sum += partner_cos * coupling_weight * decay * PHI
...
# ════════════════════════════════════════════════════════════════
# BUILD ATOM VEHICLE WITH PIPE DECAY
# ════════════════════════════════════════════════════════════════

def build_rung_ladder_pipe_decay(target_rung, target_period, seed_ara):
    """
    Build φ-ladder with GEOMETRIC ARAs (from 243b, the thing that
    actually helped EQ) and STEP-DECAYED pipe transfer rates.
    """
    mirror_floor = 0.15
    ...
    for offset in range(-2, 5):
        ...
        # GEOMETRIC ARAs from connection field (kept from 243b)
        ...
        # PIPE DECAY: transfer rate decays with distance from target
        dist = abs(offset)
        pipe_decay = INV_PHI ** dist  # 1/φ per step

        rungs.append((r, p, prefix, engine_ara, consumer_ara, offset, pipe_decay))

    return sorted(rungs, key=lambda x: -x[0])


def grid_search_pipe_decay(period, ara, rung, times, peaks, n_tr=80, n_ba=40):
    ...
    for tr in t_refs:
        for ba in base_amps:
            node = AtomPipeNode("temp", period, ara, rung, ba, "engine")
            node.set_t_ref(tr)
            errors = []
...
def run_pipe_decay_vehicle(times, peaks, period, ara, target_rung,
    ...
    all_nodes = {}
    engine_nodes = {}
    pipe_rates = {}  # Track actual pipe rates for diagnostics

    for rung_power, rung_period, prefix, eng_ara, cons_ara, offset, pipe_decay in rungs:
        for archetype, ara_val in [("engine", eng_ara), ("clock", 1.0),
            ...
            node = AtomPipeNode(name, rung_period, ara_val,
    ...
    # Wire connections WITH STEP-DECAYED PIPE RATES
    rung_list = [(rp, prefix, offset, pd)
                 for rp, _, prefix, _, _, offset, pd in rungs]

    for i, (rp, prefix, offset, pd) in enumerate(rung_list):
        engine = all_nodes[f"{prefix}_engine"]
        clock_n = all_nodes[f"{prefix}_clock"]
        consumer = all_nodes[f"{prefix}_consumer"]

        # Horizontal coupling: same rate but decayed by distance from target
        h_rate = PHI_LEAK * pd
        consumer.add_drain(engine, rate=h_rate)
        clock_n.add_drain(engine, rate=h_rate)
        engine.add_feed_target(clock_n, INV_PHI_4 * pd, "horizontal")
        clock_n.add_feed_target(consumer, INV_PHI_4 * pd, "horizontal")
        ...
        # Vertical coupling: ALSO step-decayed
        if i < len(rung_list) - 1:
            below_prefix = rung_list[i + 1][1]
            below_pd = rung_list[i + 1][3]
            # Average the two rungs' decay for the pipe between them
            avg_decay = (pd + below_pd) / 2
            v_rate = INV_PHI_4 * avg_decay
            engine.add_feed_target(all_nodes[f"{below_prefix}_engine"],
                                   v_rate, "vertical_down")
            pipe_rates[prefix]['vertical_down'] = v_rate

        if i > 0:
            above_prefix = rung_list[i - 1][1]
            above_pd = rung_list[i - 1][3]
            avg_decay = (pd + above_pd) / 2
            v_rate_up = PHI_LEAK * INV_PHI * avg_decay
            consumer.add_drain(all_nodes[f"{above_prefix}_engine"],
                              rate=v_rate_up)
            pipe_rates[prefix]['vertical_up'] = v_rate_up
...
print("=" * 78)
print("Script 243d — Atom Rider with Step-Decayed Pipe Transfer")
print("=" * 78)
print()
print("From 243c: whisper isn't the mechanism. Geometric ARAs are.")
print("From 243b: geometric ARAs help EQ (-49.9%) but hurt Solar (+22.8%)")
print()
print("NEW: Pipe transfer rates decay (1/φ)^dist from target rung")
print("  Target rung:    full coupling (1/φ⁴)")
print(f"  ±1 rung:        1/φ⁴ × 1/φ  = 1/φ⁵ = {INV_PHI_5:.5f}")
print(f"  ±2 rungs:       1/φ⁴ × 1/φ² = 1/φ⁶ = {INV_PHI_6:.5f}")
print("  This reduces distant-rung interference on Solar")
print()

# Show pipe decay structure
print("PIPE DECAY STRUCTURE:")
print("─" * 78)
for offset in range(-2, 5):
    dist = abs(offset)
    decay = INV_PHI ** dist
    rate = INV_PHI_4 * decay
    bar = "█" * int(decay * 20)
    label = {-2: "SubSub", -1: "BelowT", 0: "TARGET",
             1: "AboveT", 2: "AboveA", 3: "AboveB", 4: "Gleissberg"}.get(offset, f"R{offset}")
    print(f"  {label:<12} offset={offset:+d}  decay={decay:.4f}  rate={rate:.6f}  {bar}")

print()

# Run all systems
print(f"  {'System':<16} │ {'235b Base':^22} │ {'PipeDecay':^22} │ {'243b Atom':^10} │ {'Δ vs Base':>9}")
print(f"  {'─'*16}─┼─{'─'*22}─┼─{'─'*22}─┼─{'─'*10}─┼─{'─'*9}")

# 243b reference results (from previous run)
...
for name, times, peaks, period, ara, rung in SYSTEMS:
    ...
    res_pipe = run_pipe_decay_vehicle(times, peaks, period=period, ara=ara,
                                       target_rung=rung, system_name=name)

    all_results[name] = {"base": res_base, "pipe": res_pipe}

    if res_base["failed"] or res_pipe["failed"]:
        print(f"\r  {name:<16} │ {'FAIL':^22} │ {'FAIL':^22} │")
        continue

    delta = res_pipe["mae"] - res_base["mae"]
    better = " ✓" if delta < 0 else ""
    ref = ref_243b.get(name, 0)
    print(f"\r  {name:<16} │ MAE={res_base['mae']:>6.2f} r={res_base['corr']:>+.3f}  │ "
          f"MAE={res_pipe['mae']:>6.2f} r={res_pipe['corr']:>+.3f}  │ "
          f"243b={ref:>5.2f} │ {delta:>+6.2f}{better}")


# Solar per-cycle
print(f"\n{'─' * 78}")
print("SOLAR PER-CYCLE — Base vs Pipe Decay Atom")
print("─" * 78)

rr = all_results["Solar (SSN)"]["base"]
ra = all_results["Solar (SSN)"]["pipe"]

if not rr["failed"] and not ra["failed"]:
    print(f"\n  {'C':>3} {'Year':>7} {'Act':>6} │ {'Base':>6} {'Err':>5} │ "
          f"{'Pipe':>6} {'Err':>5} │ {'Win':>5}")
    ...
    for i in range(len(solar_t)):
        re_b = rr["errors"][i]
        pe = ra["errors"][i]
        win = "Pipe" if pe < re_b else "Base" if re_b < pe else "Tie"
        if pe < re_b: pw += 1
        elif re_b < pe: bw += 1
        print(f"  C{SOLAR_CYCLES[i][0]:>2} {solar_t[i]:>7.1f} {solar_a[i]:>6.1f} │ "
              f"{rr['preds'][i]:>6.1f} {re_b:>5.1f} │ "
              f"{ra['preds'][i]:>6.1f} {pe:>5.1f} │ {win:>5}")

    print(f"\n  Base wins: {bw}  |  Pipe wins: {pw}")


# Pipe rate diagnostics
print(f"\n{'─' * 78}")
print("PIPE RATE DIAGNOSTICS")
print("─" * 78)
for name in all_results:
    rp = all_results[name]["pipe"]
    if rp["failed"] or "pipe_rates" not in rp:
        continue
    print(f"\n  {name}:")
    for prefix, rates in sorted(rp["pipe_rates"].items(),
                                  key=lambda x: -x[1].get('decay', 0)):
        decay = rates.get('decay', 0)
        h = rates.get('horizontal', 0)
        vd = rates.get('vertical_down', 0)
        vu = rates.get('vertical_up', 0)
        print(f"    {prefix:<12} decay={decay:.4f}  H={h:.6f}  "
              f"V↓={vd:.6f}  V↑={vu:.6f}")


# Summary
print(f"\n{'=' * 78}")
print("SUMMARY — 243d Pipe Decay Atom")
print("=" * 78)

print(f"\n  {'System':<16} {'235b':>8} {'243b':>8} {'243c':>8} {'243d':>8} {'Best':>8}")
...
for name in all_results:
    ...
    rp = all_results[name]["pipe"]
    if rb["failed"] or rp["failed"]:
        continue
    b243b = ref_243b.get(name, 0)
    b243c = ref_243c.get(name, 0)
    vals = [rb['mae'], b243b, b243c, rp['mae']]
    best_val = min(vals)
    labels = ['235b', '243b', '243c', '243d']
    best_label = labels[vals.index(best_val)]
    print(f"  {name:<16} {rb['mae']:>8.2f} {b243b:>8.2f} {b243c:>8.2f} "
          f"{rp['mae']:>8.2f} {best_label:>8}")

ra_solar = all_results["Solar (SSN)"]["pipe"]
if not ra_solar["failed"]:
    print(f"\n  vs Champion (28.11): 243d = {ra_solar['mae']:.2f} "
