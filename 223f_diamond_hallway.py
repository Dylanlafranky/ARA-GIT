#!/usr/bin/env python3
"""
Script 223f — Diamond Hallway (Rectangle-Triangle-Rectangle)

DYLAN'S INSIGHT:
  The hallway isn't a pure triangle — it's rectangle, then triangle,
  then rectangle. Wide at the ends, narrow in the MIDDLE.

  φ¹¹: rectangle (wide, even)     → low pressure
  φ⁹:  triangle narrows           → HIGH pressure (chokepoint)
  φ⁶:  triangle narrows           → HIGH pressure (chokepoint)
  φ⁴:  rectangle opens back up    → low pressure

  Like a diamond or lens shape. The pressure peaks at the MIDDLE
  periods, not at the extremes.

  Combined with mirror collision from 223d (champion).

2 models:
  [1] Gentle diamond: [1.0, √φ, √φ, 1.0]
  [2] Strong diamond: [1.0, φ, φ, 1.0]
"""
...
def mk_model(h_cc=INV_PHI_3, shape_pressure=None):
    """
    Mirror collision cascade with optional hallway shape pressure.
    shape_pressure: list of 4 pressure values for each cascade period.
    """
    if shape_pressure is None:
        shape_pressure = [1.0, 1.0, 1.0, 1.0]
    ...
    def _p(years, amps, ba, tr):
        ...
        for i, t in enumerate(years):
            ...
            for j, per in enumerate(PERIODS):
                ...
                # Coupling with hallway shape pressure
                eps = INV_PHI_4 * gate * shape_pressure[j]
...
if __name__ == '__main__':
    t0 = time.time()
    print("="*70)
    print("SCRIPT 223f — DIAMOND HALLWAY (RECT-TRI-RECT)")
    print("="*70)

    GENTLE = [1.0, SQRT_PHI, SQRT_PHI, 1.0]
    STRONG = [1.0, PHI, PHI, 1.0]

    print(f"""
  Rectangle → Triangle → Rectangle
  Pressure peaks in the MIDDLE of the cascade.

  Gentle diamond: [{', '.join('%.3f'%p for p in GENTLE)}]
  Strong diamond: [{', '.join('%.3f'%p for p in STRONG)}]
  (√φ = {SQRT_PHI:.3f})
""")

    results = []

    # [1] Gentle diamond: √φ in middle
    print("[1/2] Gentle diamond (√φ middle)...")
    fn1 = mk_model(h_cc=INV_PHI_3, shape_pressure=GENTLE)
    r = evaluate("Diamond √φ", fn1)
    results.append(r)
    print(f"  LOO={r['loo']:.2f}, r={r['rr']:+.3f}, s={r['sw']}/7  [{time.time()-t0:.0f}s]")

    # [2] Strong diamond: φ in middle
    print("\n[2/2] Strong diamond (φ middle)...")
    fn2 = mk_model(h_cc=INV_PHI_3, shape_pressure=STRONG)
    r = evaluate("Diamond φ", fn2)
    results.append(r)
    print(f"  LOO={r['loo']:.2f}, r={r['rr']:+.3f}, s={r['sw']}/7  [{time.time()-t0:.0f}s]")
