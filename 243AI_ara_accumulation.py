#!/usr/bin/env python3
"""
Script 243AI — ARA-Scaled Accumulation Envelope

Based on 243AI (midline champion, Solar LOO 44.90).

NEW: ARA-scaled meta-envelope that modulates amplitude on a SLOWER timescale.
Dylan's insight: "we are letting it snap earlier than it needs to — it actually
needs to accumulate for like ARA cycles and then snap back and do the reverse."

Mechanism:
  - Meta-period = Schwabe × φ^(1+ARA) — sits one φ-rung above the ARA-scaled Schwabe
    Solar: ~11 × φ^2.618 ≈ 41.6 yr ≈ 3.8 Schwabe cycles
  - Same sawtooth shape as the existing gate: acc_frac = 1/(1+ARA) ≈ 0.382
  - During accumulation phase: envelope ramps 1.0 → φ  (energy builds)
  - During release phase: envelope ramps φ → 1/φ  (energy drains)
  - Envelope scales deviation from midline: shape = midline + (shape - midline) × envelope

This gives amplitude dynamic range of 1/φ to φ (factor 2.6×), allowing the wave
to reach extreme peaks (C3: 264) and deep troughs (C5-C6: 82) that the flat
midline-only model can't achieve.
...
class EngineMemoryNode(OrigARANode):
    ...
    def __init__(self, name, period, ara, rung, base_amp, archetype,
        ...
        # NEW: ARA-scaled accumulation envelope
        # Meta-period sits one φ-rung above ARA-scaled Schwabe
        # This is the "ARA cycles" timescale Dylan described
        self.meta_period = None  # set when schwabe is known
    ...
    def cascade_shape(self, t):
        ...
        # ── NEW: ARA-scaled accumulation envelope ──
        # Meta-period = Schwabe × φ^(1+ARA) — one rung above ARA-scaled Schwabe
        if self.meta_period is None:
            self.meta_period = self.schwabe * PHI ** (1.0 + self.ara)
        meta_phase = TAU * (t - self.t_ref) / self.meta_period
        meta_cp = (meta_phase % TAU) / TAU

        # Same sawtooth as main gate: accumulate then release
        if meta_cp < acc:
            # Accumulation: envelope ramps 1.0 → φ
            env_ramp = meta_cp / acc
            envelope = 1.0 + (PHI - 1.0) * env_ramp
        else:
            # Release: envelope ramps φ → 1/φ
            rel_ramp = (meta_cp - acc) / (1.0 - acc)
            envelope = PHI * (1.0 - rel_ramp) + INV_PHI * rel_ramp

        # Scale deviation from midline by envelope
        deviation = shape - self.midline
        shape = self.midline + deviation * envelope
...
print("=" * 78)
print("Script 243AI — Ablation: Midline ONLY")
print("=" * 78)
print()
...
print(f"\n{'=' * 78}")
print("SUMMARY — 243AI ARA-Scaled Accumulation Envelope")
print("=" * 78)
print()
