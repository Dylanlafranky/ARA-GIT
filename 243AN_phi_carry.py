#!/usr/bin/env python3
"""
Script 243AN — φ-Distance Carry: Amplitude from Distance to φ

Based on 243AJ (static midline amp, Solar LOO 42.89).

The midline is where the wave centers. φ is where it WANTS to be.
The distance between inst_ara and φ is the energy budget:
  - Positive (inst_ara > φ): overshot φ → excess energy carries forward → boost
  - Negative (inst_ara < φ): still climbing toward φ → deficit → dampen

  carry = (inst_ara - φ) × 1/φ²    (attenuated like a bouncing ball)
  amp_scale = midline + carry

This is what the static midline was masking — it's the φ-distance signal,
but averaged out. Now we let the actual distance speak, attenuated by 1/φ²
so the signal doesn't overwhelm the proven static base.
...
class EngineMemoryNode(OrigARANode):
    ...
    def __init__(self, name, period, ara, rung, base_amp, archetype,
        ...
        # FIX 5: φ-distance carry — dynamic amplitude from distance to φ
        # (computed in cascade_shape using inst_ara, not stored statically)
    ...
    def cascade_shape(self, t):
        ...
        # ── FIX 5: φ-distance carry ──
        # inst_ara > φ → positive carry → amp > midline (boost)
        # inst_ara < φ → negative carry → amp < midline (dampen, trying to reach φ)
        # inst_ara = φ → carry = 0 → amp = midline (same as 243AJ)
        phi_distance = inst_ara - PHI
        carry = phi_distance * INV_PHI * INV_PHI  # 1/φ² attenuation
        amp_scale = self.midline + carry
...
print("=" * 78)
print("Script 243AN — φ-Distance Carry (Amplitude from Distance to φ)")
print("=" * 78)
print()
...
print(f"  {'ARA':>6} │ {'Role':>10} │ {'Midline':>8} │ {'φ-dist':>8} │ {'Amp@ARA':>8} │ {'Amp@hi':>8} │ {'Amp@lo':>8}")
print(f"  {'─'*6}─┼─{'─'*10}─┼─{'─'*8}─┼─{'─'*8}─┼─{'─'*8}─┼─{'─'*8}─┼─{'─'*8}")
for ara, role in [(0.15, "deep cons"), (INV_PHI, "consumer"), (1.0, "clock"),
                  (PHI, "ENGINE"), (2.0, "exotherm")]:
    mid = ara_midline(ara)
    phi_d = ara - PHI
    carry = phi_d * INV_PHI * INV_PHI
    amp_at = mid + carry
    # Show what happens when inst_ara is 50% above/below ARA
    hi_d = min(2.0, ara * 1.5) - PHI
    lo_d = max(0.01, ara * 0.5) - PHI
    amp_hi = mid + hi_d * INV_PHI * INV_PHI
    amp_lo = mid + lo_d * INV_PHI * INV_PHI
    print(f"  {ara:>6.3f} │ {role:>10} │ {mid:>8.4f} │ {phi_d:>+8.4f} │ "
          f"{amp_at:>8.4f} │ {amp_hi:>8.4f} │ {amp_lo:>8.4f}")
print()
...
print(f"\n{'=' * 78}")
print("SUMMARY — 243AN φ-Distance Carry")
print("=" * 78)
print()
