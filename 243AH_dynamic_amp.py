#!/usr/bin/env python3
"""
Script 243AH — Midline + Dynamic Amplitude Pipes

Builds on 243AH (midline champion) by adding amplitude-responsive pipe
capacity. The idea: the pipe between rungs widens after a big cycle and
narrows after a quiet one, so the wave can breathe its amplitude range
dynamically instead of being locked to base_amp.

Mechanism: track a rolling "amplitude regime" — a φ-decaying weighted
average of recent observed/predicted amplitudes. Use the ratio of this
regime to base_amp as a dynamic scaler on the wave shape output.

  ON:
  - Midline reintegration (champion fix)
  - Dynamic amplitude pipe: regime_amp tracks recent amplitude, scales output

  OFF:
  - Memory buffer (using prev_amp only)
  - Grief bleed
  - ARA-dependent momentum (hardcoded 0.3)
"""
...
class EngineMemoryNode(OrigARANode):
    ...
    def __init__(self, name, period, ara, rung, base_amp, archetype,
        ...
        # ── DYNAMIC AMPLITUDE PIPE ──
        # regime_amp tracks the local amplitude regime via φ-decay
        # Starts at base_amp, blends toward recent observations
        self.regime_amp = base_amp if base_amp > 0 else 1.0
    ...
    def cascade_shape(self, t):
        ...
        # ── DYNAMIC AMPLITUDE PIPE ──
        # Update regime_amp toward most recent observation (φ-blend)
        if self.prev_amp is not None and self.base_amp > 0:
            # Blend: new regime = (1/φ) × old_regime + (1 - 1/φ) × prev_amp
            # This gives recent observations weight (1-1/φ) ≈ 0.382
            self.regime_amp = INV_PHI * self.regime_amp + (1 - INV_PHI) * self.prev_amp
        # Scale the deviation from midline by the regime ratio
        if self.base_amp > 0:
            regime_ratio = self.regime_amp / self.base_amp
            # shape is currently: midline + oscillations around midline
            # We want to amplify/dampen the oscillation part
            deviation = shape - self.midline
            shape = self.midline + deviation * regime_ratio
...
print("=" * 78)
print("Script 243AH — Midline + Dynamic Amplitude Pipes")
print("=" * 78)
print()
...
print("  NEW:   Dynamic amplitude pipe (regime_amp φ-blends to recent) ← ON")
print()
...
print(f"\n{'=' * 78}")
print("SUMMARY — 243AH Midline + Dynamic Amplitude Pipes")
print("=" * 78)
print()
