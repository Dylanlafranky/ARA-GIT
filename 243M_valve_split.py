#!/usr/bin/env python3
"""
Script 243M — Palindrome Valve: Dynamic Up/Down Split at Each Rung

Dylan: "what if there is a valve between the build up going up and to
the right and one going down and to the right at each log, and it has
its own dynamic pressure valve that is based on the system's
palindrome style thingie."

The palindrome valve: acc = 1/(1+ARA)
  - High ARA (engine, φ): acc = 0.382 → 38% up, 62% down
  - Clock ARA (1.0):      acc = 0.500 → 50/50 split
  - Low ARA (consumer):   acc = 0.724 → 72% up, 28% down

At each rung, when the engine snaps:
  - down_fraction = ARA/(1+ARA) → feeds child rung below
  - up_fraction = 1/(1+ARA) → feeds parent rung above

The valve position is DYNAMIC — uses instantaneous ARA, not static.
So as the system's ARA shifts cycle-by-cycle, the valve opens and
closes, changing how much energy flows up vs down.

The consumer IS the valve. Energy going up = consumed by parent.
Energy going down = fed to child. The split is the palindrome.

NO new constants. NO decay. The existing acc formula handles everything.
"""
...
class ValveSplitNode(OrigARANode):
    """
    Triangle rider node with PALINDROME VALVE for vertical flow.

    At each rung, the valve acc = 1/(1+inst_ARA) splits energy:
      up_fraction = acc       (consumed by parent above)
      down_fraction = 1 - acc (fed to child below)

    The valve is dynamic — inst_ARA changes cycle-by-cycle.
    The child/parent references come from the temporal gear wiring.
    """
    ...
    def __init__(self, name, period, ara, rung, base_amp, archetype,
        ...
        # Valve diagnostics
        self.valve_history = []  # Track valve positions over time
    ...
    def cascade_shape(self, t):
        ...
        # ── PALINDROME VALVE ──
        # acc = 1/(1+ARA): the valve that splits vertical flow
        valve_acc = 1.0 / (1.0 + max(0.01, inst_ara))
        # up_frac = valve_acc (energy going UP to parent = consumed)
        # down_frac = 1 - valve_acc (energy going DOWN to child = engine output)

        # Apply valve to temporal gear:
        # Child's last snap × down_frac tells us how much the child
        # fed downward (away from us) — that's energy we LOST
        # Parent's last snap × up_frac tells us how much the parent
        # pushed up (away from us) — that's energy we GAINED from below
        gear_tilt = 0.0
        ...
        if self.child_last_snap_amp is not None and self.child_engine is not None:
            ...
            # Child's UP fraction feeds into us (we're the parent)
            child_valve = 1.0 / (1.0 + max(0.01, self.child_engine.ara))
            gear_tilt += child_valve * INV_PHI_4 * child_dev
        ...
        if self.parent_last_snap_amp is not None and self.parent_engine is not None:
            ...
            # Parent's DOWN fraction feeds into us (we're the child)
            parent_valve = 1.0 - 1.0 / (1.0 + max(0.01, self.parent_engine.ara))
            gear_tilt += parent_valve * INV_PHI_4 * parent_dev

        # OUR OWN valve: how much of our energy goes up (consumed) vs down (engine)
        # When valve_acc is high (consumer), more goes UP = we lose more
        # This creates a natural drain proportional to how consumer-like we are
        # The drain is the DIFFERENCE between up and down fractions
        # At ARA=1 (clock): up=down=0.5, no net drain
        # At ARA=φ (engine): up=0.382, down=0.618, net drain = -0.236 (net engine)
        # At ARA=0.382 (consumer): up=0.724, down=0.276, net drain = +0.448 (net consumer)
        valve_drain = valve_acc - (1 - valve_acc)  # positive = net consumer
        gear_tilt -= valve_drain * INV_PHI_4  # Consumer drain reduces ARA further

        inst_ara = max(0.01, min(2.0, inst_ara + gear_tilt))

        self.valve_history.append(valve_acc)
        ...
        # ── VALVE-MODULATED SHAPE ──
        # The valve split also affects the CASCADE SHAPE itself.
        # When our valve sends more UP (consumer), our own shape is dampened.
        # When our valve sends more DOWN (engine), our shape is amplified.
        # This is the "pressure" Dylan described — the valve creates
        # back-pressure that modulates the cascade output.
        #
        # net_pressure = down_frac - up_frac = (1-acc) - acc = 1 - 2*acc
        # At ARA=φ: net_pressure = 1 - 2*0.382 = +0.236 (slight amplification)
        # At ARA=1: net_pressure = 0 (neutral)
        # At ARA=0.382: net_pressure = 1 - 2*0.724 = -0.448 (dampening)
        net_pressure = 1.0 - 2.0 * valve_acc
        shape *= (1 + net_pressure * INV_PHI_4)
...
def grid_search(period, ara, rung, times, peaks, n_tr=80, n_ba=40):
    ...
    for tr in t_refs:
        for ba in base_amps:
            node = ValveSplitNode("temp", period, ara, rung, ba, "engine")
            node.set_t_ref(tr)
            errors = []
...
def run_valve_split(times, peaks, period, ara, target_rung,
    ...
    for rung_power, rung_period, prefix, eng_ara, cons_ara, offset in rungs:
        for archetype, ara_val in [("engine", eng_ara), ("clock", 1.0),
            ...
            node = ValveSplitNode(name, rung_period, ara_val,
    ...
    # Valve diagnostics
    target_valves = target_node.valve_history
    avg_valve = np.mean(target_valves) if target_valves else 0
...
print("=" * 78)
print("Script 243M — Palindrome Valve: Dynamic Up/Down Split at Each Rung")
print("=" * 78)
print()
print("  acc = 1/(1+ARA): the palindrome valve")
print("  High ARA (φ):   38% up, 62% down → mostly engine")
print("  Clock (1.0):    50% up, 50% down → balanced")
print("  Low ARA (0.38): 72% up, 28% down → mostly consumer")
print()
print("  The valve IS the consumer. No new constants.")
print("  Vertical flow splits dynamically based on instantaneous ARA.")
print()
...
print(f"  {'System':<18} │ {'235b':>7} │ {'Best243':>7} │ {'243M':>7} │ {'Δ 235b':>8} │ {'Valve':>5} │ Notes")
print(f"  {'─'*18}┼{'─'*9}┼{'─'*9}┼{'─'*9}┼{'─'*10}┼{'─'*7}┼{'─'*25}")

for label, t, p, period, ara, rung in [
    ("Solar (SSN)", solar_t, solar_p, 11.0, PHI, 4),
    ("ENSO (ONI)",  enso_t,  enso_p,  3.7, INV_PHI_2, 2),
    ("Sanriku EQ",  eq_t,    eq_p,   25.0, 0.5, 3),
]:
    print(f"  Running {label}...", end="", flush=True)
    r = run_valve_split(t, p, period, ara, rung, label)

    base_ref = base_solar if "Solar" in label else base_enso if "ENSO" in label else base_eq
    best_ref = best_243["Solar"] if "Solar" in label else best_243["ENSO"] if "ENSO" in label else best_243["EQ"]
    d235 = r["mae"] - base_ref["mae"]

    marks = []
    if r["mae"] < base_ref["mae"]: marks.append("beats 235b ✓")
    if r["mae"] < best_ref: marks.append("NEW BEST!")

    print(f"\r  {label:<18} │ {base_ref['mae']:>7.2f} │ {best_ref:>7.2f} │ {r['mae']:>7.2f} │ {d235:>+8.2f} │ {r['avg_valve']:>5.3f} │ {', '.join(marks)}")
