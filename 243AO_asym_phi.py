#!/usr/bin/env python3
"""
Script 243AO — Asymmetric φ-Distance: Stronger Bounce Up, Weaker Pull Down

Based on 243AN (symmetric φ-distance, Solar LOO 45.45 — too harsh downward).

When the previous cycle overshot φ (inst_ara > φ), the excess energy
bounces forward freely → 1/φ² carry weight (strong).
When it undershot (inst_ara < φ), the system is TRYING to reach φ but
can't sustain it → 1/φ⁴ carry weight (weak, resists pull-down).

  if phi_distance > 0:  carry = phi_distance × 1/φ²  (bounce forward)
  if phi_distance ≤ 0:  carry = phi_distance × 1/φ⁴  (gentle resistance)
...
class EngineMemoryNode(OrigARANode):
    ...
    def cascade_shape(self, t):
        ...
        # ── FIX 5: Asymmetric φ-distance carry ──
        # Overshot φ → strong bounce forward (1/φ²)
        # Undershot φ → weak resistance (1/φ⁴) — system resists being pulled from φ
        phi_distance = inst_ara - PHI
        if phi_distance > 0:
            carry = phi_distance * INV_PHI * INV_PHI          # 1/φ² = 0.382
        else:
            carry = phi_distance * INV_PHI_4                   # 1/φ⁴ = 0.146
...
print("=" * 78)
print("Script 243AO — Asymmetric φ-Distance (Bounce Up Strong, Pull Down Weak)")
print("=" * 78)
print()
...
for ara, role in [(0.15, "deep cons"), (INV_PHI, "consumer"), (1.0, "clock"),
    ...
    phi_d = ara - PHI
    # Asymmetric carry
    carry = phi_d * (INV_PHI * INV_PHI if phi_d > 0 else INV_PHI_4)
    amp_at = mid + carry
    hi_d = min(2.0, ara * 1.5) - PHI
    lo_d = max(0.01, ara * 0.5) - PHI
    amp_hi = mid + hi_d * (INV_PHI * INV_PHI if hi_d > 0 else INV_PHI_4)
    amp_lo = mid + lo_d * (INV_PHI * INV_PHI if lo_d > 0 else INV_PHI_4)
...
print(f"\n{'=' * 78}")
print("SUMMARY — 243AO Asymmetric φ-Distance Carry")
print("=" * 78)
print()
