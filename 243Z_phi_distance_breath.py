#!/usr/bin/env python3
"""
Script 243Z — φ-Distance Asymmetric Breathing

Dylan: "Can we modify it by their ARA distance from phi or something?"

The exhale decay rate is now DYNAMIC — computed each step from
|inst_ara - φ|. The closer you are to φ, the longer your exhale rings.

  At φ (engine sweet spot): decay ≈ 0.618 → long sustained exhale
  Far from φ (deep consumer): decay ≈ 0.22 → quick snap back
  At clock (1.0): decay ≈ 0.44 → moderate ring

This means engines naturally get the long exhale they need, while
consumers get the short exhale that keeps them responsive. And it's
all driven by the SAME formula — distance from φ.

Since inst_ara is dynamic (the node breathes), the decay rate changes
each step. As a node breathes toward φ, its exhale lengthens.
As it breathes away, its exhale shortens.

Three decay curves (lean run):
  A — φ-exponential: decay = (1/φ)^(1 + dist)
  B — Hyperbolic:    decay = (1/φ) / (1 + dist)
  C — Exp-falloff:   decay = (1/φ) × e^(-dist)

Where dist = |inst_ara - φ|

Based on 243Y-B (golden exhale) which got Solar 49.83 — closest to
protecting the engine in the entire breathing series.
"""
...
# ── Decay curves ──
def decay_phi_exp(inst_ara):
    """φ-exponential: (1/φ)^(1 + |ARA - φ|). At φ: 0.618. At 0.15: 0.22."""
    dist = abs(inst_ara - PHI)
    return INV_PHI ** (1.0 + dist)

def decay_hyperbolic(inst_ara):
    """Hyperbolic: (1/φ) / (1 + |ARA - φ|). At φ: 0.618. At 0.15: 0.25."""
    dist = abs(inst_ara - PHI)
    return INV_PHI / (1.0 + dist)

def decay_exp_falloff(inst_ara):
    """Exp-falloff: (1/φ) × e^(-|ARA-φ|). At φ: 0.618. At 0.15: 0.14."""
    dist = abs(inst_ara - PHI)
    return INV_PHI * math.exp(-dist)
...
class PhiDistBreathNode(OrigARANode):
    """
    φ-distance asymmetric breathing node.

    INHALE: tilt toward home ARA → sharp, direct, one-step.
    EXHALE: tilt away from home → fired as momentum projectile.
            Decay rate = f(|inst_ara - φ|) — DYNAMIC each step.

    At φ: longest exhale (decay ≈ 0.618).
    Far from φ: shortest exhale (decay → 0).
    As the node breathes, its decay rate breathes with it.
    """

    def __init__(self, name, period, ara, rung, base_amp, archetype,
                 seed_ara=None, rung_offset=0,
                 child_sign=-1, parent_sign=-1,
                 gate_send=True, decay_func=decay_phi_exp):
    ...
    def cascade_shape(self, t):
        ...
        # ── DYNAMIC EXHALE DECAY from φ-distance ──
...
def grid_search(period, ara, rung, times, peaks, n_tr=80, n_ba=40,
    ...
    for tr in t_refs:
        for ba in base_amps:
            node = PhiDistBreathNode("temp", period, ara, rung, ba, "engine",
...
def run_phi_dist_breath(times, peaks, period, ara, target_rung,
    ...
    for rung_power, rung_period, prefix, eng_ara, cons_ara, offset in rungs:
        for archetype, ara_val in [("engine", eng_ara), ("clock", 1.0),
            ...
            node = PhiDistBreathNode(name, rung_period, ara_val,
...
VARIANTS = {
    "A": {
        "label": "φ-exponential: (1/φ)^(1+dist)",
        "decay_func": decay_phi_exp,
    },
    "B": {
        "label": "Hyperbolic: (1/φ)/(1+dist)",
        "decay_func": decay_hyperbolic,
    },
    "C": {
        "label": "Exp-falloff: (1/φ)×e^(-dist)",
        "decay_func": decay_exp_falloff,
    },
}
...
print("=" * 78)
print("Script 243Z — φ-Distance Asymmetric Breathing")
print("=" * 78)
print()
print("  Exhale decay = f(|inst_ara - φ|) — DYNAMIC each step.")
print("  At φ: decay ≈ 0.618 (long exhale). Far from φ: decay → 0 (quick snap).")
print("  The engine breathes long. The consumer snaps short.")
print()

# Show decay values at key ARAs
print("  Decay rates at key ARA positions:")
print(f"  {'ARA':>6} │ {'dist(φ)':>7} │ {'Role':>10} │ {'A (φ-exp)':>9} │ {'B (hyper)':>9} │ {'C (e-fall)':>9}")
print(f"  {'─'*6}─┼─{'─'*7}─┼─{'─'*10}─┼─{'─'*9}─┼─{'─'*9}─┼─{'─'*9}")
for ara, role in [(0.15, "deep cons"), (INV_PHI, "consumer"), (1.0, "clock"),
                  (PHI, "engine"), (2.0, "exotherm")]:
    d = abs(ara - PHI)
    print(f"  {ara:>6.3f} │ {d:>7.3f} │ {role:>10} │ "
          f"{decay_phi_exp(ara):>9.4f} │ {decay_hyperbolic(ara):>9.4f} │ "
          f"{decay_exp_falloff(ara):>9.4f}")
print()
...
for var_key in ["A", "B", "C"]:
    ...
    print(f"\n  {'System':<17} │ {'235b':>7} │ {'O-C':>7} │ {'Y-B':>7} │ "
          f"{'243Z':>7} │ {'Δ vs 235b':>9} │ {'Δ vs O-C':>9} │ {'avg_dec':>7}")
    print(f"  {'─'*17}─┼─{'─'*7}─┼─{'─'*7}─┼─{'─'*7}─┼─"
          f"{'─'*7}─┼─{'─'*9}─┼─{'─'*9}─┼─{'─'*7}")
...
print(f"\n{'=' * 78}")
print("SUMMARY — 243Z φ-Distance Asymmetric Breathing")
print("=" * 78)
print()
print("  Exhale decay = f(|inst_ara - φ|)")
print("  At φ: long exhale (decay=0.618). Far from φ: quick snap.")
print("  The node's distance from φ dictates how long it gets to breathe out.")
print()
print("  243Y-B (fixed 1/φ decay) got Solar 49.83 — best in breathing series.")
print("  Does dynamic φ-distance improve further?")
print()
