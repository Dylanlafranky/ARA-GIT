#!/usr/bin/env python3
"""
Script 243O — Breathing Gear: Valve-Gated Temporal Coupling

Dylan: "What if personality breathes? When the Gleissberg is transitioning,
the Schwabe has to absorb that shock — it temporarily takes on consumer-like
behavior. The valve gates the gear reception, and that naturally makes rungs
breathe between engine and consumer roles."

The insight: acc = 1/(1+ARA) already exists in the formula.
Instead of gating geometric ARAs (243N, hurt Solar), we gate the
TEMPORAL GEAR TILT signal itself.

  - Engine (high ARA, low acc) → resists gear tilt → self-sustaining
  - Consumer (low ARA, high acc) → absorbs gear tilt → receptive
  - During transitions, gear tilt pushes ARA down → acc rises →
    rung becomes MORE receptive → breathes into consumer role
  - Same formula everywhere. No special cases. Mirror of itself.

Three variants (lean run):
  A — Valve-gated reception only (pure breathing, original 243j signs)
  B — Valve-gated reception + consumer sign convention (child_sign=-1)
  C — Full mirror: send ∝ (1-acc), receive ∝ acc (you give what you have,
...
class BreathingGearNode(OrigARANode):
    ...
    def __init__(self, name, period, ara, rung, base_amp, archetype,
        ...
        self.gate_send = gate_send  # If True, sender's (1-acc) also gates
...
VARIANTS = {
    "A": {
        "label": "Valve-gated reception (original signs)",
        "child_sign": 1, "parent_sign": -1, "gate_send": False,
    },
    "B": {
        "label": "Valve-gated reception + consumer signs",
        "child_sign": -1, "parent_sign": -1, "gate_send": False,
    },
    "C": {
        "label": "Full mirror: send∝(1-acc), receive∝acc",
...
print("=" * 78)
print("Script 243O — Breathing Gear: Valve-Gated Temporal Coupling")
print("=" * 78)
print()
print("The palindrome valve acc = 1/(1+ARA) gates gear reception.")
print("Engines resist tilt. Consumers absorb it. Rungs BREATHE.")
print()

ref = {
    '235b':   {"Solar (SSN)": 46.30, "ENSO (ONI)": 0.44, "Sanriku EQ": 1.19},
    '243j':   {"Solar (SSN)": 52.13, "ENSO (ONI)": 0.45, "Sanriku EQ": 0.46},
}
...
for var_key in ["A", "B", "C"]:
    ...
    print(f"{'─' * 78}")
    print(f"\n  {'System':<16} │ {'235b':>7} │ {'243j':>7} │ {'243O':>7} │ "
          f"{'Δ vs 235b':>9} │ {'Δ vs 243j':>9} │ {'Valve':>6}")
    ...
    for name, times, peaks, period, ara, rung in SYSTEMS:
        ...
        d243j = res['mae'] - ref['243j'][name]
        b235 = " ✓" if d235 < 0 else ""
        b243j = " ✓" if d243j < 0 else ""
        print(f"\r  {name:<16} │ {ref['235b'][name]:>7.2f} │ {ref['243j'][name]:>7.2f} │ "
              f"{res['mae']:>7.2f} │ {d235:>+8.2f}{b235} │ {d243j:>+8.2f}{b243j} │ "
              f"{res['avg_valve']:>.4f}")
...
print(f"\n{'=' * 78}")
print("SUMMARY — 243O Breathing Gear")
print("=" * 78)
print()
print("  Valve acc = 1/(1+ARA) gates temporal gear reception.")
print("  Same formula everywhere. No special cases.")
print("  Engines resist tilt → stable during transitions.")
print("  Consumers absorb tilt → breathe with the system.")
print()
print(f"  Key question: Does valve-gating PROTECT Solar while")
print(f"  letting ENSO/EQ benefit from gear coupling?")
print(f"  (243N's camshaft-gated geometric ARAs couldn't do this)")
print()
