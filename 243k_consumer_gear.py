#!/usr/bin/env python3
"""
Script 243k — Temporal Gear + System-Level Consumer

Dylan: "I honestly feel it is probably accurate for the sun, but we're
just missing something for the sun. I feel like it is losing a fair bit
of energy each cycle... Maybe the Sun is missing a consumer?"

The ARA framework says every system is a triad: engine, clock, consumer.
We've been modeling the Sun's internal cascade (SubSub → Gleissberg),
but the SYSTEM AS A WHOLE has been treated as a pure engine (ARA = φ).
Nothing completes the Sun's triad at the system level.

FIX: Add a "Heliospheric Consumer" rung ABOVE the Gleissberg.
  - Period = Gleissberg × φ (one more φ-step up)
  - ARA = 2 - φ ≈ 0.382 (consumer mirror of the engine)
  - Drains from Gleissberg engine through existing pipe mechanism
  - In the temporal gear, Gleissberg's parent is this consumer
  - The consumer's snaps feed back DOWN via temporal gear
  - When the consumer accumulates too much, it snaps (Grand Minimum reset)
  - When it's depleted, the Sun runs hot

Fully dynamic. No hardcoded decay. The formula IS the mechanism.
Everything from 243j (temporal gear) is preserved.
"""
...
class ConsumerGearNode(OrigARANode):
    """
    Triangle rider node with TEMPORAL gear coupling + system-level consumer.

    Same as 243j's TemporalGearNode, but the rung ladder now includes
    a consumer rung above Gleissberg. The temporal gear handles the
    feedback naturally — energy flows up, the consumer accumulates,
    and when it snaps, it feeds back down through the gear.
    """
...
# ════════════════════════════════════════════════════════════════
# BUILD RUNG LADDER WITH SYSTEM-LEVEL CONSUMER
# ════════════════════════════════════════════════════════════════

def build_rung_ladder_consumer(target_rung, target_period, seed_ara):
    """
    Same as 243j's rung ladder, but extended one rung ABOVE Gleissberg:
    the "Helio" (heliospheric consumer) rung.

    This is the system-level consumer that completes the Sun's triad.
    Its ARA is the mirror of the seed (2 - seed_ara), making it a
    natural consumer counterpart to the engine.
    """
    mirror_floor = 0.15
    atom = derive_atom_nodes(seed_ara, target_period)
    rungs = []

    # Extended range: -2 to 5 (existing) PLUS offset=5 for Helio consumer
    for offset in range(-2, 6):
        r = target_rung + offset
        if r < 0: continue
        p = target_period * PHI**offset

        if offset == 5:   prefix = "Helio"      # NEW: system-level consumer
        elif offset == 4: prefix = "Gleissberg"
        ...
        # For the Helio rung: the ENGINE is a consumer-type ARA
        # This is the key insight — the top rung IS the system consumer
        if offset == 5:
            engine_ara = max(mirror_floor, 2.0 - seed_ara)  # Mirror of seed
            consumer_ara = seed_ara  # Its own consumer mirrors back to engine
...
def grid_search_consumer(period, ara, rung, times, peaks, n_tr=80, n_ba=40):
    ...
    for tr in t_refs:
        for ba in base_amps:
            node = ConsumerGearNode("temp", period, ara, rung, ba, "engine")
            node.set_t_ref(tr)
            errors = []
...
def run_consumer_gear(times, peaks, period, ara, target_rung,
                      system_name="System", sim_margin=None):
    if sim_margin is None:
        sim_margin = max(period * 5, times[-1] - times[0]) * 0.2

    fit_tr, fit_ba, fit_mae = grid_search_consumer(
        period, ara, target_rung, times, peaks)
    rungs = build_rung_ladder_consumer(target_rung, period, ara)
    ...
    for rung_power, rung_period, prefix, eng_ara, cons_ara, offset in rungs:
        for archetype, ara_val in [("engine", eng_ara), ("clock", 1.0),
            ...
            if prefix == "Target" and archetype == "engine":
                ...
                ba = 1.0
            node = ConsumerGearNode(name, rung_period, ara_val,
    ...
    # Same as 243j: each engine connects to child/parent rung engines.
    # NOW the Gleissberg's parent is the Helio consumer engine —
    # this is the system-level consumer completing the triad.
    for i, (rp, prefix, offset) in enumerate(rung_list):
        engine = engine_nodes[prefix]

        # Child = rung below (longer period, higher rung index in sorted list)
        if i < len(rung_list) - 1:
            child_prefix = rung_list[i + 1][1]
            engine.child_engine = engine_nodes[child_prefix]

        # Parent = rung above (shorter period, lower rung index in sorted list)
        if i > 0:
...
print("=" * 78)
print("Script 243k — Temporal Gear + System-Level Consumer")
print("=" * 78)
print()
print("  Everything from 243j (temporal gear) PLUS:")
print("  NEW: 'Helio' rung above Gleissberg — the system-level consumer")
print("    → ARA = 2 - seed_ara (mirror of engine)")
print("    → Period = Gleissberg × φ (one more φ-step up)")
print("    → Drains from top of cascade, snaps feed back down")
print("    → Completes the system's engine-clock-consumer triad")
print("    → Fully dynamic: no hardcoded decay")
print()
...
# Reference: best from 243 series
best_solar = 43.48   # 243f inverted gear
best_enso  = 0.43    # 243b full atom
best_eq    = 0.42    # 243g pure gear asymmetric

print(f"  {'System':<18}│ {'235b':>7} │ {'Best243':>7} │ {'243k':>7} │ {'Δ vs 235b':>9} │ {'Δ vs Best':>9}")
print(f"  {'─'*18}┼{'─'*9}┼{'─'*9}┼{'─'*9}┼{'─'*11}┼{'─'*10}")

results = {}
for label, t, p, period, ara, rung, base_mae in [
    ("Solar (SSN)", solar_t, solar_p, 11.0, PHI, 4, base_solar["mae"]),
    ("ENSO (ONI)",  enso_t,  enso_p,  3.7, INV_PHI_2, 2, base_enso["mae"]),
    ("Sanriku EQ",  eq_t,    eq_p,   25.0, 0.5, 3, base_eq["mae"]),
]:
    print(f"  Running {label}...", end="", flush=True)
    r = run_consumer_gear(t, p, period, ara, rung, label)
    results[label] = r

    best_ref = best_solar if "Solar" in label else best_enso if "ENSO" in label else best_eq
    d235 = r["mae"] - base_mae
    dbest = r["mae"] - best_ref
    mark235 = " ✓" if d235 < -0.5 else ""
    markbest = " ✓" if dbest < -0.01 else ""

    print(f"\r  {label:<18}│ {base_mae:>7.2f} │ {best_ref:>7.2f} │ {r['mae']:>7.2f} │ {d235:>+9.2f}{mark235} │ {dbest:>+8.2f}{markbest}")

# ── Rung snap diagnostics ──
print()
print("─" * 78)
print("RUNG SNAP COUNTS (including Helio consumer rung)")
print("─" * 78)
...
for label in results:
    ...
    if "rung_snaps" in r:
        print(f"\n  {label}:")
        max_snaps = max(r["rung_snaps"].values()) if r["rung_snaps"] else 1
        for prefix in ["Helio", "Gleissberg", "AboveB", "AboveA", "AboveT",
                        "Target", "BelowT", "SubSub"]:
            if prefix in r["rung_snaps"]:
                n = r["rung_snaps"][prefix]
                bar = "█" * max(1, int(50 * n / max_snaps))
                marker = " ← SYSTEM CONSUMER" if prefix == "Helio" else ""
                print(f"    {prefix:<12} {n:>4} snaps  {bar}{marker}")

# ── Solar per-cycle ──
print()
print("─" * 78)
print("SOLAR PER-CYCLE — 243k Consumer Gear")
print("─" * 78)
r = results["Solar (SSN)"]
if not r.get("failed"):
    print(f"\n    {'C':>4}  {'Year':>6}  {'Act':>5} │ {'Pred':>6}  {'Err':>5} │")
    for i, (cn, yr, act) in enumerate(SOLAR_CYCLES):
        pred = r["preds"][i]
        err = r["errors"][i]
        print(f"  C{cn:>2}  {yr:>6.1f}  {act:>5.1f} │ {pred:>6.1f}  {err:>5.1f} │")

# ── Summary ──
print()
print("=" * 78)
print("SUMMARY — 243k Consumer Gear")
print("=" * 78)
print()
print("  Temporal gear from 243j + Helio consumer rung above Gleissberg.")
print("  The system's engine-clock-consumer triad is now complete.")
print(f"  vs Champion (28.11): 243k Solar = {results['Solar (SSN)']['mae']:.2f} (Δ={results['Solar (SSN)']['mae']-28.11:+.2f})")
print()
