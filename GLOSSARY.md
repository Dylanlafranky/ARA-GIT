# Glossary — ARA Framework Terms

A plain-language key to the vocabulary used across this repository. Where a term is easy to misread, the confusion is flagged directly. New readers: start here, then `what_is_this.html`.
...
- **The scale (0–2)** — a system's ARA position. **It measures asymmetry around 1.0.** See "Pole orientation" below for the crucial caveat about which end is which.
- **Gradient rule** — every ARA axis is a continuous relational gradient. Poles, quadrants, walls, \(\phi\), the
  \(1.0\) ridge, rungs and named regimes are landmarks, orientations or scale boundaries on that gradient—not
...
- **ARA singularity / singularity crossing** — the relational handover where a declared signed phase coordinate
  ...
  density, black-hole singularity or field-topology singularity. Its numerical location depends on the declared
...
## Disambiguations & cautions (read these)

- **"Primes"** — in this framework, **irreducible system parameters**, NOT prime numbers. They just fit a similar role as Prime numbers but for system cycles.
...
*See `CLAIMS_STATUS.md` for which claims are safe to quote, `INDEX.md` for findings by confidence, and `what_is_this.html` for the plain-English overview.*
