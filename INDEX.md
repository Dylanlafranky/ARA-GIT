...
The current mapping-first atlas is recorded in [`ARA_MAPPING_ATLAS_RESULT.md`](ARA_MAPPING_ATLAS_RESULT.md). It rebuilds the old temporal-coordinate visualiser as a reusable 3D diagnostic workbench with 206 nodes across the original catalogue, measured fitted rungs, and current state-geometry rungs. This is not a predictor; it is the place to map more systems and diagnose rings, boundaries, and relation classes.
...
| **Mapping-first atlas replaces prediction pressure with geometry diagnosis** | 2026-05-24 atlas build: **206 nodes**, **235 relations**, **6 triangle candidates**, **40 systems/subsystems**, spanning **157.7 phi-rungs**. It merges old catalogue nodes with measured fitted rungs and current state-geometry rungs. | `ARA_MAPPING_ATLAS_RESULT.md`, `Mapping/ara_mapping_atlas_3d.html`, `Mapping/ara_mapping_atlas_data.js` |
