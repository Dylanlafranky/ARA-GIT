#!/usr/bin/env python3
"""
Script 243AS — Reverse Gate: Scales Breathe in Opposition

Based on 243AJ (midline champion, Solar LOO 42.89).

KEY INSIGHT: In Dylan's meta triangle, the ARA flows between rungs (cascade
periods). Currently all rungs share the same sawtooth gate — they all
accumulate and release together. But scales should BREATHE IN OPPOSITION:
when short periods accumulate, long periods release, and vice versa.

  Short periods (d <= 0): normal gate      -> accumulate with Schwabe
  Long periods  (d > 0):  inverted gate    -> release when Schwabe accumulates

  inv_gate = 2.0 - gate   (mirror around 1.0)

This creates natural energy exchange between rungs — the exhale of the
Gleissberg feeds the inhale of the Schwabe — without adding any fitted
parameters. Purely structural.
...
class EngineMemoryNode(OrigARANode):
    ...
    def cascade_shape(self, t):
        ...
        # ── UNIFIED SINGULARITY VALVE ──
        # All coupling follows the same log-singularity law:
        # approaching 0 or 2 costs more energy → valve tightens
        # The double-log decay already captures this for tilt_momentum;
        # now use it for eps_vals too, replacing gate as primary modulator.
        singularity_valve = decay_double_log(inst_ara)  # 0.147 @ 0.15, 0.618 @ φ, 0.742 @ 2.0
        ...
        for j in range(len(phases)):
            ...
            # Valve replaces gate: coupling strength = f(inst_ara), not f(phase)
            eps_vals.append(base_eps * singularity_valve)
        for j in range(len(phases)):
            d = live_d[j]
...
print("=" * 78)
print("Script 243AS — Reverse Gate: Scales Breathe in Opposition")
print("=" * 78)
print()
print("  v2: UNIFIED SINGULARITY VALVE")
print("      eps_vals modulated by decay_double_log(inst_ara)")
print("      NOT by sawtooth gate phase — avoids double-counting")
print("      Approaching singularity (0 or 2) tightens all coupling")
print()
...
print(f"\n{'=' * 78}")
print("SUMMARY — 243AS Reverse Gate (Scales Breathe in Opposition)")
print("=" * 78)
print()
