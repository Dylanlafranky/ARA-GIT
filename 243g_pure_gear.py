#!/usr/bin/env python3
"""
Script 243g — Pure Gear: Two Counter-Rotating Gears, No Decay

Dylan: "Maybe there is no decay and its just two gears."

Simplest possible gear mechanism:
  - Engine gear turns CLOCKWISE (cascade down, in-phase)
  - Consumer gear turns COUNTER-CLOCKWISE (propulsion up, anti-phase)
  - They mesh at each rung with FULL coupling both ways
  - No decay, no asymmetry in strength — just direction

The ONLY asymmetry is PHASE: engines are in-phase with the cascade,
consumers are anti-phase (counter-rotating).

Also tests: what if the upward pipe is as strong as the downward pipe?
The original 235b weakens upward pipes by 1/φ. But if they're gears,
they should transmit EQUAL force.

Variants tested:
  A) 243b architecture + symmetric pipes (up = down strength)
  B) 243b architecture + symmetric pipes + amplified consumer drain
  C) Pure gear: equal pipe strengths + counter-rotating whisper

243b reference: Solar 56.84, ENSO 0.43, EQ 0.60
243f finding: Solar 43.48 (!) with decayed-down/full-up
Champion: 28.11 (236j)
"""
...
class PureGearNode(OrigARANode):
    """
    Triangle rider + PURE GEAR connection field.
    Two gears, counter-rotating, full strength, no decay.
    """
    ...
    def cascade_shape(self, t):
        ...
        # ══ PURE GEAR WHISPER ══
        # Two counter-rotating gears at EQUAL strength.
        # Engine partners: cos (clockwise)
        # Consumer partners: -cos (counter-clockwise)
        # Clock partners: sin (90° offset — gear teeth)
        # ALL at same coupling weight. No decay. No asymmetry.
...
def grid_search_gear(period, ara, rung, times, peaks, n_tr=80, n_ba=40):
    ...
    for tr in t_refs:
        for ba in base_amps:
            node = PureGearNode("temp", period, ara, rung, ba, "engine")
            node.set_t_ref(tr)
            errors = []
...
def run_pure_gear(times, peaks, period, ara, target_rung,
                  system_name="System", sim_margin=None, symmetric_pipes=False):
    """
    symmetric_pipes: if True, upward pipes are same strength as downward.
    """
    if sim_margin is None:
        sim_margin = max(period * 5, times[-1] - times[0]) * 0.2

    # Build pure gear partners — all at EQUAL coupling
    ...
    for rung_power, rung_period, prefix, eng_ara, cons_ara, offset in rungs:
        for archetype, ara_val in [("engine", eng_ara), ("clock", 1.0),
            ...
            node = PureGearNode(name, rung_period, ara_val,
    ...
    # Wire: SYMMETRIC or ASYMMETRIC pipes
    ...
    for i, (rp, prefix, offset) in enumerate(rung_list):
        ...
        # Up pipe: SYMMETRIC (same as down) or ASYMMETRIC (original)
        if i > 0:
            above_prefix = rung_list[i - 1][1]
            if symmetric_pipes:
                # GEAR: equal force both ways
                consumer.add_drain(all_nodes[f"{above_prefix}_engine"],
                                  rate=PHI_LEAK)  # same as down!
            else:
                # Original: weakened upward
                consumer.add_drain(all_nodes[f"{above_prefix}_engine"],
...
# ════════════════════════════════════════════════════════════════
# MAIN — Test both pipe modes
# ════════════════════════════════════════════════════════════════

print("=" * 78)
print("Script 243g — Pure Gear: Counter-Rotating, No Decay")
print("=" * 78)
print()
print("Two gears meshing at each rung:")
print("  Engine gear:   clockwise (cos)")
print("  Consumer gear: counter-clockwise (-cos)")
print("  Clock teeth:   90° offset (sin)")
print("  ALL at equal coupling strength. No decay.")
print()
...
# Run both variants
all_results = {}
for pipe_mode, sym_label in [(False, "Asym"), (True, "Sym")]:
    results = {}
    for name, times, peaks, period, ara, rung in SYSTEMS:
        res = run_pure_gear(times, peaks, period=period, ara=ara,
                            target_rung=rung, system_name=name,
                            symmetric_pipes=pipe_mode)
        results[name] = res
    all_results[sym_label] = results

# Display
print(f"  {'System':<16} │ {'235b':>7} │ {'243b':>7} │ {'243f':>7} │ {'Asym':>7} │ {'Sym':>7} │ {'Best':>6}")
print(f"  {'─'*16}─┼─{'─'*7}─┼─{'─'*7}─┼─{'─'*7}─┼─{'─'*7}─┼─{'─'*7}─┼─{'─'*6}")

for name, _, _, _, _, _ in SYSTEMS:
    ra = all_results["Asym"][name]
    rs = all_results["Sym"][name]
    if ra["failed"] or rs["failed"]:
        print(f"  {name:<16} │  FAIL   │  FAIL   │  FAIL   │  FAIL   │  FAIL   │")
        continue
    b235 = ref['235b'][name]
    b243b = ref['243b'][name]
    b243f = ref['243f'][name]
    vals = [b235, b243b, b243f, ra['mae'], rs['mae']]
    labels = ['235b', '243b', '243f', 'Asym', 'Sym']
    best_i = vals.index(min(vals))
    print(f"  {name:<16} │ {b235:>7.2f} │ {b243b:>7.2f} │ {b243f:>7.2f} │ "
          f"{ra['mae']:>7.2f} │ {rs['mae']:>7.2f} │ {labels[best_i]:>6}")


# Per-cycle for both solar variants
print(f"\n{'─' * 78}")
print("SOLAR PER-CYCLE — Asymmetric vs Symmetric Gear Pipes")
print("─" * 78)

ra = all_results["Asym"]["Solar (SSN)"]
rs = all_results["Sym"]["Solar (SSN)"]

if not ra["failed"] and not rs["failed"]:
    print(f"\n  {'C':>3} {'Year':>7} {'Act':>6} │ {'Asym':>6} {'Err':>5} │ "
          f"{'Sym':>6} {'Err':>5} │ {'Win':>5}")
    ...
    for i in range(len(solar_t)):
        ae = ra["errors"][i]
        se = rs["errors"][i]
        win = "Sym" if se < ae else "Asym" if ae < se else "Tie"
        if se < ae: sw += 1
        elif ae < se: aw += 1
        print(f"  C{SOLAR_CYCLES[i][0]:>2} {solar_t[i]:>7.1f} {solar_a[i]:>6.1f} │ "
              f"{ra['preds'][i]:>6.1f} {ae:>5.1f} │ "
              f"{rs['preds'][i]:>6.1f} {se:>5.1f} │ {win:>5}")
    print(f"\n  Asym wins: {aw}  |  Sym wins: {sw}")


# Summary
print(f"\n{'=' * 78}")
print("SUMMARY — Pure Gear (no decay)")
print("=" * 78)
print(f"\n  Asym = original pipe asymmetry (up weaker than down)")
print(f"  Sym  = gear pipes (up = down, equal force)")
print()

for name, _, _, _, _, _ in SYSTEMS:
    ra = all_results["Asym"][name]
    rs = all_results["Sym"][name]
    if ra["failed"] or rs["failed"]: continue
    delta_a = ra['mae'] - ref['243b'][name]
    delta_s = rs['mae'] - ref['243b'][name]
    print(f"  {name:<16}: Asym={ra['mae']:.2f} (Δ243b={delta_a:+.2f}) "
          f" Sym={rs['mae']:.2f} (Δ243b={delta_s:+.2f})")

# Champion check
best_solar = min(all_results["Asym"]["Solar (SSN)"]["mae"],
                 all_results["Sym"]["Solar (SSN)"]["mae"])
print(f"\n  vs Champion (28.11): best gear = {best_solar:.2f} (Δ={best_solar-28.11:+.2f})")
if best_solar < 28.11:
