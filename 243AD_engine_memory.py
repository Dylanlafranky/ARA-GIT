#!/usr/bin/env python3
"""
Script 243AD — Engine Memory: Four Fixes for LOO Generalisation

The LOO in 243AC revealed that engines (Solar) don't generalise well.
The problem isn't geometry — it's MEMORY. Engines carry tension across
cycles, and we've been feeding them only one step of history.

Four fixes applied to 243AB-C (double log singularity breathing):

  1. GLEISSBERG MEMORY BUFFER: φ-decaying rolling average of past peaks.
     Not just prev_amp = peaks[i-1], but a weighted sum:
       N-1 weight 1.0, N-2 weight 1/φ, N-3 weight 1/φ², ...
     Creates a "tension buffer" — if past cycles were all big, the
     buffer forces the next one small to return to φ-baseline.

  2. ASYMMETRIC GRIEF BLEED: Trapped energy from a weak cycle doesn't
     dump instantly. It bleeds back over ~1/φ cycles instead of
     multiplying the next cycle by φ all at once.
     grief_bleed decays by 1/φ each cycle instead of instant correction.

  3. ARA-DEPENDENT MOMENTUM: Stop hardcoding momentum_param = 0.3.
     Derive from ARA: momentum = log1p(ARA) / 2.0
     Consumer (0.15) → 0.07 (stops dead). Engine (φ) → 0.48 (bounces).

  4. MIDLINE REINTEGRATION: From 237d. The wave doesn't pivot around 1.0.
     midline = 1 + acc_frac × (ARA - 1) where acc_frac = 1/(1+ARA).
     Solar (φ): midline = 1.236. Consumer (1/φ): midline = 0.764.

All four applied simultaneously. Full-fit + LOO on all three systems.
"""
...
class EngineMemoryNode(OrigARANode):
    ...
    def __init__(self, name, period, ara, rung, base_amp, archetype,
        ...
        # FIX 3: ARA-dependent momentum
        self.momentum_param = ara_momentum(ara)
        ...
        # FIX 2: Grief bleed state
        self.grief_reservoir = 0.0  # accumulated trapped energy
    ...
    def cascade_shape(self, t):
        ...
        # ── FIX 1: Memory buffer for inst_ara ──
        # Use φ-decaying weighted average of past amplitudes, not just prev_amp
        memory_amp = compute_memory_buffer(
            self.amp_history if hasattr(self, 'amp_history') and self.amp_history else [],
            self.prev_amp)

        if memory_amp is not None and self.base_amp > 0:
            inst_ara = memory_amp / self.base_amp
        ...
        # ── FIX 2: Slow-bleed grief ──
        # Instead of instant grief_mult, accumulate trapped energy and bleed slowly
        if self.prev_amp is not None:
            prev_dev = (self.prev_amp - self.base_amp) / self.base_amp
            if prev_dev < 0:
                # Weak cycle: energy trapped. Add to reservoir, don't dump instantly.
                self.grief_reservoir += abs(prev_dev) * INV_PHI_3
            else:
                # Strong cycle: apply normally (no reservoir needed)
                shape += (-INV_PHI_3) * prev_dev * math.exp(-PHI)

        # Bleed grief reservoir slowly (1/φ decay per cycle)
        if self.grief_reservoir > 0:
            bleed = self.grief_reservoir * INV_PHI  # bleed 1/φ of reservoir
            shape += bleed  # positive: pushes next cycle up (corrective)
            self.grief_reservoir *= (1.0 - INV_PHI)  # remaining decays
            if self.grief_reservoir < 1e-6:
                self.grief_reservoir = 0.0
...
print("=" * 78)
print("Script 243AD — Engine Memory: Four Fixes for LOO Generalisation")
print("=" * 78)
print()
print("  Fix 1: Gleissberg memory buffer (φ-decaying rolling average)")
print("  Fix 2: Slow-bleed grief (trapped energy bleeds over ~1/φ cycles)")
print("  Fix 3: ARA-dependent momentum (engines bounce, consumers stop)")
print("  Fix 4: Midline reintegration (wave pivots around ARA midline)")
print()

# Show fix values
print("  ARA-dependent parameters:")
print(f"  {'ARA':>6} │ {'Role':>10} │ {'Momentum':>8} │ {'Midline':>8} │ {'Decay@φ':>8}")
print(f"  {'─'*6}─┼─{'─'*10}─┼─{'─'*8}─┼─{'─'*8}─┼─{'─'*8}")
for ara, role in [(0.15, "deep cons"), (INV_PHI, "consumer"), (1.0, "clock"),
    ...
    print(f"  {ara:>6.3f} │ {role:>10} │ {ara_momentum(ara):>8.4f} │ "
          f"{ara_midline(ara):>8.4f} │ {decay_double_log(ara):>8.4f}")
print()
...
print(f"\n  {'System':<17} │ {'235b':>7} │ {'243AB-C':>7} │ {'243AD':>7} │ {'Δ vs 235b':>9} │ {'Δ vs AB-C':>9}")
...
print(f"\n{'=' * 78}")
print("SUMMARY — 243AD Engine Memory Fixes")
print("=" * 78)
print()
