#!/usr/bin/env python3
"""
Script 222d — Below Cascade with Wave-Shaped Phase Offset

DYLAN'S INSIGHT:
  The phase relationship between the below system and the Sun
  IS ITSELF A WAVE. The inner system's clock leads and lags
  the Sun's sinusoidally — sometimes ahead, sometimes behind.

  tr_inner = tr + amplitude × sin(schwabe_phase)

  This phase-modulates the ENTIRE cascade. All four periods
  [φ¹¹, φ⁹, φ⁶, φ⁴] get shifted, and the shift varies over
  time. The below cascade output becomes a genuinely independent
  signal that still uses the same universal geometry.

  Combined with:
  - cos(schwabe) wave carrying energy across levels
  - Singularity decay at ARA rate φ
  - Hale horizontal (inverted deviation)

2 models:
  [1] Phase oscillation amplitude = φ years
  [2] Phase oscillation amplitude = φ² years
"""
...
# PREDICTION MODEL
# =================================================================
def mk_model(h_cc=INV_PHI_3, below_cc=INV_PHI_3, phase_amp=PHI):
    """
    Three-axis crossing with wave-shaped phase offset for below.

    ABOVE CASCADE: Sun's own [φ¹¹, φ⁹, φ⁶, φ⁴] with causal gate.
    BELOW CASCADE: Same geometry, phase-modulated reference.
      tr_inner = tr + phase_amp × sin(schwabe_phase)
      The wave-shaped offset makes the below cascade genuinely
      independent while preserving the same connection angles.
      Energy carried by cos(schwabe), decays at ARA rate.
    HORIZONTAL: Hale inverted deviation.
    """
    def _p(years, amps, ba, tr):
        ...
        for i, t in enumerate(years):
            ...
            # === BELOW CASCADE (wave-shaped phase offset) ===
            sp = 2*np.pi*(t-tr)/SCHWABE

            # The inner system's clock oscillates relative to Sun's
            tr_inner = tr + phase_amp * np.sin(sp)

            # Same cascade geometry, same gate, different time reference
            below_cascade = evaluate_cascade(t, ba, tr_inner, gate_sun)
            below_dev = (below_cascade - ba) / ba

            # Energy carried by Schwabe wave, decays at ARA rate
...
if __name__ == '__main__':
    t0 = time.time()
    print("="*70)
    print("SCRIPT 222d — BELOW CASCADE: WAVE-SHAPED PHASE OFFSET")
    print("="*70)
    print(f"""
  Inner system's clock oscillates relative to Sun's:
    tr_inner = tr + amplitude × sin(schwabe_phase)

  This phase-modulates the entire cascade, creating a genuinely
  independent signal with the same universal geometry.

  Phase amp φ  = {PHI:.3f} years
  Phase amp φ² = {PHI**2:.3f} years
""")

    results = []

    # [1] Phase amplitude = φ years
    print("[1/2] Phase amp = φ years...")
    fn1 = mk_model(h_cc=INV_PHI_3, below_cc=INV_PHI_3, phase_amp=PHI)
    r = evaluate("Phase φ + Hale", fn1)
    results.append(r)
    print(f"  LOO={r['loo']:.2f}, r={r['rr']:+.3f}, s={r['sw']}/7  [{time.time()-t0:.0f}s]")

    # [2] Phase amplitude = φ² years
    print("\n[2/2] Phase amp = φ² years...")
    fn2 = mk_model(h_cc=INV_PHI_3, below_cc=INV_PHI_3, phase_amp=PHI**2)
    r = evaluate("Phase φ² + Hale", fn2)
    results.append(r)
    print(f"  LOO={r['loo']:.2f}, r={r['rr']:+.3f}, s={r['sw']}/7  [{time.time()-t0:.0f}s]")
    ...
    for i, c in enumerate(cycle_nums):
        ...
        # Use whichever phase_amp won
        pa = PHI if 'φ²' not in bn['label'] else PHI**2
        tr_inner = tr_fit + pa * np.sin(sp)
        below_c = evaluate_cascade(t, ba_fit, tr_inner, gate_sun)
    ...
    print(f"  222c cascade+decay:     LOO=37.63, 2/7, r=+0.775")
    print(f"  ---")
