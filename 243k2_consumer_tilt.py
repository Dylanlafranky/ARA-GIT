#!/usr/bin/env python3
"""
Script 243k2 — Consumer Tilt: Child Output = Consumption

Dylan: "Maybe the Sun is missing a consumer?"

Reinterpretation: The consumer ISN'T a separate rung. The consumer IS
the cycle itself. Each child rung's output represents energy CONSUMED
from the parent — not energy pushed up.

When the child snaps strong → it consumed MORE from the parent
  → parent's ARA tilts toward consumer (lower)
  → parent produces LESS next cycle
  → natural energy loss per cycle

When the child snaps weak → it consumed LESS
  → parent's ARA tilts toward engine (higher)
  → parent produces MORE next cycle

This is a SIGN FLIP from 243j's gear tilt.
Same architecture (7 rungs), same pipes, same triangle rider.
The only change is the child contribution direction.

Variant A: FLIP child sign only (child consumes, parent feeds)
Variant B: FLIP both signs (child consumes, parent pushes down)
"""
...
class ConsumerTiltNode(OrigARANode):
    def __init__(self, name, period, ara, rung, base_amp, archetype,
                 seed_ara=None, rung_offset=0, child_sign=-1, parent_sign=-1):
        ...
        self.child_sign = child_sign    # -1 = child consumes (flipped from 243j's +1)
        self.parent_sign = parent_sign  # -1 = parent feeds down (flipped from 243j's -1)
    ...
    def cascade_shape(self, t):
        ...
        if self.child_last_snap_amp is not None and self.child_engine is not None:
            ...
            # child_sign: -1 means child output = consumption (reduces parent)
            # child_sign: +1 means child output = feeding (original 243j)
            gear_tilt += self.child_sign * INV_PHI_4 * child_dev
        ...
        if self.parent_last_snap_amp is not None and self.parent_engine is not None:
            ...
            gear_tilt += self.parent_sign * INV_PHI_4 * parent_dev
...
def grid_search(period, ara, rung, times, peaks, child_sign, parent_sign,
                n_tr=80, n_ba=40):
    ...
    for tr in t_refs:
        for ba in base_amps:
            node = ConsumerTiltNode("temp", period, ara, rung, ba, "engine",
...
def run_consumer_tilt(times, peaks, period, ara, target_rung,
                      system_name="System", child_sign=-1, parent_sign=-1,
                      sim_margin=None):
    if sim_margin is None:
        sim_margin = max(period * 5, times[-1] - times[0]) * 0.2

    fit_tr, fit_ba, fit_mae = grid_search(
        period, ara, target_rung, times, peaks, child_sign, parent_sign)
    ...
    for rung_power, rung_period, prefix, eng_ara, cons_ara, offset in rungs:
        for archetype, ara_val in [("engine", eng_ara), ("clock", 1.0),
            ...
            node = ConsumerTiltNode(name, rung_period, ara_val,
...
print("=" * 78)
print("Script 243k2 — Consumer Tilt: What If Child Output = Consumption?")
print("=" * 78)
print()
print("  243j (original): child_sign=+1, parent_sign=-1")
print("  Variant A: child_sign=-1, parent_sign=-1 (child consumes)")
print("  Variant B: child_sign=-1, parent_sign=+1 (both flipped)")
print("  Variant C: child_sign=+1, parent_sign=+1 (parent feeds, not drains)")
print()
...
orig_243j = {"Solar": 52.13, "ENSO": 0.45, "EQ": 0.46}

variants = [
    ("A: child consumes",     -1, -1),
    ("B: both flipped",       -1, +1),
    ("C: parent feeds up",    +1, +1),
    ("243j (reference)",      +1, -1),
]

print(f"  {'Variant':<22} │ {'Solar':>7} │ {'ENSO':>6} │ {'EQ':>6} │ Notes")
print(f"  {'─'*22}┼{'─'*9}┼{'─'*8}┼{'─'*8}┼{'─'*30}")
print(f"  {'235b baseline':<22} │ {base_solar['mae']:>7.2f} │ {base_enso['mae']:>6.2f} │ {base_eq['mae']:>6.2f} │ proven champion cascade")
print(f"  {'Best 243 series':<22} │ {best_243['Solar']:>7.2f} │ {best_243['ENSO']:>6.2f} │ {best_243['EQ']:>6.2f} │ best from each variant")
print(f"  {'─'*22}┼{'─'*9}┼{'─'*8}┼{'─'*8}┼{'─'*30}")

for vname, csign, psign in variants:
    print(f"  Running {vname}...", end="", flush=True)

    r_sol = run_consumer_tilt(solar_t, solar_p, 11.0, PHI, 4,
                              "Solar", child_sign=csign, parent_sign=psign)
    r_enso = run_consumer_tilt(enso_t, enso_p, 3.7, INV_PHI_2, 2,
                               "ENSO", child_sign=csign, parent_sign=psign)
    r_eq = run_consumer_tilt(eq_t, eq_p, 25.0, 0.5, 3,
                             "EQ", child_sign=csign, parent_sign=psign)
    ...
    print(f"\r  {vname:<22} │ {r_sol['mae']:>7.2f}{sol_mark:2s} │ {r_enso['mae']:>6.2f}{enso_mark:2s} │ {r_eq['mae']:>6.2f}{eq_mark:2s} │ {', '.join(notes) if notes else ''}")

    # Print per-cycle for the best solar variant
    if vname == "A: child consumes":
        print()
        print("  Per-cycle (Variant A):")
