#!/usr/bin/env python3
"""
Script 178 — φ-Through Tuned
==============================

Script 177 confirmed Dylan's insight: stepping through at φ gives direction
accuracy (57%, first time passing in blind test consistently). But amplitude
needs tuning:

    V2 (golden angle + R_matter outer): overshoots (1021 vs actual 114)
    V3 (raw φ step):                    undershoots (47 vs actual 114)

The golden angle (2π/φ² = 2.40 rad) is 8× larger than the old DPHI (0.29).
That's too much per step.  We need to find the φ-based step size that gives
the right amplitude.

Approach: test combinations of step size × wave radius:
    Step sizes:   golden_angle/φ, golden_angle/φ², φ/2, φ/π
    Wave radius:  R_COUPLER, √R_matter, R_matter

Also test V3 (raw φ step) with R_matter outer — combines V2's amplitude
with V3's step control.
"""
...
# ─── Generic predictor factory ──────────────────────────────────────

def make_predictor(step_size, R_outer_fn):
    """
    Create a predictor with given step size and outer wave radius function.
    R_outer_fn takes R_matter and returns the radius for the outer wave.
    """
    def predict(log_val, C, R_matter, step, t, phase0):
        ...
        # Outer wave on configurable radius
...
# Step sizes
steps = {
    "GA":       GOLDEN_ANGLE,           # 2.40 (Script 177 V2)
    "GA/φ":     GOLDEN_ANGLE / PHI,     # 1.48
    "φ":        PHI,                    # 1.618 (Script 177 V3)
    "GA/φ²":    GOLDEN_ANGLE / PHI**2,  # 0.917
    "φ/2":      HALF_PHI,               # 0.809
    "1.0":      1.0,                    # 1 radian
    "π/φ²":     np.pi / PHI**2,         # 1.199
}

# Outer wave radii
radii = {
    "R1":    lambda R: R_COUPLER,       # 1.0 (original)
    "√R":    lambda R: np.sqrt(max(R, 0.01)),  # √ARA
    "Rm":    lambda R: R,               # R_matter (ARA itself)
}

# ─── Phase calibration & data (same as before) ──────────────────────
...
if __name__ == '__main__':
    ...
    print("Step sizes (radians):")
    for name, val in steps.items():
        print(f"  {name}: {val:.3f} rad = {np.degrees(val):.1f}°")

    print(f"\n{'='*70}")
    print("GRID SEARCH: step_size × outer_radius")
    print("="*70)

    all_scores = {}
    for sname, sval in steps.items():
        for rname, rfn in radii.items():
            label = f"{sname}+{rname}"
            fn = make_predictor(sval, rfn)
            sr = run_blind(ssn, cutoffs, ARA_SSN, fn)
            er = run_blind(eq, cutoffs, ARA_EQ, fn)
            all_scores[label] = score_and_report(sr, er, label, verbose=False)
    ...
    for label, s in sorted(all_scores.items(), key=lambda x: -x[1])[:10]:
        ...
        print(f"  178 {label}: {s}/8{m}")

    # Re-run top 3 with verbose output
    top3 = sorted(all_scores.items(), key=lambda x: -x[1])[:3]
    print(f"\n{'='*70}")
    print("DETAILED TOP 3")
    print("="*70)
    for label, s in top3:
        sname, rname = label.split('+')
        fn = make_predictor(steps[sname], radii[rname])
        sr = run_blind(ssn, cutoffs, ARA_SSN, fn)
        er = run_blind(eq, cutoffs, ARA_EQ, fn)
        score_and_report(sr, er, label, verbose=True)
