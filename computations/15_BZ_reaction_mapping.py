...
print()
print("  CRITICAL DISTINCTION: NAKED vs CONTAINED Type 3")
print("  In a sealed beaker (CLOSED system):")
print("    - Each burst depletes reactants (malonic acid, bromate)")
print("    - No resupply → oscillation dies after ~10-20 minutes")
print("    - This is NAKED Type 3: destruction with no supply chain")
print()
print("  In a CSTR (Continuous Stirred-Tank Reactor, OPEN system):")
print("    - Fresh reactants continuously pumped in, waste removed")
print("    - Type 3 coupling still present (burst still consumes Br⁻)")
print("    - But wrapped in Type 1 supply chain → oscillates INDEFINITELY")
print("    - This is CONTAINED Type 3: destruction sustained by logistics")
print()
print("  The heart works the same way: each beat consumes ATP/O₂ (Type 3")
print("  at molecular level), but the circulatory system (Type 1 supply)")
print("  continuously replenishes fuel. Heart in a jar dies in minutes —")
print("  just like BZ in a beaker.")
print()
print("  PREDICTION from topology: In a CLOSED system, naked Type 3 → finite lifespan.")
print("  In an OPEN system with Type 1 supply → indefinite persistence.")
print()
...
print(f"    log₁₀(Action/π) = {log_bz:.2f} (for 50 mL beaker)")
print(f"    → HUMAN cluster (log -5 to +5)")
print()
print(f"  BEAKER PROBLEM CAVEAT:")
print(f"    Unlike a heart (fixed spatial boundary), BZ energy scales with volume:")
print(f"    50 µL droplet → E ≈ 0.01 J → log₁₀ ≈ -0.90")
print(f"    50 mL beaker  → E ≈ 10 J   → log₁₀ ≈ 2.10")
print(f"    5000 L vat    → E ≈ 10⁶ J  → log₁₀ ≈ 7.10")
print(f"    The ARA and period are volume-INDEPENDENT (physically intrinsic).")
print(f"    The Action/π requires normalization for continuous media:")
print(f"    e.g., per mole, per unit volume, or per wavefront.")
print(f"    What IS meaningful: BZ operates at chemical energy scales (J)")
print(f"    and accessible timescales (s), placing it in the broad habitable")
print(f"    zone (log -5 to +5) regardless of normalization convention.")
print()
...
predictions = [
    ...
    ("Finite lifespan in closed system (naked Type 3); indefinite in CSTR (contained Type 3)",
     "✓ Dies in beaker (~10-20 min); oscillates indefinitely in CSTR",
     "Persistence depends on coupling topology: naked vs contained Type 3"),
...
print("  - Raw chemistry reaches ARA ≈ 2.0 (BZ in beaker) — sustained but mortal*")
print("  - Raw chemistry + supply chain (BZ in CSTR) — sustained AND immortal")
...
print()
print("  *'Mortal' = naked Type 3 in a closed system (no supply chain).")
print("  Hearts also consume fuel each beat (Type 3 at molecular level),")
print("  but the circulatory system (Type 1 supply) replenishes continuously.")
print("  BZ in a CSTR proves: add a supply chain → immortality, even at ARA ≈ 2.3.")
print()
...
print(f"  │ log₁₀(Action/π) = {log_bz:.2f}                       │")
print(f"  │ Cluster: HUMAN (log -5 to +5) *volume-dependent  │")
print(f"  │ Zone: Engine/Exothermic boundary                │")
print(f"  │ Predictions: {confirmed}/{len(predictions)} confirmed                      │")
print(f"  │ Type 3: naked in beaker (mortal), contained     │")
print(f"  │         in CSTR (immortal)                      │")
print(f"  └────────────────────────────────────��────────────┘")
print()
...
print("  3. The path from dead chemistry to life has TWO axes:")
print("     a. ARA drift: 2.0 → φ (temporal efficiency)")
print("     b. Coupling: naked Type 3 → Type 3 contained in Type 1 (supply logistics)")
print("  4. The CSTR proves the logistical axis independently:")
print("     BZ at ARA 2.3 + Type 1 supply = indefinite persistence")
print("  5. Natural selection drives both axes simultaneously")
print()
print("  Abiogenesis didn't need a miracle.")
print("  It needed a chemical oscillator to find the φ-attractor")
print("  AND build a supply chain.")
