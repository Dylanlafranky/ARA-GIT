#!/usr/bin/env python3
"""
Script 176 — Tuned Nested Circles
===================================

V3 from Script 175 had the RHYTHM (60% direction, 16 oscillation turns)
but the amplitude exploded (gear=ARA²=2.99 was too aggressive).

V2 had the AMPLITUDE (51% within 2×) but no rhythm.

Fix: V3's nested circle mechanism with the gear ratio dialled back.

Structure:
    Inner circle:  R = 1/ARA  (fits inside, makes full rotations)
    Outer mapping: R = ARA    (covers data's full range)
    Gear ratio:    varies     (ARA, φ, √ARA, etc.)
    Clock:         Hale cycle (effective ARA oscillates)
    φ-stable:      inside the inner circle (prevents runaway)
"""
...
def nested_predict(log_val, C, R_matter, step, t, phase0, gear_ratio):
    """
    Nested circle prediction with configurable gear ratio.

    1. Map value on OUTER circle (R=R_matter) — no clipping at extremes
    2. φ-stable on INNER circle (R=1/R_matter) — stability
    3. Standard wave on INNER circle — rhythm
    4. Clock modulates drive strength
    5. Output amplified by gear_ratio
    """
    eff = effective_ara(R_matter, t, phase0)
    eff = max(eff, 0.1)

    R_outer = R_matter
    R_inner = 1.0 / max(R_matter, 0.1)
    ...
    # Standard wave on inner circle
    s1n = wave(phi, R_inner)
    s2n = wave(phi+HALF_PHI, R_inner)
    s1x = wave(phi_next, R_inner)
    s2x = wave(phi_next+HALF_PHI, R_inner)
    outer_drive = ((s1x+s2x)/2 - (s1n+s2n)/2)
    ...
    # Combined, amplified by gear ratio
    dlog = (inner + drive * gear_engage * outer_drive) * gear_ratio * np.exp(-MIDPOINT_OFFSET)
    return log_val + dlog

# Factory functions for each gear ratio
def make_predictor(gear_ratio):
    def pred(log_val, C, R_matter, step, t, phase0):
        return nested_predict(log_val, C, R_matter, step, t, phase0, gear_ratio)
    return pred
...
if __name__ == '__main__':
    ...
    gear_ratios = {
        "gear=1.0 (no amplification)": 1.0,
        "gear=ARA (1.73)": ARA_SSN,          # The target
        "gear=φ (1.618)": PHI,
        "gear=√ARA (1.32)": np.sqrt(ARA_SSN),
        "gear=ARA/φ (1.07)": ARA_SSN / PHI,
        "gear=φ/2+1 (1.81)": HALF_PHI + 1,
        "gear=2.0": 2.0,
        "gear=ARA² (2.99) [Script 175 V3]": ARA_SSN**2,
    }

    print("="*70)
    print("OSCILLATION DIAGNOSTIC")
    print("="*70)
    for label, gr in gear_ratios.items():
        fn = make_predictor(gr)
