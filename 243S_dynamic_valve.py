#!/usr/bin/env python3
"""
Script 243S — Dynamic Valve Alpha (Triangle Method)

243R showed a clear trade-off: lower alpha → better EQ/ENSO, worse Solar.
The insight: engines NEED the receive valve (damping protects stability),
consumers DON'T (they should absorb freely). The receiver's own ARA
already encodes this — high ARA = engine = needs protection, low ARA =
consumer = let it through.

Instead of a fixed valve_alpha for all nodes, compute alpha dynamically
from the receiver's instantaneous ARA in cascade_shape. The triangle
determines its own valve power.

Three alpha mappings (lean run):
  A — Linear:    alpha = inst_ara / 2.0  (ARA 0→0, ARA 2→1)
  B — Self-ref:  alpha = acc             (valve determines its own power)
  C — Inverse:   alpha = 1 - acc         (engines high, consumers low)
"""
...
class BreathingGearNode(OrigARANode):
    """
    Triangle rider node with DYNAMIC VALVE-GATED temporal gear.

    Instead of a fixed valve_alpha, the alpha is computed at runtime
    from the receiver's instantaneous ARA. The triangle determines
    its own valve power:
      - Engine (high ARA) → high alpha → full compression (protect)
      - Consumer (low ARA) → low alpha → open valve (absorb freely)
    """

    def __init__(self, name, period, ara, rung, base_amp, archetype,
                 seed_ara=None, rung_offset=0,
                 child_sign=1, parent_sign=-1,
                 gate_send=False, valve_mode="linear"):
        ...
        self.valve_mode = valve_mode    # "linear", "self_ref", "inverse"
    ...
    def cascade_shape(self, t):
        ...
        # ── DYNAMIC ALPHA ──
        # The receiver's own nature sets its valve power.
        if self.valve_mode == "linear":
            # ARA near 2 → alpha near 1, ARA near 0 → alpha near 0
            dynamic_alpha = inst_ara / 2.0
        elif self.valve_mode == "self_ref":
            # The valve determines its own power: acc itself
            # High acc (consumer) → high alpha → MORE compression (wrong way?)
            # Actually: high acc = consumer, alpha=acc≈0.62 → moderate
            #           low acc = engine, alpha=acc≈0.38 → lighter
            # This is the OPPOSITE of what we want for engines...
            # But let's test it — self-referential systems can surprise
            dynamic_alpha = receiver_acc
        elif self.valve_mode == "inverse":
            # 1 - acc: engines get high alpha, consumers get low
            # Engine: acc≈0.38 → alpha≈0.62
            # Consumer: acc≈0.62 → alpha≈0.38
            # Clock: acc=0.50 → alpha=0.50
            dynamic_alpha = 1.0 - receiver_acc
        else:
            dynamic_alpha = 0.5  # fallback

        dynamic_alpha = max(0.0, min(1.0, dynamic_alpha))

        # ── TEMPORAL GEAR TILT (dynamically valve-gated) ──
        gear_tilt_raw = 0.0
        ...
        # Gate the tilt by receiver's valve with DYNAMIC alpha
        effective_acc = receiver_acc ** dynamic_alpha
        gear_tilt_gated = gear_tilt_raw * effective_acc
        ...
        self.alpha_history.append(dynamic_alpha)
...
def run_breathing_gear(times, peaks, period, ara, target_rung,
    ...
    fit_tr, fit_ba, fit_mae = grid_search(
        ...
        gate_send=gate_send, valve_mode=valve_mode)
    ...
    alpha_hist = target_node.alpha_history
    ...
    return {
        ...
        "avg_alpha": np.mean(alpha_hist) if alpha_hist else 0,
        "alpha_range": (min(alpha_hist), max(alpha_hist)) if alpha_hist else (0, 0),
...
VARIANTS = {
    "A": {
        "label": "Linear: alpha = ARA/2 (engines high, consumers low)",
        "child_sign": -1, "parent_sign": -1, "gate_send": True,
        "valve_mode": "linear",
    },
    "B": {
        "label": "Self-ref: alpha = acc (valve sets own power)",
        "child_sign": -1, "parent_sign": -1, "gate_send": True,
        "valve_mode": "self_ref",
    },
    "C": {
        "label": "Inverse: alpha = 1-acc (mirror of acc)",
        "child_sign": -1, "parent_sign": -1, "gate_send": True,
        "valve_mode": "inverse",
    },
}
...
print("=" * 78)
print("Script 243S — Dynamic Valve Alpha (Triangle Method)")
print("=" * 78)
print()
print("The receiver's ARA determines its own valve power at runtime.")
print("Engines protect themselves (high alpha), consumers open up (low alpha).")
print()
print("  A: alpha = ARA/2    (linear, 0-to-1)")
print("  B: alpha = acc      (self-referential)")
print("  C: alpha = 1 - acc  (inverse — engines high, consumers low)")
print()

ref = {
    '235b':   {"Solar (SSN)": 46.30, "ENSO (ONI)": 0.44, "Sanriku EQ": 1.19},
    '243O-C': {"Solar (SSN)": 48.45, "ENSO (ONI)": 0.41, "Sanriku EQ": 1.07},
    '243R-A': {"Solar (SSN)": 50.60, "ENSO (ONI)": 0.45, "Sanriku EQ": 0.74},
}
...
for var_key in ["A", "B", "C"]:
    ...
    print(f"    child_sign={var['child_sign']}, parent_sign={var['parent_sign']}, "
          f"gate_send={var['gate_send']}, valve_mode={var['valve_mode']}")
    print(f"{'─' * 78}")
    print(f"\n  {'System':<16} │ {'235b':>7} │ {'243O-C':>7} │ {'243R-A':>7} │ {'243S':>7} │ "
          f"{'Δ vs 235b':>9} │ {'α avg':>6} │ {'α range':>12}")
    print(f"  {'─'*16}─┼─{'─'*7}─┼─{'─'*7}─┼─{'─'*7}─┼─{'─'*7}─┼─{'─'*9}─┼─{'─'*6}─┼─{'─'*12}")
    ...
    for name, times, peaks, period, ara, rung in SYSTEMS:
        ...
        print(f"\r  {name:<16} │ {ref['235b'][name]:>7.2f} │ {ref['243O-C'][name]:>7.2f} │ "
              f"{ref['243R-A'][name]:>7.2f} │ {res['mae']:>7.2f} │ {d235:>+8.2f}{b235} │ "
              f"{res['avg_alpha']:>.4f} │ [{ar[0]:.3f},{ar[1]:.3f}]")

    # Valve diagnostics
    print(f"\n  Valve & alpha diagnostics:")
    for name in var_results:
        r = var_results[name]
        if r["failed"]: continue
        vr = r["valve_range"]
        ar = r["alpha_range"]
        print(f"    {name:<16}: avg_valve={r['avg_valve']:.4f}, avg_alpha={r['avg_alpha']:.4f}, "
              f"alpha_range=[{ar[0]:.4f}, {ar[1]:.4f}], "
...
print(f"\n{'=' * 78}")
print("SUMMARY — 243S Dynamic Valve Alpha (Triangle Method)")
print("=" * 78)
print()
print("  The triangle determines its own valve power:")
print("    Engine (ARA~φ) → high alpha → receive valve compressed")
print("    Consumer (ARA~1/φ) → low alpha → receive valve open")
print()
print("  This is the triangle method: the same formula that classifies")
print("  engine/clock/consumer also sets the valve strength.")
print()
print(f"  Baselines: 235b Solar=46.30, 243O-C Solar=48.45, 243R-A Solar=50.60")
print(f"  Target: beat 243O-C on ENSO+EQ without regressing Solar as much")
