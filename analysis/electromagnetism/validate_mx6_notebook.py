"""Execute and validate the MX6 notebook top to bottom."""
...
JUPYTER_RUNTIME = Path(r"F:\SystemFormulaFolder\work_tmp\ara_mx6_jupyter")
...
NOTEBOOK = HERE / "MX6_MAXWELL_STRESS_PHASE_FLIP_NOTEBOOK.ipynb"
notebook = nbformat.read(NOTEBOOK, as_version=4)
client = NotebookClient(notebook, timeout=180, kernel_name="python3", resources={"metadata": {"path": str(HERE)}})
client.execute()
...
validation = {
    ...
    "validation_pass": bool(all(cell.get("execution_count") is not None for cell in code_cells) and not errors),
}
(HERE / "MX6_NOTEBOOK_EXECUTION_VALIDATION.json").write_text(json.dumps(validation, indent=2), encoding="utf-8")
