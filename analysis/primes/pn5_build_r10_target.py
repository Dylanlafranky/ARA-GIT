...
PREDICTIONS = HERE / "PN5_FROZEN_PREDICTIONS.json"
FREEZE_MANIFEST = HERE / "PN5_FROZEN_PREDICTION_MANIFEST.json"
TARGET_PACKET = HERE / "PN5_R10_TARGET_AGGREGATES.json"
...
def edge_death(left: np.ndarray, right: np.ndarray) -> np.ndarray:
    ...
    le = np.where(left == 0, sentinel, left)
    re = np.where(right == 0, sentinel, right)
    out = np.minimum(le, re).astype(np.uint32)
...
def main() -> None:
    ...
    q_end = np.asarray(frozen["gate_path"]["q_end"], dtype=np.int64)
    qmax = int(frozen["gate_path"]["qmax"])

    all_primes = primes_through(math.isqrt(high - 1))
    active = all_primes[all_primes > 29]
    if int(active[-1]) != qmax:
        raise AssertionError("Target gate maximum does not match frozen packet")
    ...
    pair_deaths = np.zeros(cells, dtype=np.int64)
    candidate_count = 0
    pair_count = 0
    candidate_survivors = 0
    pair_survivors = 0
    last_death: np.uint32 | None = None
    ...
    for chunk_low in range(low, high, CHUNK_SIZE):
        ...
        for p_value in active:
            p = int(p_value)
            start = max(p * p, ((chunk_low + p - 1) // p) * p)
            if start >= chunk_high:
                continue
        ...
        if last_death is not None:
            bridge = edge_death(np.asarray([last_death], dtype=np.uint32), death[:1])
            pair_count += 1
            if bridge[0] == 0:
                pair_survivors += 1
            else:
                add_death_counts(bridge, factor_cell, pair_deaths)
        if len(death) > 1:
            local_edges = edge_death(death[:-1], death[1:])
            pair_count += len(local_edges)
            pair_survivors += int(np.count_nonzero(local_edges == 0))
            add_death_counts(local_edges, factor_cell, pair_deaths)
        last_death = death[-1]
        chunks += 1
        if chunks % 10 == 0:
    ...
    if pair_count != pair_survivors + int(pair_deaths.sum()):
        raise AssertionError("Pair accounting does not close")
    if pair_count != candidate_count - 1:
        raise AssertionError("Pair count is not candidate count minus one")

    def path(total: int, deaths: np.ndarray) -> dict[str, np.ndarray]:
        ...
        for j in range(cells):
            before[j] = alive
            alive -= int(deaths[j])
            survival[j] = alive / total
        return {"before": before, "deaths": deaths, "survival": survival, "hazard": deaths / before}

    candidate_path = path(candidate_count, candidate_deaths)
    edge_path = path(pair_count, pair_deaths)
    elapsed = time.perf_counter() - started
    packet = {
        "test_id": frozen["test_id"],
        "target_state": "OPENED AFTER FROZEN PREDICTION HASH VERIFIED",
        ...
        "edge": {"n0": pair_count, "terminal_survivors": pair_survivors, **edge_path},
        "accounting_checks": {
            "candidate_closes": candidate_count == candidate_survivors + int(candidate_deaths.sum()),
            "edge_closes": pair_count == pair_survivors + int(pair_deaths.sum()),
            "edge_count_is_candidate_minus_one": pair_count == candidate_count - 1,
        },
