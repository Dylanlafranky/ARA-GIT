...
**Test ID:** `T325-PHI-CIRCLE-TRAIN-PHYLLOTAXIS-v2`  
...
**Correction history:** v1 aborted at its source-range eligibility check before
any endpoint was calculated. The workbook contains directed perturbation angles
above `180 degrees`; v2 corrects that source declaration and nothing else.
...
9. **Direction boundary:** Source Data 21 records an oriented divergence angle
   on `0..360 degrees`; mutant values extend above `180 degrees`. It does not
   include a separate clockwise/counter-clockwise chirality field. The published
   wild-type convention is centred near the `137.5 degree` golden angle, so T325
   freezes that recorded orientation. It does not infer a hidden chirality.
...
Because the published wild-type orientation is the approximately
`137.5 degree` branch rather than its `222.5 degree` complement, the
predeclared source-compatible orientation is
...
The equivalence between `+2/phi` and `-2/phi` is mathematical. The source's
angle field selects one recorded orientation but does not separately encode a
physical chirality label.
...
All increments are written in the source's wild-type `137.5 degree`
orientation. Perturbation events remain exactly as recorded, including values
above `180 degrees`.
