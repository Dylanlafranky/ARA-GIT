#!/usr/bin/env python3
"""
Script 223i — Beeswax Edge-Gated Mirror Collision

DYLAN'S INSIGHT:
  At VERTICES (pointed, |cos| high):
    The two balls travel TOGETHER down the corridor.
    They glance off each other — weak interaction.

  At EDGES (diverging, |cos| low, zero-crossing):
    The balls SEPARATE — forced through their own doors.
    This is where the real collision energy exchange happens.

  The champion mirror collision (-cos_prev × cos_curr) is strongest
  at VERTICES. But Dylan says the real action is at EDGES.

  Edge collision uses VELOCITY (sin), not position (cos):
    At zero-crossings, sin is at max — balls have max velocity.
    Opposite velocities = head-on separation = strong exchange.
    Same velocities = parallel separation = weak exchange.

  No vertex boost on base coupling (that crashed in 223h).

3 models:
  [1] Edge-only mirror: sin-based collision at zero-crossings
  [2] Blended: weak vertex glance + strong edge separation
  [3] Best of above + gentle log tension (from 223g)
"""
...
# =================================================================
# MODEL 1: Edge-only mirror collision
# =================================================================
def mk_edge_mirror(h_cc=INV_PHI_3):
    """
    Mirror collision fires at EDGES (zero-crossings) where balls separate.

    At zero-crossings: |sin| is high (max velocity), |cos| is low.
    Edge proximity: (1-|cos_prev|)(1-|cos_curr|) — high when both
    balls are near zero-crossings (doors wide open, balls separating).

    Direction: -sin_prev × sin_curr
      Opposite velocities (head-on separation) → positive → strong
      Same velocities (parallel separation) → negative → weak

    Standard tension asymmetry (no φ scaling, no log).
    """
    def _p(years, amps, ba, tr):
        ...
        for i, t in enumerate(years):
            ...
            for j, per in enumerate(PERIODS):
                ...
                # EDGE-GATED mirror collision
                if j > 0:
                    # Edge proximity: both near zero-crossing
                    edge_prev = 1 - abs(cos_vals[j-1])
                    edge_curr = 1 - abs(cos_vals[j])
                    door_open = edge_prev * edge_curr

                    # Separation direction: opposite velocities = head-on
                    sep_dir = -sin_vals[j-1] * sin_vals[j]

                    # Collision at edges, not vertices
                    eps *= (1 + sep_dir * door_open * PHI)
...
# =================================================================
# MODEL 2: Blended — weak vertex glance + strong edge separation
# =================================================================
def mk_blended_mirror(h_cc=INV_PHI_3):
    """
    Both interactions exist, weighted by position:

    VERTEX (|cos| high): balls travel together, weak glancing
      Champion-style -cos_prev × cos_curr, but scaled by 1/φ
      Weighted by vertex proximity: |cos_prev| × |cos_curr|

    EDGE (|cos| low): balls separate, strong collision
      Sin-based: -sin_prev × sin_curr
      Weighted by edge proximity: (1-|cos_prev|)(1-|cos_curr|)
      Scaled by φ (strong)

    The blend naturally transitions: at any position, the ball
    is either more vertex-like or more edge-like.
    """
    def _p(years, amps, ba, tr):
        preds = []
        for i, t in enumerate(years):
            ...
            for j, per in enumerate(PERIODS):
                ...
                if j > 0:
                    # VERTEX: traveling together, weak glancing
                    vertex_wt = abs(cos_vals[j-1]) * abs(cos_vals[j])
                    vertex_col = -cos_vals[j-1] * cos_vals[j]

                    # EDGE: separating, strong collision
                    edge_wt = (1 - abs(cos_vals[j-1])) * (1 - abs(cos_vals[j]))
                    edge_col = -sin_vals[j-1] * sin_vals[j]

                    # Blend: weak vertex + strong edge
...
# =================================================================
# MODEL 3: Blended mirror + gentle log tension
# =================================================================
def mk_blended_log(h_cc=INV_PHI_3):
    """
    Best collision model + gentle log tension from 223g.
    Log1p on tension: always touching walls in beeswax.
    Vertex glance + edge separation for mirror.
    """
    LOG2 = np.log(2)
    def _p(years, amps, ba, tr):
        preds = []
        for i, t in enumerate(years):
            ...
            for j, per in enumerate(PERIODS):
                ...
                # Gentle LOG tension (beeswax: always touching walls)
...
if __name__ == '__main__':
    t0 = time.time()
    print("="*70)
    print("SCRIPT 223i — BEESWAX EDGE-GATED MIRROR COLLISION")
    print("="*70)
    print(f"""
  Vertex (|cos| high): balls travel together, weak glancing (×1/φ)
  Edge   (|cos| low):  balls separate through doors, strong (×φ)

  Edge collision uses VELOCITY (-sin), not position (cos).
  Opposite velocities at separation = head-on = max exchange.
""")

    results = []

    # [1] Edge-only mirror
    print("[1/3] Edge-only mirror collision...")
    fn1 = mk_edge_mirror(h_cc=INV_PHI_3)
    r = evaluate("Edge mirror", fn1)
    results.append(r)
    print(f"  LOO={r['loo']:.2f}, r={r['rr']:+.3f}, s={r['sw']}/7  [{time.time()-t0:.0f}s]")

    # [2] Blended vertex+edge
    print("\n[2/3] Blended vertex glance + edge separation...")
    fn2 = mk_blended_mirror(h_cc=INV_PHI_3)
    r = evaluate("Blend v+e", fn2)
    results.append(r)
    print(f"  LOO={r['loo']:.2f}, r={r['rr']:+.3f}, s={r['sw']}/7  [{time.time()-t0:.0f}s]")

    # [3] Blended + log tension
    print("\n[3/3] Blended mirror + log tension...")
    fn3 = mk_blended_log(h_cc=INV_PHI_3)
    r = evaluate("Blend+log", fn3)
    results.append(r)
    print(f"  LOO={r['loo']:.2f}, r={r['rr']:+.3f}, s={r['sw']}/7  [{time.time()-t0:.0f}s]")
