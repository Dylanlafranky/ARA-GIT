#!/usr/bin/env python3
"""
Script 237k2 — Refined Camshaft Valve

The palindrome zone is [0, 1/φ] φ-rungs from clock.
Inside: perfect φ energy transfer → midline = 1.0.

The ramp zone is [1/φ, 1] φ-rungs.
Width of ramp = 1 - 1/φ = 1/φ² (all φ-geometry).
Factor ramps quadratically from 0 to 1.

Beyond 1 full φ-rung: full inverse valve offset.

         zone=1/φ    1 rung
    ─────|──────────|──────────
    0    1/φ        1         φ-rungs
    factor=0   ramp²     factor=1

Variants:
E) Ramp-to-1: full offset at 1 φ-rung
F) Ramp-to-1 with linear (not quadratic) transition
G) Zone at 1/φ, ramp linear to φ rungs (wider ramp)
"""
...
def midline_camshaft_E(ara):
    """E) Palindrome zone [0, 1/φ], quadratic ramp to 1 rung, full beyond.

    zone_width = 1/φ          (palindrome boundary)
    ramp_width = 1/φ²         (= 1 - 1/φ)
    full_threshold = 1 rung   (= zone + ramp)
...
def midline_camshaft_F(ara):
    """F) Same zones but linear ramp (not quadratic).

    factor = (pd - 1/φ) / (1/φ²)   for pd in [1/φ, 1]
    """
    inv_offset = midline_inverse_valve(ara) - 1.0
    pd = _phi_dist(ara)
    zone = 1.0 / PHI
...
print("=" * 78)
print("237k2 — Refined Camshaft Valve")
print("=" * 78)
print()
print("MIDLINE VALUES:")
print(f"  {'System':<12} {'ARA':>6} {'φ-dist':>7} {'InvV':>8} "
      f"{'E-quad':>8} {'F-lin':>8}")
print(f"  {'─'*12} {'─'*6} {'─'*7} {'─'*8} {'─'*8} {'─'*8}")
for name, a in test_systems:
    pd = _phi_dist(a)
    print(f"  {name:<12} {a:>6.3f} {pd:>7.3f} "
          f"{midline_inverse_valve(a):>8.4f} "
          f"{midline_camshaft_E(a):>8.4f} "
          f"{midline_camshaft_F(a):>8.4f}")
print()
print(f"  Palindrome zone: [0, 1/φ] = [0, {1/PHI:.3f}] φ-rungs")
print(f"  Ramp zone:       [1/φ, 1] (width = 1/φ² = {1/PHI**2:.3f})")
print(f"  Full zone:       [1, ∞)")
print()
...
configs = [
    ...
    ("Camshaft-E",  make_predict_fn(midline_camshaft_E)),
    ("Camshaft-F",  make_predict_fn(midline_camshaft_F)),
]
...
for sys_name, cfg in systems.items():
    ...
    print(f"── {sys_name.upper()} (ARA={ara:.3f}, φ-dist={_phi_dist(ara):.3f}) ──")
