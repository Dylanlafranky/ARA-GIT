#!/usr/bin/env python3
"""
Script 243R — Acceptance-Compensated Breathing Gear

The valve acc = 1/(1+ARA) gates reception, but there's no compensation
for the compression it causes. An engine (acc~0.38) gets crushed to 38%
and nobody gives that back. The double-gate product (send x receive)
for engine-engine is ~0.24 — that's the amplitude suppression.

The fix: soften the receive valve with acc^alpha where alpha < 1.
The send gate (1-acc) stays untouched — that's the directionality.

  alpha=0 → acc^0 = 1 → receive valve OFF → send-only asymmetry
  alpha=1 → acc^1 = acc → full compression → 243O-C behavior

Three variants (lean run):
  A — Send-only: alpha=0 (receive valve removed)
  B — Half-power: alpha=1/phi (softer compression)
  C — Quarter-power: alpha=1/phi^2 (barely there)
"""
...
class BreathingGearNode(OrigARANode):
    ...
    def __init__(self, name, period, ara, rung, base_amp, archetype,
                 ...
                 gate_send=False, valve_alpha=1.0):
        ...
        self.valve_alpha = valve_alpha     # Receive valve power: acc^alpha (0=off, 1=full)
    ...
    def cascade_shape(self, t):
        ...
        # Gate the tilt by receiver's valve (compensated)
        # alpha=0 → acc^0=1 (no receive gate, send-only asymmetry)
        # alpha=1 → acc^1=acc (full 243O-C compression)
        effective_acc = receiver_acc ** self.valve_alpha
...
def run_breathing_gear(times, peaks, period, ara, target_rung,
    ...
    fit_tr, fit_ba, fit_mae = grid_search(
        ...
        gate_send=gate_send, valve_alpha=valve_alpha)
...
VARIANTS = {
    "A": {
        "label": "Send-only: alpha=0 (receive valve OFF)",
        "child_sign": -1, "parent_sign": -1, "gate_send": True,
        "valve_alpha": 0.0,
    },
    "B": {
        "label": "Half-power: alpha=1/φ (softer compression)",
        "child_sign": -1, "parent_sign": -1, "gate_send": True,
        "valve_alpha": INV_PHI,
    },
    "C": {
        "label": "Quarter-power: alpha=1/φ² (barely there)",
        "child_sign": -1, "parent_sign": -1, "gate_send": True,
        "valve_alpha": INV_PHI_2,
    },
}
...
print("=" * 78)
print("Script 243R — Acceptance-Compensated Breathing Gear")
print("=" * 78)
print()
print("Receive valve acc^alpha: alpha=0 removes it, alpha=1 is 243O-C.")
print("A: alpha=0 (send-only)   B: alpha=1/φ   C: alpha=1/φ²")
print()
...
for var_key in ["A", "B", "C"]:
    ...
    print(f"    child_sign={var['child_sign']}, parent_sign={var['parent_sign']}, "
          f"gate_send={var['gate_send']}, valve_alpha={var.get('valve_alpha',1.0):.4f}")
    print(f"{'─' * 78}")
    ...
    for name, times, peaks, period, ara, rung in SYSTEMS:
        ...
        res = run_breathing_gear(
            ...
            valve_alpha=var.get("valve_alpha", 1.0))
...
print(f"\n{'=' * 78}")
print("SUMMARY — 243R Acceptance-Compensated Breathing Gear")
print("=" * 78)
print()
print("  Receive valve uses acc^alpha instead of acc.")
print("  Send gate (1-acc) kept intact for directionality.")
print("  alpha=0 → send-only, alpha=1 → 243O-C full compression.")
print()
print(f"  Key question: Does softening the receive valve")
print(f"  recover amplitude while keeping send asymmetry?")
print()
