#!/usr/bin/env python3
"""
Script 243AK — Gate-Modulated Amplitude Scale

Based on 243AK (midline champion) + 243AJ amplitude insight.

The midline captures HALF the accumulate/snap energy relationship (center shift).
The other half is amplitude scaling — but it should follow the gate's timing,
not be static. The wave follows the path of least resistance:

  - During accumulation: energy builds → amp_scale ramps toward φ
  - At accumulation peak: amp_scale = φ, midline × φ = 2.0 (full energy)
  - During release: amp_scale fades → settles at midline value (1.236 for Solar)

Formula: amp_scale = 1 + (φ - 1) × (state / φ)
  where state is the sawtooth value: 0 → φ (acc peak) → 1/φ (release trough)

  At state=0:   amp_scale = 1.0    (neutral start)
  At state=φ:   amp_scale = φ      (full boost, mid×φ = 2.0)
  At state=1/φ: amp_scale = 1+1/φ³ = midline  (the 243AJ champion value!)
...
class EngineMemoryNode(OrigARANode):
    ...
    def cascade_shape(self, t):
        ...
        # ── FIX 5: Gate-modulated amplitude scale ──
        # state (sawtooth): 0 → φ (acc peak) → 1/φ (release trough)
        # amp_scale: 1.0 → φ (peak) → midline (trough)
        # At peak: midline × φ = 2.0. At trough: midline × midline ≈ 1.528.
        amp_scale = 1.0 + (PHI - 1.0) * (state / PHI)
...
print("=" * 78)
print("Script 243AK — Ablation: Midline ONLY")
print("=" * 78)
print()
...
print(f"\n{'=' * 78}")
print("SUMMARY — 243AK Gate-Modulated Amplitude Scale")
print("=" * 78)
print()
