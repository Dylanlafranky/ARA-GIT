#!/usr/bin/env python3
"""
Script 243AT — phi-Offset Gate: Long Periods on Different Rhythm

Based on 243AJ (midline champion, Solar LOO 42.89).

KEY INSIGHT: Long cascade periods (d > 0) see the gate shifted by 1/phi^2
of a full cycle. They accumulate on a DIFFERENT rhythm than Schwabe — not
opposite (that's too violent) but phi-offset.

  Short periods (d <= 0): gate from Gleissberg phase cp
  Long periods  (d > 0):  gate from cp + 1/phi^2 (golden-angle offset)

This means the Gleissberg is in a different part of its accumulate/release
cycle relative to the Schwabe, creating natural energy exchange at the
golden angle rather than in opposition.

  NO new parameters — just a phase offset derived from phi.
...
class EngineMemoryNode(OrigARANode):
    ...
    def cascade_shape(self, t):
        ...
        # ── NORMAL GATE (for short periods, d <= 0) ──
        cp = (gp % TAU) / TAU
        ...
        # ── PHI-OFFSET GATE (for long periods, d > 0) ──
        # Shifted by 1/φ² of a full cycle — the golden angle fraction
        cp_offset = ((gp + TAU * INV_PHI_2) % TAU) / TAU
        if cp_offset < acc: state_off = (cp_offset / acc) * PHI
        else:
            ramp_off = (cp_offset - acc) / (1 - acc)
            state_off = PHI * (1 - ramp_off) + INV_PHI * ramp_off
        gate_offset = state_off / ((PHI + INV_PHI) / 2)
        ...
        for j in range(len(phases)):
            ...
            # Short periods: normal gate. Long periods: φ²-offset gate.
            local_gate = gate_offset if d > 0 else gate
            eps_vals.append(base_eps * local_gate)
        for j in range(len(phases)):
            d = live_d[j]
...
print("=" * 78)
print("Script 243AT — phi-Offset Gate: Long Periods on Different Rhythm")
print("=" * 78)
print()
print("  KEY CHANGE: Long periods (d > 0) gate shifted by 1/phi^2 of cycle")
print("              Short periods (d <= 0) keep normal gate")
print("              Golden-angle offset, not opposition")
print()
...
print(f"\n{'=' * 78}")
print("SUMMARY — 243AT phi-Offset Gate")
print("=" * 78)
print()
