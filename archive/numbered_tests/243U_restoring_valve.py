#!/usr/bin/env python3
"""
Script 243U — Restoring Force Valve (Alignment-Aware)

243S/243T showed: consumers love open valves (EQ=0.44 record),
but engines keep regressing on Solar. The problem: every valve
so far only asks "WHO is the receiver?" — never "WHAT is this
tilt doing to the receiver?"

New approach: the valve checks ALIGNMENT between the gear tilt
and the receiver's deviation from equilibrium.

  deviation = inst_ara - natural_ara   (how far from home)
  alignment = tanh(tilt × deviation × k)

  alignment > 0 → DESTABILIZING (tilt pushes FURTHER from natural)
    → valve dampens (mult < 1)
  alignment < 0 → CORRECTIVE (tilt pushes BACK toward natural)
    → valve amplifies (mult > 1)
  alignment ≈ 0 → neutral (near equilibrium, or tiny tilt)
    → valve passes through (mult ≈ 1)

  valve_mult = 1 - alignment × 1/φ

This means:
  - Engine pushed away from φ during a crash → resists
  - Engine being pulled back toward φ after a crash → welcomes
  - Consumer near equilibrium → passes most tilt through
  - The Dalton minimum recovery tilt gets amplified, not dampened

Three sensitivity variants:
  A — k=φ²   (moderate sensitivity)
  B — k=φ³   (higher sensitivity)
  C — k=φ⁴   (aggressive — snaps quickly)
"""
...
class BreathingGearNode(OrigARANode):
    """
    Triangle rider node with RESTORING FORCE valve.

    The valve checks whether the gear tilt is pushing the receiver
    toward or away from its natural ARA. Corrective tilts get
    amplified, destabilizing tilts get dampened.
    ...
    def __init__(self, name, period, ara, rung, base_amp, archetype,
                 ...
                 gate_send=False, restore_k=PHI_2):
        ...
        self.restore_k = restore_k  # Sensitivity of alignment detection
    ...
    def cascade_shape(self, t):
        ...
        # ── RESTORING FORCE VALVE ──
        #
        # deviation: how far inst_ara is from the node's natural ARA
        #   positive = above natural, negative = below
        #
        # The gear tilt will push inst_ara in some direction.
        # If tilt × deviation > 0: DESTABILIZING (pushing further away)
        # If tilt × deviation < 0: CORRECTIVE (pushing back home)
        #
        # tanh smoothly maps this to [-1, +1]:
        #   +1 = strongly destabilizing → dampen hard
        #   -1 = strongly corrective → amplify
        #    0 = neutral → pass through

        # ── TEMPORAL GEAR TILT (compute raw first, then gate) ──
        gear_tilt_raw = 0.0
        ...
        # ── ALIGNMENT CHECK ──
        deviation = inst_ara - self.ara  # how far from natural state

        # The alignment product: tilt × deviation
        # Positive → destabilizing, Negative → corrective
        alignment_raw = gear_tilt_raw * deviation
        alignment = math.tanh(alignment_raw * self.restore_k)

        # valve_mult: 1 when neutral, <1 when destabilizing, >1 when corrective
        valve_mult = 1.0 - alignment * INV_PHI

        # Clamp: don't let it go negative (no sign reversal)
        valve_mult = max(0.0, valve_mult)

        # Apply restoring valve
...
def run_breathing_gear(times, peaks, period, ara, target_rung,
    ...
    dev_hist = target_node.deviation_history
    tilt_raw = target_node.gear_tilts_raw
    tilt_gated = target_node.gear_tilts_gated

    # Count how often valve was corrective vs destabilizing
    n_corrective = sum(1 for a in align_hist if a < -0.01)
    n_destab = sum(1 for a in align_hist if a > 0.01)
    n_neutral = len(align_hist) - n_corrective - n_destab
    ...
    return {
        ...
        "valve_mult_range": (min(vm_hist), max(vm_hist)) if vm_hist else (0, 0),
        "avg_deviation": np.mean(np.abs(dev_hist)) if dev_hist else 0,
...
print("=" * 78)
print("Script 243U — Restoring Force Valve (Alignment-Aware)")
print("=" * 78)
print()
print("  alignment = tanh(tilt × deviation × k)")
print("  valve_mult = 1 - alignment × 1/φ")
print()
print("  Corrective tilts → amplified (mult > 1)")
print("  Destabilizing tilts → dampened (mult < 1)")
print("  Neutral → pass through (mult ≈ 1)")
print()

ref = {
    '235b':   {"Solar (SSN)": 46.30, "ENSO (ONI)": 0.44, "Sanriku EQ": 1.19},
    '243O-C': {"Solar (SSN)": 48.45, "ENSO (ONI)": 0.41, "Sanriku EQ": 1.07},
    '243T-A': {"Solar (SSN)": 61.00, "ENSO (ONI)": 0.49, "Sanriku EQ": 0.44},
}
...
for var_key in ["A", "B", "C"]:
    ...
    print(f"    child_sign={var['child_sign']}, parent_sign={var['parent_sign']}, "
          f"gate_send={var['gate_send']}, k={var['restore_k']:.4f}")
    print(f"{'─' * 78}")
    print(f"\n  {'System':<16} │ {'235b':>7} │ {'243O-C':>7} │ {'243T-A':>7} │ {'243U':>7} │ "
          f"{'Δ vs 235b':>9} │ {'v_mult':>7} │ {'corr/dest':>10}")
    print(f"  {'─'*16}─┼─{'─'*7}─┼─{'─'*7}─┼─{'─'*7}─┼─{'─'*7}─┼─{'─'*9}─┼─{'─'*7}─┼─{'─'*10}")
    ...
    for name, times, peaks, period, ara, rung in SYSTEMS:
        ...
        b235 = " ✓" if d235 < 0 else ""
        cd = f"{res['n_corrective']}/{res['n_destab']}"
        print(f"\r  {name:<16} │ {ref['235b'][name]:>7.2f} │ {ref['243O-C'][name]:>7.2f} │ "
              f"{ref['243T-A'][name]:>7.2f} │ {res['mae']:>7.2f} │ {d235:>+8.2f}{b235} │ "
              f"{res['avg_valve_mult']:>7.4f} │ {cd:>10}")

    # Valve diagnostics
    print(f"\n  Restoring force diagnostics:")
    for name in var_results:
        r = var_results[name]
        if r["failed"]: continue
        mr = r["valve_mult_range"]
        total = r['n_corrective'] + r['n_destab'] + r['n_neutral']
        pct_corr = r['n_corrective'] / total * 100 if total > 0 else 0
        pct_dest = r['n_destab'] / total * 100 if total > 0 else 0
        print(f"    {name:<16}: avg_align={r['avg_alignment']:>+.4f}, "
              f"avg_mult={r['avg_valve_mult']:.4f}, "
              f"mult_range=[{mr[0]:.4f}, {mr[1]:.4f}], "
              f"avg_dev={r['avg_deviation']:.4f}")
        print(f"    {'':16}  corrective={pct_corr:.0f}%, destab={pct_dest:.0f}%, "
              f"tilt_raw={r['avg_tilt_raw']:.6f}, tilt_gated={r['avg_tilt_gated']:.6f}")
...
print(f"\n{'=' * 78}")
print("SUMMARY — 243U Restoring Force Valve")
print("=" * 78)
print()
print("  The valve now knows WHAT the tilt is doing, not just WHO receives it.")
print("  Corrective tilts (pushing back to natural ARA) get amplified.")
print("  Destabilizing tilts (pushing further away) get dampened.")
print()
print("  Key question: does this selective gating finally help Solar")
print("  while keeping the consumer/clock improvements?")
print()
print(f"  Baselines: 235b Solar=46.30, 243O-C=48.45")
print(f"  Best EQ ever: 243T-A = 0.44")
