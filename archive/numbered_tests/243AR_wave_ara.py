#!/usr/bin/env python3
"""
Script 243AR — Wave-ARA Carry: Conserved/Consumed Energy

Based on 243AJ (champion, Solar LOO 42.89).

KEY INSIGHT (Dylan): The ARA between consecutive peaks tells you whether
the wave conserved or consumed energy. The DIFFERENCE from φ is the carry:

  wave_ara = peak[i-1] / peak[i-2]   (the wave's own growth/decay)
  phi_distance = wave_ara - φ

  If positive → excess energy was conserved → bounces forward (amplifies)
  If negative → energy was consumed → deficit carries (dampens)

This is fundamentally different from inst_ara (= prev_amp / base_amp):
  - inst_ara normalizes against a FITTED parameter (base_amp) → r=0.146 with residual
  - wave_ara normalizes against ACTUAL prior data → directly data-driven

The carry modifies amp_scale asymmetrically:
  Conserved (positive): carry × 1/φ² (moderate bounce forward)
  Consumed (negative):  carry × 1/φ⁴ (weaker — consumed energy harder to recover)
...
class EngineMemoryNode(OrigARANode):
    ...
    def cascade_shape(self, t):
        ...
        # ── FIX 5: Wave-ARA carry — absolute distance from φ-equilibrium ──
        #
        # How far was the previous peak from the system's natural equilibrium?
        # Equilibrium = base_amp × midline (the ARA-shifted center).
        #
        # If previous peak was ABOVE equilibrium → excess energy conserved
        # If previous peak was BELOW equilibrium → energy consumed (deficit)
        #
        # The carry = distance from equilibrium, attenuated asymmetrically:
        #   Above (conserved): × 1/φ²  (excess bounces forward)
        #   Below (consumed):  × 1/φ⁴  (deficit harder to recover)
        #
        # Key difference from inst_ara: this measures absolute LEVEL
        # relative to the ARA-derived equilibrium, not direction of change.
        #
        # C3=264 (far above equil ~170) → big positive carry → amplifies C4
        # C5=82  (far below equil ~170) → negative carry → dampens C6
        #
        wave_carry = 0.0
        if self.prev_amp is not None and self.base_amp > 0:
            equilibrium = self.base_amp * self.midline
            # Fractional distance from equilibrium
            frac_distance = (self.prev_amp - equilibrium) / equilibrium
            # Asymmetric: conserved energy bounces harder than consumed
            if frac_distance > 0:
                wave_carry = frac_distance * INV_PHI_2    # 1/φ² = 0.382
            else:
                wave_carry = frac_distance * INV_PHI_4    # 1/φ⁴ = 0.146

        # Dynamic amp_scale: static base + carry from previous cycle
        dynamic_amp_scale = self.amp_scale + wave_carry

        deviation = shape - self.midline
        shape = self.midline + deviation * dynamic_amp_scale
...
print("=" * 78)
print("Script 243AR — Wave-ARA Carry: Conserved/Consumed Energy")
print("=" * 78)
print()
print("  KEY CHANGE: wave_ara = peak[i-1] / peak[i-2]")
print("              phi_distance = wave_ara - φ")
print("              Conserved (>0): carry × 1/φ² = 0.382")
print("              Consumed  (<0): carry × 1/φ⁴ = 0.146")
print("              amp_scale = midline + wave_carry")
print()

# Show absolute distance carry examples
mid = ara_midline(PHI)
ba_est = 170.0
equil = ba_est * mid
print(f"  Absolute distance carry (equil = base_amp x midline ~ {equil:.0f} SSN):")
print(f"    {'prev_peak':>9} | {'frac_dist':>9} | {'carry':>7} | {'amp_scale':>9}")
print(f"    {chr(9472)*9}-+-{chr(9472)*9}-+-{chr(9472)*7}-+-{chr(9472)*9}")
for prev in [285.0, 264.3, 235.3, 193.0, 175.7, 144.1, 116.4, 82.0, 81.2]:
    fd = (prev - equil) / equil
    if fd > 0:
        carry = fd * INV_PHI_2
    else:
        carry = fd * INV_PHI_4
    amp_s = mid + carry
    print(f"    {prev:>9.1f} | {fd:>+9.3f} | {carry:>+7.4f} | {amp_s:>9.4f}")
print()
...
print(f"\n{'=' * 78}")
print("SUMMARY — 243AR Wave-ARA Carry")
print("=" * 78)
print()
