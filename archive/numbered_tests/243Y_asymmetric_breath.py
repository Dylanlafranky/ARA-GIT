#!/usr/bin/env python3
"""
Script 243Y — Asymmetric Breathing: Fast Inhale, Slow Exhale

Dylan: "When we inhale, we're doing good, but when we exhale, we need
to do it for longer."  and  "Test the current tilt, fire it off at that
angle — the distance it takes to reach 0 is how long it's open."

The insight: real breathing is ASYMMETRIC.
  - INHALE (tilt pushes inst_ara toward home) → sharp, direct, one-step.
    The consumer/clock snaps back quickly.
  - EXHALE (tilt pushes inst_ara away from home) → fired as a momentum
    projectile. The RAW tilt sets the launch angle; it decays each step.
    The valve stays "open" until momentum decays to zero.

Why this might fix the engine problem:
  Engines have large raw tilts but small GATED tilts (receiver_acc ~0.38).
  Currently, that small gated tilt is applied once and gone.
  With momentum: the large raw tilt fires far → decays over many steps →
  each step applies momentum × receiver_acc → small per-step but SUSTAINED.
  Total influence = sum of decaying series ≈ raw_tilt × acc / (1-decay).

  Consumers have small raw tilts → short fire distance → exhale is brief.
  They already work fine with sharp inhale.

Three variants (lean run — top 3 decay rates):
  A — decay = 1/φ² ≈ 0.382  (short exhale, quick ring-down)
  B — decay = 1/φ  ≈ 0.618  (golden exhale, standard)
  C — decay = 1/√φ ≈ 0.786  (long exhale, sustained ring)
"""
...
class AsymBreathNode(OrigARANode):
    """
    Asymmetric breathing node.

    INHALE: tilt pushes inst_ara toward home ARA → apply gated tilt directly.
    EXHALE: tilt pushes inst_ara away from home → fire raw tilt as momentum
            projectile. Decays each step. Valve stays "open" until it reaches 0.

    The RAW tilt magnitude = fire angle = determines how long the exhale lasts.
    Engines fire big raw tilts → long exhale. Consumers fire small → short exhale.
    """

    def __init__(self, name, period, ara, rung, base_amp, archetype,
                 seed_ara=None, rung_offset=0,
                 child_sign=-1, parent_sign=-1,
                 gate_send=True, exhale_decay=INV_PHI):
        ...
        self.exhale_decay = exhale_decay
        ...
        # ── Momentum state (persists across cascade_shape calls) ──
    ...
    def cascade_shape(self, t):
        ...
        # ── TEMPORAL GEAR TILT (raw, before gating) ──
        gear_tilt_raw = 0.0
        ...
        # Determine direction: is the tilt pushing TOWARD or AWAY from home ARA?
        home_ara = self.ara
        deviation = inst_ara - home_ara  # positive = above home, negative = below

        # Tilt pushing toward home: sign(tilt) opposite to sign(deviation)
        # i.e. deviation>0 and tilt<0 (pulling back down) → inhale
        #      deviation<0 and tilt>0 (pulling back up)   → inhale
        # Use small threshold for near-home ambiguity
        if abs(deviation) < 0.01:
            # Near home — treat as inhale (sharp, direct)
            is_inhale = True
        else:
            is_inhale = (gear_tilt_raw * deviation) < 0

        if is_inhale:
            # ── INHALE: sharp, direct, one-step ──
            gear_tilt_gated = gear_tilt_raw * receiver_acc
            inst_ara = max(0.01, min(2.0, inst_ara + gear_tilt_gated))
            self.inhale_count += 1
        else:
            # ── EXHALE: fire the tilt as a momentum projectile ──
            # Raw tilt = launch angle. Don't apply directly.
            # Add to momentum (it'll decay and apply over many steps).
            self.tilt_momentum += gear_tilt_raw
            self.exhale_count += 1
            gear_tilt_gated = 0.0  # nothing applied directly this step

        # ── Apply decaying momentum (every step, regardless of direction) ──
        # The momentum is gated by receiver_acc when applied
        ...
        self.tilt_momentum *= self.exhale_decay

        # Kill negligible momentum to prevent drift
...
def grid_search(period, ara, rung, times, peaks, n_tr=80, n_ba=40,
    ...
    for tr in t_refs:
        for ba in base_amps:
            node = AsymBreathNode("temp", period, ara, rung, ba, "engine",
                                  exhale_decay=exhale_decay)
            node.set_t_ref(tr)
            errors = []
...
def run_asym_breath(times, peaks, period, ara, target_rung,
    ...
    for rung_power, rung_period, prefix, eng_ara, cons_ara, offset in rungs:
        for archetype, ara_val in [("engine", eng_ara), ("clock", 1.0),
            ...
            node = AsymBreathNode(name, rung_period, ara_val,
...
VARIANTS = {
    "A": {
        "label": "Short exhale: decay = 1/φ² ≈ 0.382",
        "exhale_decay": INV_PHI_2,
    },
    "B": {
        "label": "Golden exhale: decay = 1/φ ≈ 0.618",
        "exhale_decay": INV_PHI,
    },
    "C": {
        "label": "Long exhale: decay = 1/√φ ≈ 0.786",
        "exhale_decay": 1.0 / math.sqrt(PHI),
    },
}
...
print("=" * 78)
print("Script 243Y — Asymmetric Breathing: Fast Inhale, Slow Exhale")
print("=" * 78)
print()
print("  INHALE: tilt toward home ARA → sharp, direct, one step.")
print("  EXHALE: tilt away from home → fired as momentum projectile.")
print("          Raw tilt = launch angle. Decays each step.")
print("          Distance to zero = how long the valve stays open.")
print()
print("  Engine (big raw tilt) → fires far → long exhale → sustained influence")
print("  Consumer (small raw tilt) → fires short → brief exhale → snaps back")
print()

# Show theoretical exhale durations at different tilt magnitudes
print("  Theoretical exhale steps to reach 0.001:")
print(f"  {'Tilt':>8} │ {'A (1/φ²)':>8} │ {'B (1/φ)':>8} │ {'C (1/√φ)':>8}")
print(f"  {'─'*8}─┼─{'─'*8}─┼─{'─'*8}─┼─{'─'*8}")
for tilt in [0.001, 0.005, 0.01, 0.02, 0.05]:
    for label, decay in [("A", INV_PHI_2), ("B", INV_PHI), ("C", 1.0/math.sqrt(PHI))]:
        if tilt > 0.001:
            steps = math.log(0.001 / tilt) / math.log(decay)
        else:
            steps = 0
        if label == "A":
            print(f"  {tilt:>8.3f} │ {steps:>8.1f}", end="")
        else:
            print(f" │ {steps:>8.1f}", end="")
    print()
print()
...
ref = {
    ...
    '243W-A': {"Solar (SSN)": 57.66, "ENSO (ONI)": 0.39, "Sanriku EQ": 0.42},
}
...
for var_key in ["A", "B", "C"]:
    ...
    print(f"{'─' * 78}")
    print(f"\n  {'System':<17} │ {'235b':>7} │ {'243O-C':>7} │ {'243W-A':>7} │ "
          f"{'243Y':>7} │ {'Δ vs 235b':>9} │ {'Δ vs O-C':>9}")
    print(f"  {'─'*17}─┼─{'─'*7}─┼─{'─'*7}─┼─{'─'*7}─┼─"
          f"{'─'*7}─┼─{'─'*9}─┼─{'─'*9}")
    ...
    for name, times, peaks, period, ara, rung in SYSTEMS:
        ...
        res = run_asym_breath(
            ...
            system_name=name, exhale_decay=var["exhale_decay"])
        ...
        print(f"\r  {name:<17} │ {ref['235b'][name]:>7.2f} │ {ref['243O-C'][name]:>7.2f} │ "
              f"{ref['243W-A'][name]:>7.2f} │ {res['mae']:>7.2f} │ "
              f"{d235:>+8.2f}{b235} │ {dOC:>+8.2f}{bOC}")
    ...
    for name in var_results:
        ...
        print(f"    {name:<16}: valve={r['avg_valve']:.4f}, "
              f"inhales={r['inhales']}, exhales={r['exhales']} ({pct_exhale:.0f}%), "
...
print(f"\n{'=' * 78}")
print("SUMMARY — 243Y Asymmetric Breathing")
print("=" * 78)
print()
print("  Fast inhale (toward home) — sharp one-step correction.")
print("  Slow exhale (away from home) — momentum projectile, decays over time.")
print("  Raw tilt = launch angle. Distance to zero = open duration.")
print()
print("  Engine fires big raw tilt → rings for many steps → sustained influence.")
print("  Consumer fires small tilt → short ring → snaps back quickly.")
print()
print(f"  Based on 243O-C champion (child_sign=-1, parent_sign=-1, gate_send=True)")
print(f"  Baselines: 235b Solar=46.30, 243O-C Solar=48.45, 243W-A ENSO=0.39/EQ=0.42")
