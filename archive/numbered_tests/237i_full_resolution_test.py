#!/usr/bin/env python3
"""
Script 237i — Full-Resolution Inverse Valve Accuracy Test

Same formula as 237h "A InvStatic" (the universal inverse valve midline),
but at 80×40 grid resolution to match 237d's precision.

Only 3 variants (Baseline, Static 237d, Inverse Valve) to stay within
time budget at high resolution.
...
def midline_inverse_valve(ara):
    """Universal inverse valve midline.

    Engines (ARA >= 1): self-modulate using own ARA
    Consumers (ARA < 1): receive external pressure via 1/ARA
    Clock (ARA = 1): ARA = 1/ARA → midline = 1.0
    """
    if ara >= 1.0:
...
def predict_baseline(times, peaks, t_ref, base_amp, ara, period):
    """226 v4 baseline — no midline shift."""
...
def predict_static_midline(times, peaks, t_ref, base_amp, ara, period):
    """237d static midline — uses ARA directly (engines only)."""
    sys226 = ARASystem("sys", ara, period, times, peaks)
    a = max(0.01, ara)
...
def predict_inverse_valve(times, peaks, t_ref, base_amp, ara, period):
    """237h inverse valve midline — universal."""
    sys226 = ARASystem("sys", ara, period, times, peaks)
    midline = midline_inverse_valve(ara)
    preds = []
    for k in range(len(times)):
        prev = peaks[k-1] if k > 0 else None
        std_pred = sys226.predict_amplitude(
...
print("=" * 78)
print("237i — Full-Resolution Inverse Valve Accuracy Test (80×40 grid)")
print("=" * 78)
print()

# Show midline values
print("MIDLINE VALUES (inverse valve):")
for name, cfg in systems.items():
    a = cfg['ara']
    m_std = _base_midline(a)
    m_inv = midline_inverse_valve(a)
    print(f"  {name:<12} ARA={a:.3f}  standard={m_std:.4f}  "
          f"inverse_valve={m_inv:.4f}")
print()

configs = [
    ("Baseline",      predict_baseline),
    ("Static 237d",   predict_static_midline),
    ("Inverse Valve", predict_inverse_valve),
]
...
for sys_name, cfg in systems.items():
    ...
    print(f"── {sys_name.upper()} (ARA={ara:.3f}, N={len(times)}, "
          f"mean={mean_amp:.2f}, sine={sine_mae:.4f}) ──")
    ...
    for label, fn in configs:
        ...
        if label == 'Baseline':
            marker = "="
        pct = (delta / sys_results.get('Baseline', loo_mae)) * 100 if sys_results.get('Baseline', loo_mae) > 0 else 0
...
print("=" * 78)
print("FINAL SCOREBOARD — 237i Full Resolution")
print("=" * 78)
print()
...
print()
print("─" * 78)
print("UNIVERSAL CHECK: Does inverse valve beat baseline on ALL systems?")
print("─" * 78)
...
print()
print("─" * 78)
print("vs SINE BASELINE (mean-prediction)")
print("─" * 78)
for sys_name, cfg in systems.items():
    peaks = cfg['peaks']
    sine_mae = np.mean(np.abs(peaks - np.mean(peaks)))
    inv_mae = all_loo[sys_name]['Inverse Valve']
    improvement = ((sine_mae - inv_mae) / sine_mae) * 100
    print(f"  {sys_name:<12} sine={sine_mae:.4f}  "
          f"inverse_valve={inv_mae:.4f}  improvement={improvement:+.1f}%")
