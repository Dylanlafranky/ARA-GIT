#!/usr/bin/env python3
"""
Script 243M2 — Pure Palindrome Valve: Only Splits the Pipes

243M had three mechanisms stacking: valve_drain + gear_tilt + geometric ARAs.
Too much for Solar. Strip back to the PURE concept:

The valve ONLY affects the PIPE coupling rates.
  - Proven pipes: down = INV_PHI_4, up = PHI_LEAK * INV_PHI
  - Valve splits these by acc = 1/(1+inst_ARA):
    - down_rate = base_down × (1 - acc)  [engine fraction]
    - up_rate   = base_up × acc           [consumer fraction]
  - No temporal gear. No self-drain. No extra shape modulation.
  - Just: the pipes themselves open and close based on the ARA palindrome.

This means at each rung:
  - High ARA (engine): down pipe WIDER, up pipe NARROWER → feeds below
  - Low ARA (consumer): up pipe WIDER, down pipe NARROWER → drains from above
  - The split is dynamic — changes as inst_ARA changes

Variant A: Valve on pipes only (no temporal gear)
Variant B: Valve on pipes + temporal gear (minimal: child consumes sign)
Variant C: No valve, no gear (pure 235b + triangle rider + geometric ARAs)
"""
...
class PureValveNode(OrigARANode):
    """
    Triangle rider with palindrome valve on PIPES ONLY.
    No extra shape modulation. No self-drain.
    The valve just opens and closes the up/down pipe coupling rates.
    """
    ...
    def __init__(self, name, period, ara, rung, base_amp, archetype,
        ...
        # Track current valve position for pipe modulation
        self.current_valve_acc = 1.0 / (1.0 + ara)
    ...
    def cascade_shape(self, t):
        ...
        # ── Minimal temporal gear (child consumes, from 243k2-A) ──
        if self.use_gear:
            gear_tilt = 0.0
        ...
        # ── Update valve position for pipe modulation ──
        self.current_valve_acc = 1.0 / (1.0 + max(0.01, inst_ara))

        # ── Triangle rider (unchanged) ──
        ...
        # ── Cascade from rider (standard — no extra modulation) ──
...
def grid_search(period, ara, rung, times, peaks, use_gear, n_tr=80, n_ba=40):
    ...
    for tr in t_refs:
        for ba in base_amps:
            node = PureValveNode("temp", period, ara, rung, ba, "engine",
                                  use_gear=use_gear)
            node.set_t_ref(tr)
            errors = []
...
def run_pure_valve(times, peaks, period, ara, target_rung,
    ...
    for rung_power, rung_period, prefix, eng_ara, cons_ara, offset in rungs:
        for archetype, ara_val in [("engine", eng_ara), ("clock", 1.0),
            ...
            node = PureValveNode(name, rung_period, ara_val,
    ...
    # ═══ VALVE-MODULATED PIPES ═══
    # The palindrome valve splits the pipe rates:
    #   down_rate = base_rate × (1 - acc) = base × ARA/(1+ARA)
    #   up_rate = base_rate × acc = base × 1/(1+ARA)
    # But acc is dynamic, so we compute it per-node at build time
    # using the node's STATIC ara. Runtime modulation happens through
    # the node's cascade_shape (inst_ARA changes the valve).
    #
    # For the pipes themselves, we use the static valve as the
    # initial split. The dynamic valve modulates the cascade shape.
    ...
    for i, (rp, prefix, offset) in enumerate(rung_list):
        ...
        # Engine's palindrome valve
        eng_acc = 1.0 / (1.0 + engine.ara)

        # Horizontal pipes (unchanged — these are within a rung)
        ...
        # Vertical DOWN pipe: modulated by (1 - acc) = engine fraction
        if i < len(rung_list) - 1:
            below_prefix = rung_list[i + 1][1]
            down_rate = INV_PHI_4 * (1 - eng_acc)  # Engine pushes down
            engine.add_feed_target(all_nodes[f"{below_prefix}_engine"],
                                   down_rate, "vertical_down")

        # Vertical UP drain: modulated by acc = consumer fraction
        if i > 0:
            above_prefix = rung_list[i - 1][1]
            up_rate = PHI_LEAK * INV_PHI * eng_acc  # Consumer pulls up
            consumer.add_drain(all_nodes[f"{above_prefix}_engine"],
                              rate=up_rate)

    # Temporal gear wiring (if enabled)
    if use_gear:
        for i, (rp, prefix, offset) in enumerate(rung_list):
...
print("=" * 78)
print("Script 243M2 — Pure Palindrome Valve: Only Splits the Pipes")
print("=" * 78)
print()
...
print(f"  {'Best 243':<22} │ {best_243['Solar']:>7.2f} │ {best_243['ENSO']:>6.2f} │ {best_243['EQ']:>6.2f} │")
print(f"  {'─'*22}┼{'─'*9}┼{'─'*8}┼{'─'*8}┼{'─'*30}")

for vname, use_gear in variants:
    print(f"  Running {vname}...", end="", flush=True)

    r_sol = run_pure_valve(solar_t, solar_p, 11.0, PHI, 4, "Solar", use_gear)
    r_enso = run_pure_valve(enso_t, enso_p, 3.7, INV_PHI_2, 2, "ENSO", use_gear)
    r_eq = run_pure_valve(eq_t, eq_p, 25.0, 0.5, 3, "EQ", use_gear)
    ...
    if not use_gear:  # Show per-cycle for the pure valve version
        print()
        for sys_label, sys_r, cycles_or_events in [
            ("Solar", r_sol, SOLAR_CYCLES),
        ]:
            print(f"  {sys_label} per-cycle (valve pipes only):")
            for i, entry in enumerate(cycles_or_events):
                cn, yr, act = entry
                pred = sys_r["preds"][i]
                err = sys_r["errors"][i]
...
# Show valve split values for reference
print()
print("─" * 78)
print("VALVE SPLIT VALUES (static, at build time):")
print("─" * 78)
for label, ara in [("Solar (φ)", PHI), ("ENSO (1/φ²)", INV_PHI_2),
                    ("EQ (0.5)", 0.5), ("Clock (1.0)", 1.0)]:
    acc = 1.0 / (1.0 + ara)
    down = INV_PHI_4 * (1 - acc)
    up = PHI_LEAK * INV_PHI * acc
    print(f"  {label:<15}: acc={acc:.3f}  down_rate={down:.4f}  up_rate={up:.4f}  ratio={down/up if up > 0 else 999:.2f}")
