#!/usr/bin/env python3
"""
Script 243AJ — The Other Half: Amplitude Scale

Based on 243AJ (midline champion, Solar LOO 44.90).

The midline captures HALF the accumulate/snap relationship — where the wave
center sits. The OTHER HALF is the amplitude: how far the wave swings.
...
class EngineMemoryNode(OrigARANode):
    ...
    def cascade_shape(self, t):
        ...
        # ── Breathing valve — FROZEN to static ARA ──
        receiver_acc = 1.0 / (1.0 + max(0.01, self.ara))
        ...
        # ── ABLATION: Freeze valve to static ARA (not inst_ara) ──
        # This removes the valve-inside-gate: gate shape stays constant
        acc = 1.0 / (1.0 + max(0.01, self.ara))  # FROZEN: uses home ARA, not inst_ara
        cp = (gp % TAU) / TAU
...
print("=" * 78)
print("Script 243AU — Ablation: FROZEN VALVE (static ARA in gate)")
print("=" * 78)
print()
print("  ABLATION: acc and receiver_acc use self.ara instead of inst_ara")
print("  This freezes the valve inside the gate — gate shape stays constant")
print("  If champion is better, the valve-inside-gate IS doing work")
print()
...
print(f"\n{'=' * 78}")
print("SUMMARY — 243AU Frozen Valve Ablation")
print("=" * 78)
print()
