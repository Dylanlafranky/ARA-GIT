#!/usr/bin/env python3
"""
Script 223b — Per-Period Pressure Cascade

DYLAN'S HALLWAY ANALOGY:
  Energy moves at φ through a hallway. Explosions keep happening
  at the origin. You're tracking the ball propelled down the hallway.

  There are different-shaped DOORS in the hallway — each cascade
  period [φ¹¹, φ⁹, φ⁶, φ⁴] is a door with its own size.

  As you approach each door:
    - Small opening + lots of force behind = EXPLOSIVE passage
    - Same-size opening = pressure differential escapes, smooth

  The PRESSURE at each door = energy accumulated so far / door size.

  Currently we apply ONE gate (Gleissberg phase) to all 4 periods.
  But each period is at its own phase, so each door has its own size.

  The cascade becomes a PRESSURE CASCADE:
    φ¹¹ door → energy modified → approaches φ⁹ door → modified →
    → φ⁶ door → modified → φ⁴ door → final output

  At each step, pressure = (current_amp / ba) / door_size.
  When pressure > φ (natural balance): explosive amplification.
  When pressure < φ: smooth, damped passage.

  Beside the hallway: another φ-hallway running OPPOSITE (Hale).
  Balls bounce off each other at singularity doors (roof/floor).

  Above: same direction hallway, doors at singularity points.

2 models:
  [1] Per-period gates + pressure scaling
  [2] Per-period gates only (no pressure, just individual sawtooths)
"""
...
# =================================================================
# MODEL 1: PER-PERIOD GATES + PRESSURE
# =================================================================
def mk_pressure_cascade(h_cc=INV_PHI_3):
    """
    Each cascade period gets its OWN sawtooth gate from its own phase.
    Pressure builds through the sequence:
      pressure = (amp/ba) / door_size
    When pressure > φ → explosive (coupling amplified)
    When pressure < φ → smooth (coupling as normal)
    """
    def _p(years, amps, ba, tr):
        preds = []
        for i, t in enumerate(years):
            # Causal gate sets accumulation fraction
            sa = SUN_ACC if i == 0 else ara_to_acc(amps[i-1] / ba)

            # Pressure cascade through each door
            amp = ba
            for per in [PHI**11, PHI**9, PHI**6, PHI**4]:
                ...
                # THIS period's own door size
                door = sawtooth_valve(ph, sa)

                # Pressure: energy behind this door vs door size
                energy_ratio = amp / ba
                pressure = energy_ratio / max(0.1, door)
                pressure_ratio = pressure / PHI

                # Coupling with pressure modification
                # Clamp to prevent runaway: max 2× amplification
                p_mod = min(2.0, pressure_ratio ** INV_PHI)
                eps = INV_PHI_4 * door * p_mod
            ...
            # Inner pulse (below) — the working simple "1"
...
# =================================================================
# MODEL 2: PER-PERIOD GATES ONLY (no pressure)
# =================================================================
def mk_perperiod_gates(h_cc=INV_PHI_3):
    """
    Each cascade period gets its own sawtooth gate, but no pressure
    scaling. Tests whether gate diversification alone helps.
    """
    def _p(years, amps, ba, tr):
        preds = []
        for i, t in enumerate(years):
            ...
            for per in [PHI**11, PHI**9, PHI**6, PHI**4]:
                ...
                # THIS period's own door — sawtooth at its own phase
                door = sawtooth_valve(ph, sa)
                eps = INV_PHI_4 * door
...
if __name__ == '__main__':
    t0 = time.time()
    print("="*70)
    print("SCRIPT 223b — PER-PERIOD PRESSURE CASCADE")
    print("="*70)
    print("""
  Each cascade period [φ¹¹, φ⁹, φ⁶, φ⁴] is a DOOR in the hallway.
  Each door has its own size from its own sawtooth phase.

  Energy travels through in sequence:
    φ¹¹ door → φ⁹ door → φ⁶ door → φ⁴ door

  Pressure at each door = (amp/ba) / door_size
  When pressure > φ: explosive coupling
  When pressure < φ: smooth coupling

  OLD: one Gleissberg gate for all 4 periods
  NEW: each period has its own gate from its own phase
""")

    results = []

    # [1] Per-period gates + pressure
    print("[1/2] Pressure cascade (per-period gates + pressure)...")
    fn1 = mk_pressure_cascade(h_cc=INV_PHI_3)
    r = evaluate("Pressure cascade", fn1)
    results.append(r)
    print(f"  LOO={r['loo']:.2f}, r={r['rr']:+.3f}, s={r['sw']}/7  [{time.time()-t0:.0f}s]")

    # [2] Per-period gates only
    print("\n[2/2] Per-period gates only (no pressure)...")
    fn2 = mk_perperiod_gates(h_cc=INV_PHI_3)
    r = evaluate("Per-period gates", fn2)
    results.append(r)
    print(f"  LOO={r['loo']:.2f}, r={r['rr']:+.3f}, s={r['sw']}/7  [{time.time()-t0:.0f}s]")
