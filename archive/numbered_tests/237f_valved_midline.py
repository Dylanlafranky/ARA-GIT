#!/usr/bin/env python3
"""
Script 237f — Valved ARA Midline (pipe-gated by ARA)

Insight: The vertical pipe isn't the same width for every system.
The ARA IS the valve aperture. A consumer (ARA=0.15) has a nearly-
closed valve — almost no vertical energy flows through to create
a standing offset. An engine (ARA=φ) has a wide-open pipe.

From three-circle architecture:
  - Pipe down = 2φ, pipe up = φ (at engine scale)
  - These scale with ARA — the pipe circumference IS the ARA

The valve gates how much of the geometric midline offset actually
takes effect:

  raw_offset = acc_frac × (ARA - 1)        # geometric offset
  valve = f(ARA)                             # pipe aperture
  midline = 1 + valve × raw_offset           # effective midline

Valve candidates (all from ARA alone, no hardcoded numbers):

  V1: valve = ARA/(1+ARA)           — release fraction
  V2: valve = ARA²/(1+ARA)²         — squared release (sharper cutoff)
  V3: valve = ARA/(1+ARA²)          — quadratic dampening for consumers

All produce:
  - Consumer (0.15): valve ≈ 0.13 → midline ≈ 0.96 (barely moves from 1.0)
  - Clock (1.0):     valve = 0.50 → midline = 1.0 (no offset anyway)
  - Engine (φ):      valve ≈ 0.62 → midline ≈ 1.15
  - Exothermic (2.0): valve ≈ 0.67 → midline ≈ 1.22

Cross-system LOO test on all 4 data sources.
"""
...
def midline_unvalved(ara):
    """Original 237d: no valve."""
...
def midline_v1(ara):
    ...
    The fraction of energy the system PRODUCES (vs consumes).
    This IS the pipe's flow capacity normalized to total system energy.
    Consumer: barely produces → valve nearly closed.
    Engine: produces more than consumes → valve open.
    """
    a = max(0.01, ara)
    acc = 1.0 / (1.0 + a)
    raw_offset = acc * (a - 1.0)
    valve = a / (1.0 + a)  # release fraction
    return 1.0 + valve * raw_offset


def midline_v2(ara):
    """V2: valve = (ARA/(1+ARA))² — squared release.

    Sharper cutoff for consumers. The pipe cross-section scales
    as circumference², so flow capacity goes as valve².
    """
    a = max(0.01, ara)
    acc = 1.0 / (1.0 + a)
    raw_offset = acc * (a - 1.0)
...
def midline_v3(ara):
    """V3: valve = ARA/(1+ARA²) — quadratic dampening.

    ARA² in denominator: consumers are quadratically suppressed,
    but engines aren't boosted as aggressively as V2.
    """
    a = max(0.01, ara)
    acc = 1.0 / (1.0 + a)
    raw_offset = acc * (a - 1.0)
...
# ================================================================
# PREDICTION & LOO (from 237d/237e)
# ================================================================

def predict_226(times, peaks, t_ref, base_amp, ara, period, midline_fn):
    """226 v4 with configurable midline function."""
...
def grid_search(predict_fn, times, peaks, ara, period, ba_lo, ba_hi):
    ...
    best = {'mae': 1e9}
    for tr in np.linspace(tr_lo, tr_hi, 50):
        for ba in np.linspace(ba_lo, ba_hi, 25):
...
def run_loo(predict_fn, times, peaks, ara, period, ba_lo, ba_hi):
    ...
    for hold_idx in range(n):
        ...
        best = {'mae': 1e9}
        for tr in np.linspace(tr_lo_l, tr_hi_l, 50):
            for ba in np.linspace(ba_lo_l, ba_hi_l, 25):
...
# ================================================================
# MIDLINE COMPARISON TABLE
# ================================================================

print("=" * 78)
print("237f — Valved ARA Midline (pipe-gated by ARA)")
print("=" * 78)
print()

midline_fns = {
    'Unvalved': midline_unvalved,
    'V1 release': midline_v1,
    'V2 squared': midline_v2,
}

print("Midline values by system and valve type:")
print(f"  {'System':<12} {'ARA':>6}", end="")
for label in midline_fns:
    print(f"  {label:>12}", end="")
print()
print(f"  {'─'*12} {'─'*6}", end="")
for _ in midline_fns:
    print(f"  {'─'*12}", end="")
print()

for name, cfg in systems.items():
    a = cfg['ara']
    print(f"  {name:<12} {a:>6.3f}", end="")
    for label, fn in midline_fns.items():
        ml = fn(a)
        print(f"  {ml:>12.4f}", end="")
    print()
...
for name, cfg in systems.items():
    ...
    v2 = (a / (1 + a)) ** 2
    v3 = a / (1 + a * a)
    print(f"    {name:<12}: V1={v1:.4f}  V2={v2:.4f}  V3={v3:.4f}")
print()


# ================================================================
# CROSS-SYSTEM LOO TEST
# ================================================================

print("=" * 78)
print("CROSS-SYSTEM LOO TEST")
print("=" * 78)
print()

# For each system, run baseline + all midline variants
all_results = {}
...
for sys_name, cfg in systems.items():
    ...
    print(f"── {sys_name.upper()} (ARA={ara:.3f}, N={len(times)}, "
          f"sine MAE={sine_mae:.4f}) ──")

    # Baseline
    print(f"  Running baseline LOO...")
    loo_base, _, _ = run_loo(predict_226_baseline, times, peaks,
                              ara, period, ba_std_lo, ba_std_hi)
    print(f"    Baseline:    LOO = {loo_base:.4f}")

    results = {'baseline': loo_base, 'sine': sine_mae}

    # Each valve variant
    for label, ml_fn in midline_fns.items():
        print(f"  Running {label} LOO...")
        pred_fn = lambda t, p, tr, ba, a, per, _fn=ml_fn: predict_226(
            t, p, tr, ba, a, per, _fn)
        loo_val, _, _ = run_loo(pred_fn, times, peaks,
                                 ara, period, ba_mid_lo, ba_mid_hi)
        results[label] = loo_val
        delta = loo_base - loo_val
        pct = delta / loo_base * 100
        marker = "✓" if delta > 0.001 else "✗" if delta < -0.001 else "─"
        print(f"    {label:<12}: LOO = {loo_val:.4f}  "
...
print("=" * 78)
print("FINAL SUMMARY — LOO MAE by system and midline variant")
print("=" * 78)
print()

headers = ['Baseline'] + list(midline_fns.keys())
print(f"  {'System':<12} {'ARA':>6} {'Sine':>8}", end="")
for h in headers:
    print(f"  {h:>12}", end="")
print("  Best")
print(f"  {'─'*12} {'─'*6} {'─'*8}", end="")
...
overall_wins = {h: 0 for h in headers}

for sys_name, r in all_results.items():
    ara = systems[sys_name]['ara']
    print(f"  {sys_name:<12} {ara:>6.3f} {r['sine']:>8.4f}", end="")

    vals = {}
    vals['Baseline'] = r['baseline']
    for label in midline_fns:
        vals[label] = r[label]

    best_label = min(vals, key=vals.get)
    overall_wins[best_label] += 1

    for h in headers:
        v = vals[h]
        marker = " ***" if h == best_label else ""
        print(f"  {v:>10.4f}{marker}", end="")
    print(f"  {best_label}")

print()
print("  Win counts:", end="")
for h in headers:
    print(f"  {h}: {overall_wins[h]}", end="")
print()

# Find the variant that wins the most
best_variant = max(overall_wins, key=overall_wins.get)
print(f"\n  Best overall variant: {best_variant}")
print()

# Show improvement vs baseline for best variant
if best_variant != 'Baseline':
    print(f"  {best_variant} vs Baseline:")
    for sys_name, r in all_results.items():
        base = r['baseline']
        best = r[best_variant]
        delta = base - best
        pct = delta / base * 100
        marker = "✓" if delta > 0.001 else "✗" if delta < -0.001 else "─"
        print(f"    {sys_name:<12}: {base:.4f} → {best:.4f}  "
              f"Δ={delta:+.4f} ({pct:+.1f}%) {marker}")
