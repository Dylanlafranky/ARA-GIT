#!/usr/bin/env python3
"""
Script 223k — Phase-Difference + Log Tension

COMBINING THE TWO BEST THREADS:

  Phase-difference collision (223j Model 2): LOO=33.20 (best LOO ever)
    cos(phase_prev - phase_curr) = the ARA of the wave
    Single smooth function. Equal vertex + edge. Balanced geometry.

  Log tension (223g Model 1): r=+0.706 (best Waldmeier ever)
    log1p perpendicular pressure = beeswax always-touching-walls
    Small tensions still matter. Large tensions compressed.

  Together: ARA of the collision + beeswax wall pressure.

3 models:
  [1] Phase-diff + log1p tension
  [2] Phase-diff + φ-log tension (log(1+|t|φ)/log(1+φ))
  [3] Phase-diff + log tension + smooth blend collision
      (what if phase-diff AND smooth blend both contribute?)
"""
...
# =================================================================
# MODEL 1: Phase-diff + log1p tension
# =================================================================
def mk_phasediff_log1p(h_cc=INV_PHI_3):
    """
    Phase-difference collision: cos(Δphase) = ARA of the wave.
    Log1p tension: log(1+|t|)/log(2) = beeswax always-touching-walls.
    """
    LOG2 = np.log(2)
    def _p(years, amps, ba, tr):
        ...
        for i, t in enumerate(years):
            ...
            for j, per in enumerate(PERIODS):
                ...
                # Phase-difference collision (ARA of the wave)
                if j > 0:
...
# =================================================================
# MODEL 2: Phase-diff + φ-log tension
# =================================================================
def mk_phasediff_philog(h_cc=INV_PHI_3):
    """
    Phase-difference collision + φ-scaled log tension.
    log(1 + |t|·φ) / log(1+φ): the log curve shaped by golden ratio.
...
# =================================================================
# MODEL 3: Phase-diff + champion mirror (dual collision) + log tension
# =================================================================
def mk_dual_collision_log(h_cc=INV_PHI_3):
    """
    What if BOTH collision types contribute?

    Phase-difference: cos(Δphase) = smooth ARA of the wave
    Champion mirror: -cos_prev × cos_curr = vertex-weighted

    These are related: cos(Δphase) = cos·cos + sin·sin
    Champion is just the cos·cos part.

    Blend both: champion at 1/φ + the sin·sin part at 1/φ
    (equal weight, balanced beeswax geometry)
    + log1p tension for beeswax walls

    Equivalently: phase-diff collision at 1/φ (same as Model 1).
    But let's test: champion + sin·sin edge addition.
    """
    LOG2 = np.log(2)
    def _p(years, amps, ba, tr):
        preds = []
        for i, t in enumerate(years):
            ...
            for j, per in enumerate(PERIODS):
                ...
                if j > 0:
                    # Champion vertex collision
                    vertex = -cos_vals[j-1] * cos_vals[j]
                    # Edge collision (the missing sin·sin part)
                    edge = -sin_vals[j-1] * sin_vals[j]

                    # Both at equal weight (balanced beeswax)
                    # This is equivalent to -cos(Δphase) but written
                    # as explicit components so we can see each part
                    collision = vertex + edge
...
if __name__ == '__main__':
    t0 = time.time()
    print("="*70)
    print("SCRIPT 223k — PHASE-DIFFERENCE + LOG TENSION")
    print("="*70)
    print(f"""
  Phase-diff: cos(Δphase) = ARA of the wave (LOO=33.20 thread)
  Log tension: log1p perpendicular = beeswax walls (r=+0.706 thread)

  [1] Phase-diff + log1p tension
  [2] Phase-diff + φ-log tension
  [3] Dual collision (vertex + edge explicit) + log1p tension
""")

    results = []

    # [1] Phase-diff + log1p
    print("[1/3] Phase-diff + log1p tension...")
    fn1 = mk_phasediff_log1p(h_cc=INV_PHI_3)
    r = evaluate("PhaseDiff+log1p", fn1)
    results.append(r)
    print(f"  LOO={r['loo']:.2f}, r={r['rr']:+.3f}, s={r['sw']}/7  [{time.time()-t0:.0f}s]")

    # [2] Phase-diff + φ-log
    print("\n[2/3] Phase-diff + φ-log tension...")
    fn2 = mk_phasediff_philog(h_cc=INV_PHI_3)
    r = evaluate("PhaseDiff+φlog", fn2)
    results.append(r)
    print(f"  LOO={r['loo']:.2f}, r={r['rr']:+.3f}, s={r['sw']}/7  [{time.time()-t0:.0f}s]")

    # [3] Dual collision + log1p
    print("\n[3/3] Dual collision + log1p tension...")
    fn3 = mk_dual_collision_log(h_cc=INV_PHI_3)
    r = evaluate("Dual+log1p", fn3)
    results.append(r)
    print(f"  LOO={r['loo']:.2f}, r={r['rr']:+.3f}, s={r['sw']}/7  [{time.time()-t0:.0f}s]")
    ...
    print("SCOREBOARD")
    print(f"{'='*70}")
    print(f"  223d Mirror collision:  LOO=33.25, 5/7, r=+0.702  (prev champion)")
