#!/usr/bin/env python3
"""
Script 243AV — Reverse Meta Triangle with Log Singularity Approach

Based on 243AJ (midline champion, Solar LOO 42.89).

KEY INSIGHT: The main triangle has phi at the apex and singularity at the base.
A REVERSE meta triangle has its apex AT the base singularity and reaches up.
Two triangles sharing a base = diamond geometry.

  Main triangle:    Gate on Gleissberg timescale, valve = acc = 1/(1+inst_ara)
                    Accumulates toward phi, releases from phi

  Reverse triangle: Gate on Schwabe timescale, valve = snap = inst_ara/(1+inst_ara)
                    Accumulates toward singularity, releases from singularity
                    Log-scaled: strengthens as inst_ara moves away from home ARA
                    Its own log reaches for the base singularity

  The reverse gate does NOT replace the main gate. It adds a second modulation
  layer that captures energy flowing toward the singularity — the energy the
  main triangle can't see because it's pointed the other way.

  NO new fitted parameters. The reverse gate uses the inverted valve position
  and the same phi-constants.
"""
...
class EngineMemoryNode(OrigARANode):
    ...
    def cascade_shape(self, t):
        ...
        # ── REVERSE META TRIANGLE ──
        # Second gate on Schwabe timescale, inverted valve (snap_frac)
        # Log approach: strengthens as inst_ara drifts from home ARA
        snap_frac = inst_ara / (1.0 + inst_ara)  # inverted valve: 1 - acc
        rev_cp = cp_schwabe  # Schwabe cycle position
        if rev_cp < snap_frac:
            rev_state = (rev_cp / snap_frac) * INV_PHI  # ramp toward 1/φ (singularity)
        else:
            rev_ramp = (rev_cp - snap_frac) / (1 - snap_frac + 1e-10)
            rev_state = INV_PHI * (1 - rev_ramp) + PHI * rev_ramp  # release back to φ
        rev_gate = rev_state / ((PHI + INV_PHI) / 2)

        # Log singularity reach: how far inst_ara is from home ARA
        # log(1 + |deviation|) — logarithmic approach, gets harder near singularity
        ara_dev = abs(inst_ara - self.ara)
        log_reach = math.log1p(ara_dev * PHI) * INV_PHI  # φ-scaled log

        # Reverse gate modulates shape: whisper-strength, log-gated
        # Uses the Schwabe cosine as the carrier wave
        reverse_mod = rev_gate * log_reach * INV_PHI_4 * math.cos(sp)
        shape *= (1.0 + reverse_mod)
...
print("=" * 78)
print("Script 243AV — Reverse Meta Triangle + Log Singularity Approach")
print("=" * 78)
print()
print("  Main triangle:    Gleissberg gate, valve = acc = 1/(1+inst_ara)")
print("  Reverse triangle: Schwabe gate,   valve = snap = inst_ara/(1+inst_ara)")
print("  Log reach: log(1 + |inst_ara - home_ara| * phi) * 1/phi")
print("  Modulation: rev_gate * log_reach * 1/phi^4 * cos(Schwabe)")
print("  NO new fitted parameters")
print()
...
print(f"\n{'=' * 78}")
print("SUMMARY — 243AV Reverse Meta Triangle")
print("=" * 78)
print()
