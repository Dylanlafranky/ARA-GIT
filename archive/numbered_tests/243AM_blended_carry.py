#!/usr/bin/env python3
"""
Script 243AM — Blended Carry: Static Base + φ-Attenuated Bounce

Based on 243AJ (static midline amp_scale, Solar LOO 42.89) and
243AL (pure dynamic carry, Solar LOO 46.95 — too noisy).

The ball bounces, but each bounce loses energy at 1/φ² rate.
Most of the amplitude comes from the proven static midline.
The carry-forward from inst_ara adds only a small correction.

  amp_scale = midline + (ara_midline(inst_ara) - midline) × 1/φ²

When inst_ara = ARA: correction = 0, amp_scale = midline (same as champion)
When inst_ara deviates: small 1/φ² weighted correction from carry energy

The energy of the previous bounce determines the amplitude of the next one.
...
class EngineMemoryNode(OrigARANode):
    ...
    def __init__(self, name, period, ara, rung, base_amp, archetype,
        ...
        # FIX 5: Amplitude scale — carry-forward "bouncing ball"
        # Dynamic: uses inst_ara from previous cycle, not static ARA
        # (set in cascade_shape, default to midline for first cycle)
    ...
    def cascade_shape(self, t):
        ...
        # ── FIX 5: Blended carry — static base + φ-attenuated bounce ──
        # Static midline is the proven champion (42.89).
        # The carry from inst_ara adds a small 1/φ² correction —
        # like a bouncing ball losing energy each hop.
        dynamic_midline = ara_midline(inst_ara)
        carry_correction = (dynamic_midline - self.midline) * INV_PHI * INV_PHI
        amp_scale = self.midline + carry_correction
        deviation = shape - self.midline
        shape = self.midline + deviation * amp_scale
...
print("=" * 78)
print("Script 243AM — Blended Carry (Static + 1/φ² Bounce)")
print("=" * 78)
print()
...
print(f"  {'ARA':>6} │ {'Role':>10} │ {'Momentum':>8} │ {'Midline':>8} │ {'Amp@ARA':>8} │ {'Amp@1.5x':>8} │ {'Amp@0.5x':>8}")
print(f"  {'─'*6}─┼─{'─'*10}─┼─{'─'*8}─┼─{'─'*8}─┼─{'─'*8}─┼─{'─'*8}─┼─{'─'*8}")
for ara, role in [(0.15, "deep cons"), (INV_PHI, "consumer"), (1.0, "clock"),
                  (PHI, "ENGINE"), (2.0, "exotherm")]:
    mid = ara_midline(ara)
    # Show blended amp at inst_ara = ARA, 1.5×ARA, 0.5×ARA
    for inst_mult, label in [(1.0, None), (1.5, None), (0.5, None)]:
        pass
    dyn_at_ara = ara_midline(ara)
    dyn_at_hi = ara_midline(min(2.0, ara * 1.5))
    dyn_at_lo = ara_midline(max(0.01, ara * 0.5))
    amp_at_ara = mid + (dyn_at_ara - mid) * INV_PHI * INV_PHI
    amp_at_hi = mid + (dyn_at_hi - mid) * INV_PHI * INV_PHI
    amp_at_lo = mid + (dyn_at_lo - mid) * INV_PHI * INV_PHI
    print(f"  {ara:>6.3f} │ {role:>10} │ {ara_momentum(ara):>8.4f} │ "
          f"{mid:>8.4f} │ {amp_at_ara:>8.4f} │ {amp_at_hi:>8.4f} │ {amp_at_lo:>8.4f}")
print()
...
print(f"\n{'=' * 78}")
print("SUMMARY — 243AL Carry-Forward Amplitude (Bouncing Ball)")
print("=" * 78)
print()
