#!/usr/bin/env python3
"""
Script 243T — Directional Valve (Inverse Exponential Curve)

243S showed dynamic alpha helps consumers/clocks but doesn't fix engines.
The problem: acc^alpha is ALWAYS a scalar gate (0-to-1). It can only
REDUCE the tilt. Engines need something fundamentally different.

New idea: the valve output is SIGNED based on triangle position.
  - Engine (ARA > 1, time-heavy) → valve goes NEGATIVE → resists tilt
  - Consumer (ARA < 1, space-heavy) → valve goes POSITIVE → amplifies tilt
  - Clock (ARA = 1) → neutral pivot (no bias)

The curve is exponential: gentle near clock, aggressive at extremes.
  valve_curve = exp(-k * (ARA - 1)) - 1

For engine (ARA=φ): exp(-k×0.618) - 1 < 0 → dampens
For consumer (ARA=1/φ): exp(-k×(-0.382)) - 1 = exp(k×0.382) - 1 > 0 → amplifies
For clock (ARA=1): exp(0) - 1 = 0 → neutral

The tilt then becomes: gear_tilt_raw × (1 + valve_curve × coupling)

Three steepness variants:
  A — k=φ   (natural steepness)
  B — k=2   (steeper — space vertex)
  C — k=φ²  (aggressive)
"""
...
class BreathingGearNode(OrigARANode):
    """
    Triangle rider node with DIRECTIONAL valve-gated temporal gear.

    The valve is no longer a scalar gate (0-to-1). It's a signed
    exponential curve centered on the clock (ARA=1):
      valve_curve = exp(-k × (ARA - 1)) - 1

    Engine (ARA>1): negative → dampens gear tilt (resists change)
    Consumer (ARA<1): positive → amplifies gear tilt (absorbs freely)
    Clock (ARA=1): zero → neutral pivot

    The tilt becomes: gear_tilt_raw × (1 + valve_curve × coupling)
    """

    def __init__(self, name, period, ara, rung, base_amp, archetype,
                 seed_ara=None, rung_offset=0,
                 child_sign=1, parent_sign=-1,
                 gate_send=False, valve_steepness=PHI):
        ...
        self.valve_steepness = valve_steepness  # k in exp(-k × (ARA-1))
    ...
    def cascade_shape(self, t):
        ...
        # ── DIRECTIONAL VALVE (inverse exponential curve) ──
        #
        # The curve: exp(-k × (ARA - 1)) - 1
        #   ARA=φ (engine):  exp(-k×0.618) - 1  → negative (dampen)
        #   ARA=1 (clock):   exp(0) - 1 = 0     → neutral
        #   ARA=1/φ (cons):  exp(k×0.382) - 1   → positive (amplify)
        #
        # "If closer to space, it goes positive for time,
        #  if closer to time, goes negative"
        #
        # The coupling factor scales how much the curve affects the tilt.
        # Using INV_PHI as coupling keeps it in the φ family.

        k = self.valve_steepness
        ara_offset = inst_ara - 1.0
        valve_curve = math.exp(-k * ara_offset) - 1.0

        # The directional multiplier on gear tilt:
        # 1 + valve_curve × coupling
        # This means:
        #   engine:   1 + (negative × INV_PHI) → < 1 → dampened tilt
        #   consumer: 1 + (positive × INV_PHI) → > 1 → amplified tilt
        #   clock:    1 + 0 = 1 → unchanged
        coupling = INV_PHI
        valve_mult = 1.0 + valve_curve * coupling

        # Clamp to prevent sign reversal (don't invert tilt, just scale)
        ...
        # Apply directional valve to tilt
        ...
        self.valve_curve_history.append(valve_curve)
    ...
    for tr in t_refs:
        for ba in base_amps:
            node = BreathingGearNode("temp", period, ara, rung, ba, "engine",
                                      ...
                                      gate_send=gate_send,
                                      valve_steepness=valve_steepness)
            node.set_t_ref(tr)
            errors = []
...
def run_breathing_gear(times, peaks, period, ara, target_rung,
    ...
    fit_tr, fit_ba, fit_mae = grid_search(
        ...
        gate_send=gate_send, valve_steepness=valve_steepness)
    ...
    vc_hist = target_node.valve_curve_history
    ...
    return {
        ...
        "avg_valve_curve": np.mean(vc_hist) if vc_hist else 0,
...
# ════════════════════════════════════════════════════════════════
# SHOW VALVE CURVE VALUES
# ════════════════════════════════════════════════════════════════

print("=" * 78)
print("Script 243T — Directional Valve (Inverse Exponential Curve)")
print("=" * 78)
print()
print("Valve curve: exp(-k × (ARA - 1)) - 1")
print("Tilt multiplier: 1 + curve × 1/φ")
print()

# Show what each system's engine sees
print("  Valve curve values at different ARA levels:")
print(f"  {'ARA':>6} │ {'Role':>10} │ {'k=φ':>8} │ {'k=2':>8} │ {'k=φ²':>8} │ {'mult(φ)':>8} │ {'mult(2)':>8} │ {'mult(φ²)':>8}")
print(f"  {'─'*6}─┼─{'─'*10}─┼─{'─'*8}─┼─{'─'*8}─┼─{'─'*8}─┼─{'─'*8}─┼─{'─'*8}─┼─{'─'*8}")
for ara, role in [(0.15, "deep cons"), (INV_PHI, "consumer"),
                   (1.0, "clock"), (PHI, "engine"), (2.0, "exotherm")]:
    offset = ara - 1.0
    curves = [math.exp(-k * offset) - 1.0 for k in [PHI, 2.0, PHI_2]]
    mults = [1.0 + c * INV_PHI for c in curves]
    print(f"  {ara:>6.3f} │ {role:>10} │ {curves[0]:>+8.4f} │ {curves[1]:>+8.4f} │ "
          f"{curves[2]:>+8.4f} │ {mults[0]:>8.4f} │ {mults[1]:>8.4f} │ {mults[2]:>8.4f}")
...
VARIANTS = {
    "A": {
        "label": "Natural steepness: k=φ",
        "child_sign": -1, "parent_sign": -1, "gate_send": True,
        "valve_steepness": PHI,
    },
    "B": {
        "label": "Steeper: k=2 (space vertex)",
        "child_sign": -1, "parent_sign": -1, "gate_send": True,
        "valve_steepness": 2.0,
    },
    "C": {
...
ref = {
    ...
    '243S-B': {"Solar (SSN)": 51.28, "ENSO (ONI)": 0.46, "Sanriku EQ": 0.54},
}
...
for var_key in ["A", "B", "C"]:
    ...
    print(f"    child_sign={var['child_sign']}, parent_sign={var['parent_sign']}, "
          f"gate_send={var['gate_send']}, k={var['valve_steepness']:.4f}")
    print(f"{'─' * 78}")
    print(f"\n  {'System':<16} │ {'235b':>7} │ {'243O-C':>7} │ {'243S-B':>7} │ {'243T':>7} │ "
          f"{'Δ vs 235b':>9} │ {'valve_m':>8} │ {'mult range':>14}")
    print(f"  {'─'*16}─┼─{'─'*7}─┼─{'─'*7}─┼─{'─'*7}─┼─{'─'*7}─┼─{'─'*9}─┼─{'─'*8}─┼─{'─'*14}")
    ...
    for name, times, peaks, period, ara, rung in SYSTEMS:
        ...
        res = run_breathing_gear(
            ...
            valve_steepness=var["valve_steepness"])
        ...
        b235 = " ✓" if d235 < 0 else ""
        mr = res['valve_mult_range']
        print(f"\r  {name:<16} │ {ref['235b'][name]:>7.2f} │ {ref['243O-C'][name]:>7.2f} │ "
              f"{ref['243S-B'][name]:>7.2f} │ {res['mae']:>7.2f} │ {d235:>+8.2f}{b235} │ "
              f"{res['avg_valve_mult']:>8.4f} │ [{mr[0]:.3f},{mr[1]:.3f}]")
    ...
    for name in var_results:
        ...
        print(f"    {name:<16}: avg_curve={r['avg_valve_curve']:>+.4f}, "
              f"avg_mult={r['avg_valve_mult']:.4f}, "
              f"mult_range=[{mr[0]:.4f}, {mr[1]:.4f}], "
              f"tilt_raw={r['avg_tilt_raw']:.6f}, "
...
print(f"\n{'=' * 78}")
print("SUMMARY — 243T Directional Valve")
print("=" * 78)
print()
print("  The valve is now SIGNED, not just a scalar gate.")
print("  exp(-k×(ARA-1))-1: engines dampened, consumers amplified.")
print("  Tilt × (1 + curve × 1/φ) → directional asymmetry.")
print()
print("  Key question: does dampening engines recover Solar")
print("  while keeping the consumer/clock gains?")
print()
print(f"  Baselines: 235b Solar=46.30, 243O-C=48.45, 243S-B=51.28")
print(f"  Best EQ ever: 243S-B = 0.54")
