#!/usr/bin/env python3
"""
Script 223o — Best Combinations (Horizontal × Vertical)

Three distinct improvements, each capturing something different:
  1. Phase-diff collision (LOO=33.20) — smoothest horizontal, ARA of wave
  2. Log tension (r=+0.706) — beeswax wall contact
  3. Asymmetric Hale (r=+0.708) — vertical grief, child dying hits harder

Phase-diff + log tension cancelled out (223k: LOO=33.65).
Mirror + asym Hale worked (223n: LOO=33.51, r=+0.708).

Untested combinations:
  [1] Phase-diff + asymmetric Hale
      (best LOO horizontal + best vertical)
  [2] Champion mirror + log tension + asymmetric Hale
      (beeswax walls + vertical grief on champion base)
  [3] Phase-diff + log tension + asymmetric Hale
      (all three threads — might cancel or might compound)
"""
...
# =================================================================
# MODEL 1: Phase-diff + asymmetric Hale
# =================================================================
def mk_phasediff_asymhale(h_cc=INV_PHI_3):
    """
    Horizontal: phase-difference collision (LOO=33.20 champion)
    Vertical: asymmetric Hale (weak prev cycle = grief × φ)
    """
...
# =================================================================
# MODEL 2: Champion mirror + log tension + asymmetric Hale
# =================================================================
def mk_mirror_log_asymhale(h_cc=INV_PHI_3):
    """
    Horizontal: champion mirror + log1p tension (223g approach)
    Vertical: asymmetric Hale (223n approach)

    Beeswax walls (log tension) + vertical grief (asym Hale)
    on the proven champion mirror base.
    """
    def _p(years, amps, ba, tr):
        preds = []
        for i, t in enumerate(years):
...
# =================================================================
# MODEL 3: Phase-diff + log tension + asymmetric Hale
# =================================================================
def mk_all_three(h_cc=INV_PHI_3):
    """
    All three threads combined:
    Horizontal: phase-diff collision + log1p tension
    Vertical: asymmetric Hale

    This might cancel (223k showed phase-diff + log tension = 33.65)
    or the asymmetric Hale might unlock the combination.
    """
    def _p(years, amps, ba, tr):
        preds = []
        for i, t in enumerate(years):
...
if __name__ == '__main__':
    t0 = time.time()
    print("="*70)
    print("SCRIPT 223o — BEST COMBINATIONS")
    print("="*70)
    print(f"""
  Three threads, three untested combinations:
  [1] Phase-diff (LOO=33.20) + asymmetric Hale (r=+0.708)
  [2] Mirror + log tension (r=+0.706) + asymmetric Hale (r=+0.708)
  [3] Phase-diff + log tension + asymmetric Hale (all three)
""")

    results = []

    # [1] Phase-diff + asymmetric Hale
    print("[1/3] Phase-diff + asymmetric Hale...")
    fn1 = mk_phasediff_asymhale(h_cc=INV_PHI_3)
    r = evaluate("PD+asymHale", fn1)
    results.append(r)
    print(f"  LOO={r['loo']:.2f}, r={r['rr']:+.3f}, s={r['sw']}/7  [{time.time()-t0:.0f}s]")

    # [2] Mirror + log tension + asymmetric Hale
    print("\n[2/3] Mirror + log tension + asymmetric Hale...")
    fn2 = mk_mirror_log_asymhale(h_cc=INV_PHI_3)
    r = evaluate("Mirr+log+aH", fn2)
    results.append(r)
    print(f"  LOO={r['loo']:.2f}, r={r['rr']:+.3f}, s={r['sw']}/7  [{time.time()-t0:.0f}s]")

    # [3] All three
    print("\n[3/3] Phase-diff + log tension + asymmetric Hale...")
    fn3 = mk_all_three(h_cc=INV_PHI_3)
    r = evaluate("All three", fn3)
    results.append(r)
    print(f"  LOO={r['loo']:.2f}, r={r['rr']:+.3f}, s={r['sw']}/7  [{time.time()-t0:.0f}s]")
