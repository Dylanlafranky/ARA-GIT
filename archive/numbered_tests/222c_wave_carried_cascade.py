#!/usr/bin/env python3
"""
Script 222c — Below Cascade Carried by Schwabe Wave

DYLAN'S INSIGHT:
  We don't know how far below the inner system actually is on the log.
  It could be close to breaking over. Applying a flat exp(-φ) assumes
  a full φ-distance, but the WAVE tells us the truth.

  The Schwabe frequency IS the resonant channel between levels.
  cos(schwabe) encodes:
    - WHEN energy arrives (phase)
    - HOW MUCH was converted (amplitude modulation)
    - The actual coupling distance (period)

  So the below contribution is:
    cascade_deviation × cos(schwabe) × [optional decay]

  The cascade (inverted gate) tells us how much the inner system HAS.
  The wave tells us how much actually CROSSES.

  This replaces the old inner pulse:
    Old: ba × 1/φ³ × exp(-φ×pos) × cos(schwabe)
         [assumed flat "1" for inner state]
    New: ba × 1/φ³ × below_deviation × exp(-φ×pos) × cos(schwabe)
         [below_deviation from actual cascade with inverted gate]

2 models:
  [1] Below cascade × decay(φ) × cos(schwabe) — full mechanism
  [2] Below cascade × cos(schwabe) — wave only, no decay
"""
...
# PREDICTION MODEL
# =================================================================
def mk_model(h_cc=INV_PHI_3, below_cc=INV_PHI_3, use_decay=True):
    """
    Three-axis crossing:

    ABOVE CASCADE (main engine):
      [φ¹¹, φ⁹, φ⁶, φ⁴] multiplicative with Sun's causal gate.
      + Gleissberg additive.

    BELOW CASCADE (wave-carried):
      Same cascade geometry, INVERTED gate.
      The cascade deviation × cos(schwabe) × [decay].
      The wave IS the channel — it carries the cascade's energy
      across the level boundary and encodes the actual distance.
    ...
    def _p(years, amps, ba, tr):
        ...
        for i, t in enumerate(years):
            ...
            gate_sun = sawtooth_valve(gp, sa)
            amp = evaluate_cascade(t, ba, tr, gate_sun)

            # Gleissberg additive
            amp += ba * INV_PHI_9 * np.cos(gp)

            # === BELOW CASCADE (wave-carried) ===
            # Inner system: same cascade geometry, inverted ARA state
            inner_acc = 1.0 - sa
            gate_inner = sawtooth_valve(gp, inner_acc)
            below_cascade = evaluate_cascade(t, ba, tr, gate_inner)
            below_dev = (below_cascade - ba) / ba  # fractional deviation

            # The Schwabe wave carries the below energy across
            sp = 2*np.pi*(t-tr)/SCHWABE
            if use_decay:
                # Wave + singularity decay at ARA rate
                below_contrib = ba * below_cc * below_dev * sing_pulse_decay(sp, PHI) * np.cos(sp)
            else:
                # Wave only — no decay
                below_contrib = ba * below_cc * below_dev * np.cos(sp)
...
if __name__ == '__main__':
    t0 = time.time()
    print("="*70)
    print("SCRIPT 222c — BELOW CASCADE CARRIED BY SCHWABE WAVE")
    print("="*70)
    print("""
  Below system: same cascade, inverted gate.
  The Schwabe wave CARRIES the cascade energy across levels.
  cos(schwabe) encodes the actual coupling distance + timing.

  Old inner pulse: ba × 1/φ³ × decay × cos(schwabe)  [flat "1"]
  New:             ba × 1/φ³ × below_dev × decay × cos(schwabe)
""")

    results = []

    # [1] Below cascade × decay × cos(schwabe)
    print("[1/2] Below cascade + decay + wave...")
    fn1 = mk_model(h_cc=INV_PHI_3, below_cc=INV_PHI_3, use_decay=True)
    r = evaluate("Cascade+decay+wave", fn1)
    results.append(r)
    print(f"  LOO={r['loo']:.2f}, r={r['rr']:+.3f}, s={r['sw']}/7  [{time.time()-t0:.0f}s]")

    # [2] Below cascade × cos(schwabe) — no decay
    print("\n[2/2] Below cascade + wave (no decay)...")
    fn2 = mk_model(h_cc=INV_PHI_3, below_cc=INV_PHI_3, use_decay=False)
    r = evaluate("Cascade+wave only", fn2)
    results.append(r)
    print(f"  LOO={r['loo']:.2f}, r={r['rr']:+.3f}, s={r['sw']}/7  [{time.time()-t0:.0f}s]")
    ...
    print(f"{'='*70}")
    print(f"\n  {'Cyc':>3s} {'Obs':>7s} {'Pred':>7s} {'Err':>7s} {'BelDev':>7s} {'BelCtb':>8s}")
    ba_fit, tr_fit = bn['ba'], bn['tr']
    for i, c in enumerate(cycle_nums):
        t = peak_years[i]
        sa = SUN_ACC if i == 0 else ara_to_acc(peak_amps[i-1] / ba_fit)
        inner_acc = 1.0 - sa
        gp = 2*np.pi*(t-tr_fit)/GLEISSBERG
        gate_inner = sawtooth_valve(gp, inner_acc)
        below_c = evaluate_cascade(t, ba_fit, tr_fit, gate_inner)
        below_dev = (below_c - ba_fit) / ba_fit
        sp = 2*np.pi*(t-tr_fit)/SCHWABE
        below_ctb = ba_fit * INV_PHI_3 * below_dev * sing_pulse_decay(sp, PHI) * np.cos(sp)
        p = bn['pf'][i]
        print(f"  {c:3d} {peak_amps[i]:7.1f} {p:7.1f} {p-peak_amps[i]:+7.1f} "
              f"{below_dev:+7.3f} {below_ctb:+8.1f}")

    # Compare: what was the OLD inner pulse contributing?
    print(f"\n{'='*70}")
    print(f"COMPARISON: Old inner pulse (flat '1') vs new (cascade deviation)")
    print(f"{'='*70}")
    print(f"\n  {'Cyc':>3s} {'OldPulse':>9s} {'NewPulse':>9s} {'Diff':>7s} {'BelDev':>7s}")
    for i, c in enumerate(cycle_nums):
        t = peak_years[i]
        sa = SUN_ACC if i == 0 else ara_to_acc(peak_amps[i-1] / ba_fit)
        inner_acc = 1.0 - sa
        ...
        below_dev = (below_c - ba_fit) / ba_fit
        sp = 2*np.pi*(t-tr_fit)/SCHWABE
        decay_wave = sing_pulse_decay(sp, PHI) * np.cos(sp)
        old_pulse = ba_fit * INV_PHI_3 * 1.0 * decay_wave  # flat "1"
        new_pulse = ba_fit * INV_PHI_3 * below_dev * decay_wave
        print(f"  {c:3d} {old_pulse:+9.1f} {new_pulse:+9.1f} {new_pulse-old_pulse:+7.1f} {below_dev:+7.3f}")
