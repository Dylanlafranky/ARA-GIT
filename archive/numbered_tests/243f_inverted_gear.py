#!/usr/bin/env python3
"""
Script 243f — Inverted Gear: Upward Propulsion FULL, Downward Cascade DECAYED

243e had it backwards. Dylan said: "they accumulate and propel it UP!"

The UPWARD consumer return is the POWERFUL mechanism — full strength.
The DOWNWARD engine cascade DECAYS with distance — it fades.

  Engine cascade DOWN: decays 1/φ per step (influence fades)
  Consumer propulsion UP: FULL STRENGTH (this is the gear!)
  Horizontal mesh: FULL (engine→consumer coupling at each rung)

The consumer isn't a passive drain — it's the return drive shaft.
It catches energy from above, accumulates, and launches it back up.
...
class InvertedGearNode(OrigARANode):
    ...
    def cascade_shape(self, t):
        ...
        # ══ INVERTED GEAR WHISPER ══
        # Consumer partners (upward propulsion): FULL, anti-phase
        # Engine partners (downward cascade): DECAYED, in-phase
        if self.field_partners:
            field_sum = 0.0
            for partner_period, coupling, direction in self.field_partners:
                partner_phase = TAU * (t - self.t_ref) / partner_period
                partner_cos = math.cos(partner_phase)
                if direction == 'horizontal':
                    field_sum -= partner_cos * coupling
                elif direction == 'vertical_down':
                    # Engine cascade DOWN: decayed
                    field_sum += partner_cos * coupling * INV_PHI
                elif direction == 'vertical_up':
                    # Consumer propulsion UP: FULL POWER
                    field_sum += partner_cos * coupling * PHI
...
def build_rung_ladder_inv(target_rung, target_period, seed_ara):
    mirror_floor = 0.15
...
def grid_search_inv(period, ara, rung, times, peaks, n_tr=80, n_ba=40):
    ...
    for tr in t_refs:
        for ba in base_amps:
            node = InvertedGearNode("temp", period, ara, rung, ba, "engine")
            node.set_t_ref(tr)
            errors = []
...
def run_inverted_gear(times, peaks, period, ara, target_rung,
    ...
    fit_tr, fit_ba, fit_mae = grid_search_inv(
        period, ara, target_rung, times, peaks)
    rungs = build_rung_ladder_inv(target_rung, period, ara)
    ...
    for rung_power, rung_period, prefix, eng_ara, cons_ara, offset in rungs:
        ...
        for archetype, ara_val in [("engine", eng_ara), ("clock", 1.0),
            ...
            if prefix == "Target" and archetype == "engine":
                ...
                ba = 1.0
            node = InvertedGearNode(name, rung_period, ara_val,
    ...
    # Wire: INVERTED GEAR
    # Down pipes: DECAYED (engine cascade fades)
    # Up pipes: FULL (consumer propulsion)
    # Horizontal: FULL (gear mesh)
    ...
    for i, (rp, prefix, offset) in enumerate(rung_list):
        ...
        # Down: DECAYED engine cascade
        if i < len(rung_list) - 1:
            below_prefix = rung_list[i + 1][1]
            below_offset = rung_list[i + 1][2]
            down_dist = abs(below_offset)  # distance below target
            down_decay = INV_PHI ** down_dist
            engine.add_feed_target(all_nodes[f"{below_prefix}_engine"],
                                   INV_PHI_4 * down_decay, "vertical_down")

        # Up: FULL consumer propulsion (the gear!)
        if i > 0:
            above_prefix = rung_list[i - 1][1]
            # NO decay — full upward propulsion
...
print("=" * 78)
print("Script 243f — Inverted Gear: Upward FULL, Downward DECAYED")
print("=" * 78)
print()
print("INVERTED from 243e:")
print("  Engine cascade DOWN:      DECAYED (1/φ per step — influence fades)")
print("  Consumer propulsion UP:   FULL STRENGTH (the gear!)")
print("  Horizontal mesh:          FULL (engine↔consumer at each rung)")
print()

ref_243b = {"Solar (SSN)": 56.84, "ENSO (ONI)": 0.43, "Sanriku EQ": 0.60}
ref_243e = {"Solar (SSN)": 62.05, "ENSO (ONI)": 0.44, "Sanriku EQ": 1.23}

print(f"  {'System':<16} │ {'235b':>8} │ {'243b':>8} │ {'243e':>8} │ {'243f':>8} │ {'Best':>6}")
...
for name, times, peaks, period, ara, rung in SYSTEMS:
    ...
    res_inv = run_inverted_gear(times, peaks, period=period, ara=ara,
                                 target_rung=rung, system_name=name)

    all_results[name] = {"base": res_base, "inv": res_inv}

    if res_base["failed"] or res_inv["failed"]:
        print(f"\r  {name:<16} │ {'FAIL':>8} │ {'FAIL':>8} │ {'FAIL':>8} │ {'FAIL':>8} │")
        continue

    b = ref_243b.get(name, 999)
    e = ref_243e.get(name, 999)
    f_mae = res_inv['mae']
    vals = [res_base['mae'], b, e, f_mae]
    labels = ['235b', '243b', '243e', '243f']
    best_i = vals.index(min(vals))
    better = " ✓" if f_mae <= min(res_base['mae'], b, e) else ""
    print(f"\r  {name:<16} │ {res_base['mae']:>8.2f} │ {b:>8.2f} │ {e:>8.2f} │ "
          f"{f_mae:>8.2f} │ {labels[best_i]:>6}{better}")


# Solar per-cycle comparison: 243b vs 243f
print(f"\n{'─' * 78}")
print("SOLAR PER-CYCLE — 243b (uniform) vs 243f (inverted gear)")
print("─" * 78)

ra = all_results["Solar (SSN)"]["inv"]
if not ra["failed"]:
    print(f"\n  {'C':>3} {'Year':>7} {'Act':>6} │ {'243f':>6} {'Err':>5} │")
...
# Summary
print(f"\n{'=' * 78}")
print("COMPLETE 243 SERIES SUMMARY")
print("=" * 78)

ref_243c = {"Solar (SSN)": 53.12, "ENSO (ONI)": 0.46, "Sanriku EQ": 1.43}
ref_243d = {"Solar (SSN)": 56.14, "ENSO (ONI)": 0.48, "Sanriku EQ": 1.61}

print(f"\n  {'System':<16} {'235b':>7} {'243b':>7} {'243c':>7} {'243d':>7} {'243e':>7} {'243f':>7} {'Best':>7}")
print(f"  {'─'*16} {'─'*7} {'─'*7} {'─'*7} {'─'*7} {'─'*7} {'─'*7} {'─'*7}")

for name in all_results:
    rb = all_results[name]["base"]
    ri = all_results[name]["inv"]
    if rb["failed"] or ri["failed"]: continue
    b = ref_243b.get(name, 999)
    c = ref_243c.get(name, 999)
    d = ref_243d.get(name, 999)
    e = ref_243e.get(name, 999)
    vals = [rb['mae'], b, c, d, e, ri['mae']]
    labels = ['235b', '243b', '243c', '243d', '243e', '243f']
    best_i = vals.index(min(vals))
    print(f"  {name:<16} {vals[0]:>7.2f} {vals[1]:>7.2f} {vals[2]:>7.2f} "
          f"{vals[3]:>7.2f} {vals[4]:>7.2f} {vals[5]:>7.2f} {labels[best_i]:>7}")

print(f"\n  KEY FINDINGS:")
print(f"    243b remains only variant that improves ANY system (EQ: -49.9%)")
print(f"    GEOMETRIC ARAs are the mechanism, not whisper or pipe decay")
print(f"    Every decay variant (c,d,e,f) hurt: decay kills what consumers need")
