#!/usr/bin/env python3
"""
Script 182 — Full ARA Range (-2 to +2)
========================================

Dylan's insight: "We don't want to go to 0 (singularity).
We actually want to go -2 and +2 for the full phiARA system."

The ARA scale is 0-to-2, centered at 1.0.  But as a WAVE, the motion
should cover the full ±2 range around center.  That's -2 to +2 in the
centered frame, or -1 to 3 in the raw ARA frame.

Currently, value_to_longitude clips (log_val-C)/R to [-1,1] then arcsin.
With R_matter=1.73 for SSN, values beyond ±1.73 from mean get clipped.
This is why predictions get trapped at extremes — the mapping saturates.

Fix: use R_map = 2.0 for the longitude mapping.
This makes the full ARA range (-2 to +2 from center) fit on the circle.
R_matter still defines the system's character.
R_map defines how much of the circle it can TRAVEL.

Testing:
    - R_map = 2.0 (full ARA range)
    - R_map = PHI (golden ratio — natural boundary)
    - R_map = 2*R_matter (twice the ARA, always covers ±2×ARA)
    - R_map = PHI² (2.618... — extends past ±2)

Combined with the best step (GA/φ) and outer wave (R_matter).
"""
...
def make_predictor(R_map_fn, step_size, R_outer_fn):
    """
    Predictor with configurable:
    - R_map_fn(R_matter): radius for the longitude mapping (how much circle to cover)
    - step_size: angular step per year
    - R_outer_fn(R_matter): radius for the outer wave (amplitude)
    """
    def predict(log_val, C, R_matter, step, t, phase0):
        eff = effective_ara(R_matter, t, phase0)
        eff = max(eff, 0.1)

        # Longitude mapping uses R_map (extended range)
        R_map = R_map_fn(R_matter)
        phi = value_to_longitude(log_val, C, R_map)
        phi_next = phi + step_size * step

        # φ-stable on R_COUPLER (universal grounding)
        def avg_w(pos, R, off):
...
def make_predictor_eff_mapped(R_map_fn, step_size, R_outer_fn):
    """Same but uses effective_ARA in the mapping (like 174 V4)."""
    def predict(log_val, C, R_matter, step, t, phase0):
        eff = effective_ara(R_matter, t, phase0)
        eff = max(eff, 0.1)

        # Use eff in the mapping but extended by R_map
        R_map = R_map_fn(eff)
        phi = value_to_longitude(log_val, C, R_map)
        phi_next = phi + step_size * step
...
R_maps = {
    "Rm":       lambda R: R,                 # Original (ARA itself)
    "2.0":      lambda R: 2.0,               # Full ARA range
    "φ":        lambda R: PHI,               # Golden ratio bound
    "φ²":       lambda R: PHI**2,            # Extended (2.618)
    "2Rm":      lambda R: 2*max(R, 0.1),     # Double ARA
    "Rm+1":     lambda R: R + 1.0,           # ARA + 1 coupler
}
...
if __name__ == '__main__':
    ...
    # Outer wave options
    R_outers = {
        "R1": lambda R: R_COUPLER,
        "√R": lambda R: np.sqrt(max(R, 0.01)),
        "Rm": lambda R: R,
    }

    # Step options (top from 178)
    steps = {
        "GA/φ": GA_OVER_PHI,      # 1.483
        "φ":    PHI,               # 1.618
    }

    print("="*70)
    print("FULL ARA RANGE MAPPING — R_map × step × R_outer")
    print("="*70)

    all_scores = {}

    # Static R_map (no eff modulation in mapping)
    for rmap_name, rmap_fn in R_maps.items():
        for step_name, step_val in steps.items():
            for rout_name, rout_fn in R_outers.items():
                label = f"map={rmap_name} step={step_name} out={rout_name}"
                fn = make_predictor(rmap_fn, step_val, rout_fn)
                sr = run_blind(ssn, cutoffs, ARA_SSN, fn)
                er = run_blind(eq, cutoffs, ARA_EQ, fn)
                all_scores[label] = score_and_report(sr, er, label)

    # Eff-modulated R_map (best combos only to save time)
    for rmap_name in ["2.0", "φ", "Rm+1"]:
        rmap_fn = R_maps[rmap_name]
        for step_name, step_val in steps.items():
            label = f"eff_map={rmap_name} step={step_name} out=Rm"
            fn = make_predictor_eff_mapped(rmap_fn, step_val, R_outers["Rm"])
            sr = run_blind(ssn, cutoffs, ARA_SSN, fn)
            er = run_blind(eq, cutoffs, ARA_EQ, fn)
            all_scores[label] = score_and_report(sr, er, label)
