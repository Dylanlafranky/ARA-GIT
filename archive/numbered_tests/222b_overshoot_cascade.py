#!/usr/bin/env python3
"""
Script 222b — Below Cascade: Overshoot Propulsion

DYLAN'S INSIGHT:
  The distance between log levels IS φ. To go up or down one level,
  energy travels a whole φ-length. The OVERSHOOT is the energy
  exchange — when a system exceeds its balance, the excess propels
  into the adjacent system.

  So the below cascade:
    1. Runs the SAME geometry [φ¹¹, φ⁹, φ⁶, φ⁴] (universal angles)
    2. Has an INVERTED gate (when Sun accumulates, inner releases)
    3. When it overshoots base → excess propels UP to Sun
    4. Attenuated by exp(-φ) for crossing one φ-distance

  This makes the below cascade genuinely DIFFERENT from the main
  cascade — same geometry, opposite ARA state, so the overshoot
  moments don't coincide.

2 models:
  [1] Full deviation × exp(-φ) — both positive and negative excess
  [2] Positive overshoot only × exp(-φ) — only excess above base
"""
...
# =================================================================
# UNIVERSAL CASCADE — same geometry at every level
# =================================================================
def evaluate_cascade(t, ba, tr, gate):
    """
    The universal cascade: [φ¹¹, φ⁹, φ⁶, φ⁴] with sawtooth valve
    and asymmetric tension. Same connection angles at every scale.
    """
    amp = ba
...
# PREDICTION MODEL
# =================================================================
def mk_model(h_cc=INV_PHI_3, overshoot_only=False):
    """
    Three-axis crossing:
    ...
    BELOW CASCADE (overshoot propulsion):
      Same geometry, INVERTED gate (when Sun accumulates, inner releases).
      The cascade overshoots base → excess propels across one φ-distance.
      Attenuation = exp(-φ) for crossing one log level.

      If overshoot_only: only positive excess (amp > base) crosses.
      If not: full deviation crosses (both directions of exchange).

    HORIZONTAL (Hale):
      Previous cycle deviation × cos(π) = -1 × exp(-φ).
    """
    PHI_TRAVEL = np.exp(-PHI)  # attenuation for one φ-distance
    ...
    def _p(years, amps, ba, tr):
        ...
        for i, t in enumerate(years):
            ...
            # === BELOW CASCADE (inverted gate, overshoot propulsion) ===
            # Inner system has OPPOSITE ARA state to Sun:
            #   Sun accumulating (high sa) → inner releasing (low gate)
            #   Sun releasing (low sa) → inner accumulating (high gate)
            inner_acc = 1.0 - sa  # inverted accumulation fraction
            gate_inner = sawtooth_valve(gp, inner_acc)

            below_cascade = evaluate_cascade(t, ba, tr, gate_inner)

            if overshoot_only:
                # Only positive excess propels upward
                overshoot = max(0, below_cascade - ba)
                amp += overshoot * PHI_TRAVEL
            else:
                # Full deviation crosses (bidirectional exchange)
                deviation = below_cascade - ba
                amp += deviation * PHI_TRAVEL
...
if __name__ == '__main__':
    t0 = time.time()
    print("="*70)
    print("SCRIPT 222b — BELOW CASCADE: OVERSHOOT PROPULSION")
    print("="*70)
    print("""
  Below system runs SAME cascade, INVERTED gate.
  Overshoot propels across one φ-distance: exp(-φ) attenuation.

  Key difference from 222: inverted gate makes below cascade
  genuinely different from the main cascade.
""")

    results = []

    # [1] Full deviation (bidirectional)
    print("[1/2] Full deviation × exp(-φ) + Hale...")
    fn1 = mk_model(h_cc=INV_PHI_3, overshoot_only=False)
    r = evaluate("Below full dev", fn1)
    results.append(r)
    print(f"  LOO={r['loo']:.2f}, r={r['rr']:+.3f}, s={r['sw']}/7  [{time.time()-t0:.0f}s]")

    # [2] Positive overshoot only
    print("\n[2/2] Positive overshoot × exp(-φ) + Hale...")
    fn2 = mk_model(h_cc=INV_PHI_3, overshoot_only=True)
    r = evaluate("Below overshoot+", fn2)
    results.append(r)
    print(f"  LOO={r['loo']:.2f}, r={r['rr']:+.3f}, s={r['sw']}/7  [{time.time()-t0:.0f}s]")
    ...
    print(f"{'='*70}")
    print(f"\n  {'Cyc':>3s} {'Obs':>7s} {'Pred':>7s} {'Err':>7s} {'BelowC':>8s}")
    ba_fit, tr_fit = bn['ba'], bn['tr']
    # Show below cascade contribution for each cycle
    for i, c in enumerate(cycle_nums):
        ...
        below_dev = below_c - ba_fit
        below_contrib = below_dev * np.exp(-PHI)
        p = bn['pf'][i]
        print(f"  {c:3d} {peak_amps[i]:7.1f} {p:7.1f} {p-peak_amps[i]:+7.1f} {below_contrib:+8.1f}")
