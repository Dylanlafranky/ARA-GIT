#!/usr/bin/env python3
"""
Script 223l — ARA-Gated Beeswax Collision

DYLAN'S INSIGHT:
  The beeswax corridor has an ARA cycle:

  A (Life): In the chamber together. Both paths connect.
    Parallel pair traveling through the system.
    Balls are paired, interacting gently.

  R (Separation): Junction point. The release moment.
    Balls forced through their own doors. Energy exchange.
    This is THE moment of maximum interaction.

  B (Death): Next straight corridor. NOT connected to mirror.
    Ball travels alone. No mirror interaction.
    Coasting through the system.

  Then: R (rejoin) → A (together again) → ...

  Measuring togetherness: cos(Δphase) between adjacent periods
    cos(Δphase) ≈ +1 → A state (fully together, in phase)
    cos(Δphase) ≈  0 → R state (junction, separating/rejoining)
    cos(Δphase) ≈ -1 → B state (fully apart, anti-phase)

  The champion collision is always-on. The ARA cycle says it
  should have phases: active when together, peak at separation,
  silent when apart.

3 models:
  [1] Togetherness-gated: champion × max(0, cos(Δphase))
      Only fires in A state (together). Silent in B (apart).

  [2] Junction-peaked: collision × sin²(Δphase)
      Strongest at R (junction/separation). Weak at A and B.
      The RELEASE is where the real energy exchange happens.

  [3] Full ARA cycle: gentle in A, peak at R, zero in B
      Separate accumulation and release phases.
"""
...
# =================================================================
# MODEL 1: Togetherness-gated collision
# =================================================================
def mk_together_gated(h_cc=INV_PHI_3):
    """
    Champion mirror collision gated by togetherness.
    Only fires when balls are in A state (together, cos(Δphase) > 0).
    Silent in B state (apart, cos(Δphase) < 0).

    Gate = max(0, cos(Δphase)):
      A (together): gate = 0→1, collision active
      R (junction): gate = 0, transitioning
      B (apart): gate = 0, silent
    """
    def _p(years, amps, ba, tr):
        ...
        for i, t in enumerate(years):
            ...
            for j, per in enumerate(PERIODS):
                ...
                if j > 0:
                    # Champion collision direction
                    mirror = -cos_vals[j-1] * cos_vals[j]

                    # Togetherness gate: only active in A state
                    phase_diff = phases[j-1] - phases[j]
                    togetherness = max(0, np.cos(phase_diff))

                    # Collision only fires when together
                    eps *= (1 + mirror * togetherness * INV_PHI)
...
# =================================================================
# MODEL 2: Junction-peaked collision (R is the release)
# =================================================================
def mk_junction_peaked(h_cc=INV_PHI_3):
    """
    The RELEASE (R) at the junction is where the real energy
    exchange happens. Not when together (A) or apart (B).

    sin²(Δphase) peaks at Δphase = π/2 (the junction between
    A and B states). Zero when fully together or fully apart.

    Collision direction still from champion (-cos·cos) but
    STRENGTH peaks at the separation/rejoining moment.
    """
    def _p(years, amps, ba, tr):
        preds = []
        for i, t in enumerate(years):
            ...
            for j, per in enumerate(PERIODS):
                ...
                if j > 0:
                    ...
                    # Junction strength: peaks at R (separation point)
                    phase_diff = phases[j-1] - phases[j]
                    junction = np.sin(phase_diff)**2  # peaks at Δ=π/2

                    # Collision strongest at the release/junction
                    # Scale by PHI since R is the peak energy moment
                    eps *= (1 + mirror * junction * PHI * INV_PHI)
...
# =================================================================
# MODEL 3: Full ARA cycle
# =================================================================
def mk_ara_cycle(h_cc=INV_PHI_3):
    """
    Full ARA beeswax cycle:

    A (together, cos(Δphase) > 0):
      Gentle accumulation. Balls paired, traveling together.
      Weak positive coupling: builds energy slowly.
      Strength: cos(Δphase) × 1/φ (gentle)

    R (junction, |cos(Δphase)| small):
      Release. Separation/rejoining. Peak energy exchange.
      Strength: sin²(Δphase) × φ (explosive)

    B (apart, cos(Δphase) < 0):
      Death. No mirror interaction. Ball travels alone.
      Strength: 0

    Total collision = A_term + R_term (B is silent)
    Direction from champion mirror (-cos·cos) for both.
    """
    def _p(years, amps, ba, tr):
        preds = []
        for i, t in enumerate(years):
            ...
            for j, per in enumerate(PERIODS):
                ...
                if j > 0:
                    ...
                    # A state: gentle accumulation (only when together)
                    a_term = mirror * max(0, cos_pd) * INV_PHI

                    # R state: peak release at junction
                    r_term = mirror * sin_pd**2 * PHI

                    # B state: silent (apart, no interaction)
                    # (nothing added)

                    eps *= (1 + (a_term + r_term) * INV_PHI)
...
if __name__ == '__main__':
    t0 = time.time()
    print("="*70)
    print("SCRIPT 223l — ARA-GATED BEESWAX COLLISION")
    print("="*70)
    print(f"""
  Beeswax corridor ARA cycle:
    A (Life):  together in chamber, gentle accumulation
    R (Sep.):  junction point, peak energy exchange
    B (Death): apart in next straight, no interaction

  cos(Δphase): +1=together, 0=junction, -1=apart
""")

    results = []

    # [1] Togetherness-gated
    print("[1/3] Togetherness-gated collision...")
    fn1 = mk_together_gated(h_cc=INV_PHI_3)
    r = evaluate("A-gated", fn1)
    results.append(r)
    print(f"  LOO={r['loo']:.2f}, r={r['rr']:+.3f}, s={r['sw']}/7  [{time.time()-t0:.0f}s]")

    # [2] Junction-peaked
    print("\n[2/3] Junction-peaked (R = release)...")
    fn2 = mk_junction_peaked(h_cc=INV_PHI_3)
    r = evaluate("R-peaked", fn2)
    results.append(r)
    print(f"  LOO={r['loo']:.2f}, r={r['rr']:+.3f}, s={r['sw']}/7  [{time.time()-t0:.0f}s]")

    # [3] Full ARA cycle
    print("\n[3/3] Full ARA cycle (A gentle + R peak + B silent)...")
    fn3 = mk_ara_cycle(h_cc=INV_PHI_3)
    r = evaluate("ARA cycle", fn3)
    results.append(r)
    print(f"  LOO={r['loo']:.2f}, r={r['rr']:+.3f}, s={r['sw']}/7  [{time.time()-t0:.0f}s]")
