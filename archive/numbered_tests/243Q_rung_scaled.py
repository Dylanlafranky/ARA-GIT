#!/usr/bin/env python3
"""
Script 243Q — Rung-Scaled Breathing Gear

Dylan: "Is the φ² scaling based on the log it is on? Maybe that is the issue."

243P applied a FLAT tilt_scale to every node. But each rung sits at a
different φ-power period. A node at offset +2 is two φ-steps from target —
the coupling should scale with position on the ladder, not uniformly.

The fix: make tilt amplification rung-aware.
  tilt_scale = φ^|rung_offset|

This way the target rung (offset=0) gets scale=1 (no boost),
one step away gets φ, two steps gets φ², etc.
Each log-step on the ladder naturally scales its own coupling.

Three variants (lean run):
  A — Full mirror + rung-scaled tilt: φ^|offset|
  B — Full mirror + rung-scaled tilt: φ^(|offset|+1) (one extra φ)
  C — Full mirror + rung-scaled tilt: φ^|offset| + soft valve
"""
...
class BreathingGearNode(OrigARANode):
    ...
    def __init__(self, name, period, ara, rung, base_amp, archetype,
        ...
        self.rung_scale = rung_scale       # If True, tilt_scale = φ^|rung_offset|
        self.rung_base = rung_base         # Extra φ-power to add: φ^(|offset|+rung_base)

        # Compute rung-aware tilt scale
        if rung_scale:
            self.tilt_scale = PHI ** (abs(rung_offset) + rung_base)
        else:
...
def run_breathing_gear(times, peaks, period, ara, target_rung,
    ...
    fit_tr, fit_ba, fit_mae = grid_search(
        ...
        soft_valve=soft_valve, rung_scale=rung_scale,
        rung_base=rung_base)
...
VARIANTS = {
    "A": {
        "label": "Rung-scaled: φ^|offset|",
        "child_sign": -1, "parent_sign": -1, "gate_send": True,
        "tilt_scale": 1.0, "soft_valve": False,
        "rung_scale": True, "rung_base": 0,
    },
    "B": {
        "label": "Rung-scaled: φ^(|offset|+1)",
        "child_sign": -1, "parent_sign": -1, "gate_send": True,
        "tilt_scale": 1.0, "soft_valve": False,
        "rung_scale": True, "rung_base": 1,
    },
    "C": {
        "label": "Rung-scaled: φ^|offset| + soft valve",
        "child_sign": -1, "parent_sign": -1, "gate_send": True,
...
print("=" * 78)
print("Script 243Q — Rung-Scaled Breathing Gear")
print("=" * 78)
print()
print("243P used flat scaling. But each rung is on a different φ-power log.")
print("A: φ^|offset|   B: φ^(|offset|+1)   C: φ^|offset| + soft valve")
print()
...
for var_key in ["A", "B", "C"]:
    ...
    print(f"    child_sign={var['child_sign']}, parent_sign={var['parent_sign']}, "
          f"gate_send={var['gate_send']}, rung_scale={var.get('rung_scale',False)}, "
          f"rung_base={var.get('rung_base',0)}")
    print(f"{'─' * 78}")
    ...
    for name, times, peaks, period, ara, rung in SYSTEMS:
        ...
        res = run_breathing_gear(
            ...
            rung_scale=var.get("rung_scale", False),
            rung_base=var.get("rung_base", 0))
...
print(f"\n{'=' * 78}")
print("SUMMARY — 243Q Rung-Scaled Breathing Gear")
print("=" * 78)
print()
print("  243P used flat φ/φ² scaling — too aggressive for Solar engines.")
print("  243Q makes tilt scale φ^|rung_offset| — each log-step scales itself.")
print("  Target rung (offset=0) gets scale=1, ±1 gets φ, ±2 gets φ², etc.")
print()
print(f"  Key question: Does rung-aware scaling let each log-step")
print(f"  breathe at its own natural amplitude?")
print()
